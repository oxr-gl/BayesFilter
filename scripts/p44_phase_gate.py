...
def _require_manifest(root: Path, manifest_path: Path, phase: str, token: str, run_id: str, result: Path, review: Path) -> bool:
    ...
    expected = {
        "schema_version": "p44.phase_evidence.v1",
        "run_id": run_id,
        "phase": phase.upper(),
        "pass_token": token,
        "status": token,
    ...
    result_required_markers = {
        "p44_evidence_manifest": manifest_rel,
    ...
    if not isinstance(commands, list) or not commands:
        ...
        return False
    result_command_count = _marker(result, "p44_command_count")
    if result_command_count != str(len(commands)):
        print(f"phase result marker p44_command_count is {result_command_count!r}, not {len(commands)!r}", file=sys.stderr)
        return False
    for index, command in enumerate(commands):
        ...
        command_markers = {
            "p44_run_id": run_id,
            "p44_phase": phase.upper(),
            "p44_command_index": str(index),
    ...
    if code_loop.get("iterations", 0) < 1 or code_loop.get("verdict") != token:
        ...
        return False
    review_iterations = _marker(review, "p44_claude_code_governance_iterations")
    if review_iterations != str(code_loop.get("iterations")):
        print(
            f"phase review marker p44_claude_code_governance_iterations is {review_iterations!r}, "
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="P44 overnight launch/phase gate.")
    ...
    root = Path(args.root)
    if args.check_runbook_pass:
        ledger = Path(args.runbook_ledger) if args.runbook_ledger else root / "docs/plans/bayesfilter-highdim-zhao-cui-p44-overnight-gated-self-recovery-claude-review-ledger-2026-06-07.md"
        if not ledger.exists():
            print(f"missing runbook ledger: {ledger}", file=sys.stderr)
            return 2
        latest = _latest_status_or_verdict(ledger)
        if latest != "PASS_P44_OVERNIGHT_RUNBOOK":
            print(
                f"runbook ledger latest status/verdict is {latest!r}, not PASS_P44_OVERNIGHT_RUNBOOK",
                file=sys.stderr,
            )
            return 2
        runbook_record = None
    ...
    if args.check_execution_ready:
        result = Path(args.execution_result) if args.execution_result else root / "docs/plans/bayesfilter-highdim-zhao-cui-p44-overnight-gated-self-recovery-execution-result-2026-06-07.md"
        if not result.exists():
            print(f"missing execution result: {result}", file=sys.stderr)
            return 2
        latest = _latest_status_or_verdict(result)
        if latest != "READY_TO_LAUNCH_AFTER_RUNBOOK_PASS":
            print(
                f"execution result latest status/verdict is {latest!r}, not READY_TO_LAUNCH_AFTER_RUNBOOK_PASS",
                file=sys.stderr,
            )
            return 2
        ...
        if args.review_cycle is not None:
            execution_record = _validated_latest_cycle_record(
                result,
                args.review_cycle,
                "READY_TO_LAUNCH_AFTER_RUNBOOK_PASS",
                args.max_review_iteration or 5,
            )
            if execution_record is None:
                return 2

    if args.check_runbook_pass and args.check_execution_ready and args.review_cycle is not None:
        ledger = Path(args.runbook_ledger) if args.runbook_ledger else root / "docs/plans/bayesfilter-highdim-zhao-cui-p44-overnight-gated-self-recovery-claude-review-ledger-2026-06-07.md"
        result = Path(args.execution_result) if args.execution_result else root / "docs/plans/bayesfilter-highdim-zhao-cui-p44-overnight-gated-self-recovery-execution-result-2026-06-07.md"
        runbook_record = _validated_latest_cycle_record(
            ledger,
