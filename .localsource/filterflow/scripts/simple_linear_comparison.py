...
def get_elbos(pf, initial_state, observations_dataset, T, modifiable_transition_values, values, filter_seed):
    elbos = tf.TensorArray(dtype=DTYPE, size=values.shape[0])
    for i in tf.range(values.shape[0]):
        ...
        elbos = elbos.write(tf.cast(i, tf.int32), final_state.log_likelihoods / tf.cast(T, DTYPE))
    return elbos.stack()
...
def main(resampling_method_value, resampling_neff, resampling_kwargs=None, T=150, batch_size=50, n_particles=25,
    ...
    transition_matrix = 0.5 * np.eye(2, dtype=NP_DTYPE)
    transition_covariance = np.eye(2, dtype=NP_DTYPE)
    observation_matrix = np.eye(2, dtype=NP_DTYPE)
    observation_covariance = 0.1 * np.eye(2, dtype=NP_DTYPE)

    values = np.array(list(zip(values, values)), dtype=NP_DTYPE)
    ...
    data, kf = get_data(transition_matrix, observation_matrix, transition_covariance, observation_covariance, T,
                        np_random_state, dtype=NP_DTYPE)
    ...
    init_transition_matrix = (0.5 * np.eye(2) + 0.1 * np_random_state.randn(2, 2)).astype(NP_DTYPE)
    modifiable_transition_matrix = tf.Variable(init_transition_matrix, trainable=True, dtype=DTYPE)
    ...
    initial_particles = np_random_state.normal(0., 1., [batch_size, n_particles, 2]).astype(NP_DTYPE)
