...
def get_data(transition_matrix, observation_matrix, transition_covariance, observation_covariance, T=100,
    ...
    return sample[1].data.astype(NP_DTYPE), kf
...
def get_surface(mesh, modifiable_transition_matrix, pf, initial_state, use_correction_term, observations_dataset, T,
                seed, use_tqdm=False):
    likelihoods = tf.TensorArray(size=len(mesh), dtype=DTYPE, dynamic_size=False, element_shape=[])
    gradients = tf.TensorArray(size=len(mesh), dtype=DTYPE, dynamic_size=False, element_shape=[2])
    i = 0
    for val in mesh:
...
def get_surface_finite_difference(mesh, modifiable_transition_matrix, pf, initial_state, use_correction_term,
    ...
    for val in mesh:
        ...
        ll_eps_list = tf.TensorArray(DTYPE, size=mesh.shape[1])
        ...
        likelihoods = likelihoods.write(tf.cast(i, tf.int32), ll)
        gradients = gradients.write(tf.cast(i, tf.int32), tf.convert_to_tensor(ll_eps_list.stack(), dtype=DTYPE))
        i += 1
...
def get_surface_kf(mesh, kf, np_obs, epsilon, use_tqdm=False):
    ...
    for i, val in iterable:
        transition_matrix = np.diag(val) + np.eye(2, 2, 1, dtype=NP_DTYPE)
        ...
        for n_val in range(n_params):
            ...
            transition_matrix = np.diag(val_eps) + np.eye(2, 2, 1, dtype=NP_DTYPE)
            kf.transition_matrices = transition_matrix
...
def main(resampling_method_value, resampling_neff, resampling_kwargs=None, T=100, batch_size=1, n_particles=25,
    ...
    transition_covariance = v ** 2 * np.array([[1 / 3, 1 / 2],
                                               [1 / 2, 1.]], dtype=NP_DTYPE)

    observation_matrix = np.array([[1., 0]], dtype=NP_DTYPE)
    observation_covariance = np.array([[t ** 2]], dtype=NP_DTYPE)

    resampling_method_enum = ResamplingMethodsEnum(resampling_method_value)

    x_linspace = np.linspace(0.95, 1., mesh_size).astype(NP_DTYPE)
    y_linspace = np.linspace(0.95, 1., mesh_size).astype(NP_DTYPE)
    ...
    modifiable_transition_matrix = tf.Variable(transition_matrix, trainable=False, dtype=DTYPE)
    observation_matrix = tf.convert_to_tensor(observation_matrix, dtype=DTYPE)
    ...
    initial_particles = np_random_state.normal(0., .01, [batch_size, n_particles, 2]).astype(NP_DTYPE)
    initial_state = State(tf.constant(initial_particles, dtype=DTYPE))
