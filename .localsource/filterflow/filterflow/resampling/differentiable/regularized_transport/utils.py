...
def diameter(x, y):
    ...
    return tf.where(res == tf.constant(0., dtype=res.dtype), tf.ones_like(res), res)
...
def squared_distances(x: tf.Tensor, y: tf.Tensor) -> tf.Tensor:
    ...
    return tf.clip_by_value(
        xx - tf.constant(2., dtype=dtype) * xy + yy,
        tf.constant(0., dtype=dtype),
        tf.constant(float('inf'), dtype=dtype),
    )
...
def cost(x: tf.Tensor, y: tf.Tensor) -> tf.Tensor:
    ...
    :rtype tf.Tensor
    """
    return squared_distances(x, y) / tf.constant(2., dtype=x.dtype)
