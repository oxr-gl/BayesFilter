...
class NeffCriterion(ResamplingCriterionBase):
    ...
    def apply(self, state: State):
        ...
        :rtype tf.Tensor
        """
        dtype = state.log_weights.dtype
        threshold = (
            tf.cast(self._threshold, dtype)
            if not self._is_relative
            else tf.cast(state.n_particles, dtype) * tf.cast(self._threshold, dtype)
        )
        if self._on_log:
...
class AlwaysResample(ResamplingCriterionBase):
    ...
    def apply(self, state: State):
        return tf.ones(state.batch_size, dtype=tf.bool), tf.zeros([state.batch_size], dtype=state.log_weights.dtype)


class NeverResample(ResamplingCriterionBase):

    def apply(self, state: State):
        return tf.zeros(state.batch_size, dtype=tf.bool), tf.zeros([state.batch_size], dtype=state.log_weights.dtype)
