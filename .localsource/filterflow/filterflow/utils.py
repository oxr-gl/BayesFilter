...
def normalize(weights, axis, n, log=True):
    ...
    if log:
        normalized_weights = tf.clip_by_value(
            _normalize(weights, axis, True),
            tf.constant(-1e3, dtype=dtype),
            tf.constant(0., dtype=dtype),
        )
        stop_gradient_mask = normalized_weights < tf.maximum(
            tf.constant(MIN_ABSOLUTE_LOG_WEIGHT, dtype=dtype),
            tf.constant(MIN_RELATIVE_LOG_WEIGHT, dtype=dtype) * float_n,
        )
    else:
        normalized_weights = _normalize(weights, axis, False)
        stop_gradient_mask = normalized_weights < tf.maximum(
            tf.constant(MIN_ABSOLUTE_WEIGHT, dtype=dtype),
            tf.constant(MIN_RELATIVE_WEIGHT, dtype=dtype) ** float_n,
        )
    float_stop_gradient_mask = tf.cast(stop_gradient_mask, dtype)
    return tf.stop_gradient(float_stop_gradient_mask * normalized_weights) + (
            tf.constant(1., dtype=dtype) - float_stop_gradient_mask) * normalized_weights
