    ...
    def __attrs_post_init__(self):
        if self.particles is not None:
            ...
            if self.log_weights is None:
                float_n = tf.cast(self.n_particles, dtype)
                log_weights = tf.zeros([self.batch_size, self.n_particles], dtype=dtype) - tf.math.log(float_n)
            ...
            if self.log_likelihoods is None:
                object.__setattr__(self, 'log_likelihoods', tf.zeros([self.batch_size], dtype=dtype))
            ...
            if self.resampling_correction is None:
                object.__setattr__(self, 'resampling_correction', tf.zeros([self.batch_size], dtype=dtype))

            if self.t is None:
                object.__setattr__(self, 't', tf.constant(0, dtype=tf.int32))

            if self.ess is None:
                object.__setattr__(self, 'ess', tf.zeros([self.batch_size], dtype=dtype) + tf.cast(self.n_particles, dtype))

            if self.collapse is None:
                object.__setattr__(self, 'collapse', tf.ones([self.batch_size], dtype=dtype))
    ...
    def __attrs_post_init__(self):
        ...
        if self.particles is not None:
            if self.rnn_state is None:
                rnn_state = tf.zeros([self.batch_size, self.n_particles, self.dimension], dtype=self.particles.dtype)
