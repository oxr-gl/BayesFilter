...
class _ScalarCubicAdditiveGaussianSSM:
    ...
    def initial_log_density(self, theta: tf.Tensor, x0: tf.Tensor) -> tf.Tensor:
        ...
        return distribution.log_prob(points)
    ...
    def transition_log_density(
        ...
        return distribution.log_prob(current)
...
def _structural_model(theta: tf.Tensor, dim: int) -> TFStructuralStateSpace:
    ...
    return TFStructuralStateSpace(
        ...
        config=StructuralFilterConfig(
            ...
            approximation_label="p44_m2_cubic_additive_gaussian_cut4_same_target_approximation",
        ),
        ...
        name=f"p44_m2_cubic_additive_gaussian_dim_{dim}",
    )
...
def _dense_config(axis: int, order: int) -> highdim.FixedBranchFilterConfig:
    return highdim.FixedBranchFilterConfig(
        ...
        deterministic_seed=f"p44-m2-dense-axis-{axis}-order-{order}",
...
def _tt_config(axis: int) -> highdim.FixedBranchFilterConfig:
    ...
    return highdim.FixedBranchFilterConfig(
        ...
        deterministic_seed=f"p44-m2-tt-axis-{axis}",
...
def _dense_panel_value(theta: tf.Tensor, dim: int, order: int = 241) -> tf.Tensor:
    terms = [_dense_scalar_value(theta, axis, order) for axis in range(dim)]
    return tf.reduce_sum(tf.stack(terms))


def _kalman_linear_value(theta: tf.Tensor, dim: int) -> tf.Tensor:
    result = highdim.FixedBranchSquaredTTFilter(_kalman_config(dim)).log_likelihood(
...
def _kalman_config(dim: int) -> highdim.FixedBranchFilterConfig:
    return highdim.FixedBranchFilterConfig(
        ...
        measure_convention=_convention(),
        deterministic_seed=f"p44-m2-linear-kalman-dim-{dim}",
    )
...
def _tt_panel_value(theta: tf.Tensor, dim: int) -> tf.Tensor:
    ...
    for axis in range(dim):
        result = highdim.scalar_nonlinear_fixed_design_tt_value_path(
            ...
            _observations(axis + 1)[:, axis : axis + 1],
            _tt_config(axis),
            fixture_id=f"p44.m2.cubic-additive-gaussian.axis-{axis}.v1",
            branch_seed_prefix=f"p44-m2-cubic-additive-gaussian-axis-{axis}",
...
def test_p44_m2_nested_linear_dense_reference_matches_exact_kalman_dims_1_2_3() -> None:
    theta = _linear_theta0()
    for dim in (1, 2, 3):
        dense_value, dense_score = _value_and_score(
            lambda current_theta: _dense_panel_value(current_theta, dim, order=241),
            theta,
        )
        kalman_value, kalman_score = _value_and_score(
            lambda current_theta: _kalman_linear_value(current_theta, dim),
            theta,
        )
        tf.debugging.assert_near(dense_value, kalman_value, atol=3e-12, rtol=3e-12)
        tf.debugging.assert_near(dense_score[:4], kalman_score[:4], atol=3e-11, rtol=3e-11)
        assert bool(tf.math.is_finite(dense_score[4]).numpy())
        tf.debugging.assert_near(kalman_score[4], tf.constant(0.0, dtype=DTYPE), atol=1e-14)


def test_p44_m2_nested_linear_cut4_structural_timing_matches_exact_kalman_dims_1_2_3() -> None:
    theta = _linear_theta0()
    for dim in (1, 2, 3):
        kalman_value, kalman_score = _value_and_score(
            lambda current_theta: _kalman_linear_value(current_theta, dim),
...
def test_p44_m2_dense_reference_refines_value_and_score_dims_1_2_3() -> None:
    theta = _theta0()
    for dim in (1, 2, 3):
        low_value, low_score = _value_and_score(
            lambda current_theta: _dense_panel_value(current_theta, dim, order=161),
            theta,
        )
...
def test_p44_m2_cut4_and_zhaocui_match_dense_statistically_for_value_and_score_dims_1_2_3() -> None:
    theta = _theta0()
    for dim in (1, 2, 3):
        dense_value, dense_score = _value_and_score(
            lambda current_theta: _dense_panel_value(current_theta, dim, order=241),
        ...
        tf.debugging.assert_less(cut4_value_gap, tf.constant(6e-3 * dim, dtype=DTYPE))
        tf.debugging.assert_less(cut4_score_gap, tf.constant(1.5e-2 * dim, dtype=DTYPE))
        tf.debugging.assert_less(cut4_score_rel, tf.constant(2.5e-2, dtype=DTYPE))
        tf.debugging.assert_less(tt_value_gap, tf.constant(4e-3 * dim, dtype=DTYPE))
...
def test_p44_m2_cut4_metadata_keeps_same_target_approximation_boundary() -> None:
    result = tf_svd_cut4_filter(
        _observations(3),
    ...
    assert result.metadata.approximation_label == "p44_m2_cubic_additive_gaussian_cut4_same_target_approximation"
