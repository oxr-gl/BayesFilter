...
Their explicit tensor points are:
\begin{itemize}[leftmargin=0.28in]
...
signed combination and duplicate merge are done, the cloud is a small,
low-interaction point set rather than the full 27-point tensor product.
...
The full 27-point tensor-product picture has collapsed to six axis points after
the signed combination and duplicate merge.  That is the concrete 3D object the
later high-dimensional notation is abstracting.
...
\section{Benchmark And Verification}
\label{sec:p40-benchmark-verification}

The preceding sections define a deterministic approximate likelihood and a
same-scalar derivative for one Gaussian-surrogate lane.  They do not solve the
separate problem of how to compare this lane with Kalman, UKF, square-root
sigma-point, CUT, Zhao--Cui tensor-density, bootstrap particle, and
particle-flow filters on nonlinear models where an exact likelihood oracle is
not available.  This section states the benchmark methodology that should be
used for that comparison.

The central change is to stop requiring an exact nonlinear reference in every
cell of the filter ladder.  Where an exact reference exists, as in a linear
Gaussian state-space model, the benchmark should still report exact
value-gradient-Hessian errors against the Kalman likelihood
\citep{durbin2012time}.  For nonlinear models, however, the primary benchmark
object is the likelihood geometry at known data-generating truth.  This is a
weaker claim than exact nonlinear correctness, but it is the right claim for a
filtering program that must choose among approximate filters before a full
Bayesian-estimation phase.

\subsection{Synthetic Truths And The Benchmark Coordinates}
\label{subsec:p40-synthetic-truths}

Each model row must first declare benchmark coordinates
\(\phi\in\R^p\) and a differentiable transformation
\begin{equation}
  \theta=\tau(\phi)
  \label{eq:p40-benchmark-transform}
\end{equation}
from unconstrained benchmark coordinates to the physical model parameters.  The
primary score and Hessian tables are reported in the \(\phi\)-coordinates.  If a
filter computes derivatives with respect to physical parameters, the score must
be transformed by
\begin{equation}
  g_\phi
  =
  J_\tau(\phi)^\top g_\theta,
  \qquad
  J_\tau(\phi)=\frac{\partial \tau(\phi)}{\partial \phi}.
  \label{eq:p40-score-coordinate-transform}
\end{equation}
For Hessians the transformation includes the second derivative of the parameter
map:
\begin{equation}
  H_\phi
  =
  J_\tau^\top H_\theta J_\tau
  +
  \sum_{k=1}^{p_\theta} g_{\theta,k}
  \frac{\partial^2 \tau_k(\phi)}{\partial \phi\,\partial\phi^\top}.
  \label{eq:p40-hessian-coordinate-transform}
\end{equation}
If the full Hessian transformation in
\eqref{eq:p40-hessian-coordinate-transform} is not implemented and reviewed,
the Hessian cell is recorded as unavailable with a transform-gap reason rather
than being treated as a curvature failure.

The benchmark then draws a batch of truth parameters from a calibrated
benchmark design distribution:
\begin{align}
  \phi_{0,b}&\sim \Pi_{\mathrm{bench}},
  &b&=1,\ldots,B,
  \label{eq:p40-truth-draws}\\
  Y_{b,r,1:T}&\sim p_{\tau(\phi_{0,b})}(y_{1:T}),
  &r&=1,\ldots,R.
  \label{eq:p40-data-draws}
\end{align}
The distribution \(\Pi_{\mathrm{bench}}\) is not automatically the full
Bayesian prior.  It is a reproducible benchmark design distribution.  Its core
lane should cover well-posed, scientifically plausible parameters; a separate
stress lane may include boundary, near-nondegenerate, or intentionally difficult
draws.  The draw manifest must record proposed draws, accepted draws, rejected
draws, rejection reasons, and random seeds.  Acceptance rules must cover at
least stationarity when relevant, positivity constraints, identifiability
constraints, finite simulated observations, finite initial filtering objects,
and numerical admissibility.

For algorithm \(a\), data replicate \(r\), and truth draw \(b\), define
\begin{align}
  \bar\ell_{a,b,r}
  &=
  T^{-1}\log p_a(Y_{b,r,1:T}\mid \tau(\phi_{0,b})),
  \label{eq:p40-average-loglik}\\
  \bar g_{a,b,r}
  &=
  T^{-1}
  \nabla_\phi
  \log p_a(Y_{b,r,1:T}\mid \tau(\phi_{0,b})),
  \label{eq:p40-average-score}\\
  \bar H_{a,b,r}
  &=
  T^{-1}
  \nabla_\phi^2
  \log p_a(Y_{b,r,1:T}\mid \tau(\phi_{0,b})).
  \label{eq:p40-average-hessian}
\end{align}
For stochastic filters, such as bootstrap DPF and LEDH-PFPF Algorithm 1, an
additional filter seed index is required:
\begin{equation}
  \bar\ell_{a,b,r,s},\qquad
  \bar g_{a,b,r,s},\qquad
  \bar H_{a,b,r,s},
  \qquad s=1,\ldots,S.
  \label{eq:p40-stochastic-filter-replicates}
\end{equation}
This nested design separates variation across truth parameters, variation
across synthetic datasets at a fixed truth, and Monte Carlo variation internal
to the filter.

\subsection{Expected Score And Long-Run Variance}
\label{subsec:p40-score-asymptotics}

For the exact likelihood, standard likelihood theory for hidden Markov and
state-space models gives the population score identity
\begin{equation}
  \E_{\theta_0}
  \left[
  ...
  =
  0
  \label{eq:p40-score-identity}
\end{equation}
under regularity conditions.  In the state-space and hidden-Markov setting, the
same asymptotic viewpoint is developed in the filtering and HMM likelihood
literature; see, for example, \citet{cappe2005hmm},
\citet{douc2004markovregime}, and \citet{douc2011consistencyhmm}.  Equation
\eqref{eq:p40-score-identity} is a population statement.  It does not say that a
single finite synthetic dataset has zero score.

Let \(s_t(\theta_0)\) denote a stationary score-increment representation, or a
stable approximation to the contribution of time \(t\).  Under the usual
forgetting and mixing conditions used in HMM likelihood asymptotics,
\begin{equation}
  T^{-1/2}
  \sum_{t=1}^{T} s_t(\theta_0)
  \Rightarrow
  \N(0,\mathcal I(\theta_0)),
  \label{eq:p40-score-clt}
\end{equation}
where \(\mathcal I(\theta_0)\) is the long-run score covariance, equivalently a
Fisher-information object under correct specification.  Therefore the total
score is typically \(O_p(\sqrt T)\), while the average score is
\(O_p(T^{-1/2})\).  This scaling is the reason the benchmark reports average
score and uncertainty rather than expecting a raw score of zero.

For a scalar per-time diagnostic \(q_t\), such as a log-likelihood increment or
one coordinate of a score increment, define the long-run variance
\begin{equation}
  \sigma_q^2
  =
  \gamma_0
  ...
  \qquad
  \gamma_k=\mathrm{Cov}(q_t,q_{t-k}).
  \label{eq:p40-long-run-variance}
\end{equation}
The standard error of the mean over \(R\) independent synthetic series of length
\(T\), conditional on one truth draw, is approximately
\begin{equation}
  \mathrm{SE}\{\bar q(\phi_{0,b})\}
  \approx
  \sqrt{\frac{\sigma_{q,b}^2}{RT}}.
  \label{eq:p40-conditional-se}
\end{equation}
The long-run variance may be estimated by a HAC estimator such as
\citet{neweywest1987} or by a reviewed batch-means estimator.  The estimator,
lag rule, and batch rule are part of the benchmark manifest.

When the benchmark averages over truth draws as well as datasets, the variance
has an additional between-truth term.  If
\(\mu_b=\E(q\mid\phi_{0,b})\), then a balanced first-order decomposition is
\begin{equation}
  \mathrm{Var}\!\left(
  \frac{1}{B}\sum_{b=1}^{B}\bar q_b
  \right)
  \approx
  \frac{\mathrm{Var}_b(\mu_b)}{B}
  +
  \frac{1}{B^2}\sum_{b=1}^{B}
  \frac{\sigma_{q,b}^2}{RT}.
  \label{eq:p40-between-truth-variance}
\end{equation}
Thus \(T\) and \(R\) reduce conditional data uncertainty at a fixed truth, while
\(B\) measures robustness across plausible truths.  The benchmark must not hide
between-truth variation by reporting only a pooled standard error.

\subsection{Choosing The Series Length And Seed Counts}
\label{subsec:p40-horizon-calibration}

There is no universal answer to how long the synthetic time series should be.
The length \(T\) is chosen by the desired precision of average log likelihood
and average score.  If \(\epsilon\) is the target half-width for a scalar
diagnostic, \(z\) is a confidence multiplier, and \(\sigma_q\) is the estimated
long-run standard deviation, a fixed-truth calibration uses
\begin{equation}
  T
  \ge
  \left(
  \frac{z\sigma_q}{\epsilon\sqrt R}
  \right)^2.
  \label{eq:p40-horizon-rule}
\end{equation}
A practical pilot ladder is
\begin{equation}
  T\in\{512,1024,2048,4096,8192\},
  \qquad
  R\in\{16,32\}.
  \label{eq:p40-pilot-ladder}
\end{equation}
The final \(T\) is the smallest horizon for which the conditional standard
errors meet their predeclared targets for every pilot truth or for a declared
high quantile, such as the 90th or 95th percentile across truth draws.  The same
pilot must check that componentwise standardized mean scores and replicate-level
curvature summaries are stable when \(T\) doubles.

For stochastic filters, the number of filter seeds \(S\) is also calibrated.
Use a seed ladder such as
\begin{equation}
  S\in\{4,8,16,32\},
  \label{eq:p40-seed-ladder}
\end{equation}
and stop only when the filter Monte Carlo standard error is small relative to
the data standard error:
\begin{equation}
  \mathrm{MCSE}
  \le
  \kappa\,\mathrm{DataSE}.
  \label{eq:p40-mcse-rule}
\end{equation}
The constant \(\kappa\) is fixed before execution, for example
\(\kappa=0.25\).  The rule applies to the average log likelihood, every
canonical score coordinate, and every reported scalar score summary.  If the
maximum planned \(S\) does not satisfy \eqref{eq:p40-mcse-rule}, the cell is
still reportable but receives an `mc-noise-dominant' flag and must not be ranked
against deterministic filters by small differences.  Particle score and
observed-information estimates are known to have special variance and
computational-cost issues; see \citet{poyiadjis2011particle} and the
linear-cost follow-up of \citet{nemeth2013particle}.

\subsection{Capability, Curvature, And Tables}
\label{subsec:p40-benchmark-tables}

Every benchmark artifact must preserve a row/draw/algorithm capability
crosswalk.  For each tuple
\((\hbox{model row},\hbox{truth draw},\hbox{algorithm})\), the manifest records
\begin{center}
\small
...
field & meaning \\
\midrule
capability status & value only, value plus score, value plus score plus Hessian,
diagnostic derivative only, or unavailable with reason \\
score coordinate system & canonical \(\phi\)-coordinate system or declared
physical-coordinate exception \\
score derivative provenance & native canonical derivative, chain-rule converted
physical derivative, fixed-branch diagnostic, or gap \\
Hessian coordinate system or reason & canonical \(\phi\)-coordinate system,
declared exception, or unavailable coordinate reason \\
Hessian derivative provenance or gap & native, reviewed full transform, partial
transform, diagnostic, or unavailable transform gap \\
diagnostic-only reason & fixed branch, resampling derivative, stochastic
surrogate, finite-difference-only path, or other reviewed diagnostic label \\
not-available reason & branch veto, missing adapter, transform gap, unsupported
derivative, nonfinite run, or other reviewed reason \\
\bottomrule
\end{tabular}
\end{center}
This crosswalk prevents an unavailable derivative from being confused with a
failed derivative, and prevents a diagnostic-only derivative from being promoted
to a main score-ladder result.

Curvature is reported replicate by replicate.  Since the convention here is log
likelihood, local maximum curvature corresponds to negative-definite Hessian, or
positive-definite \(-\bar H_{a,b,r}\).  The primary curvature table therefore
uses
\begin{equation}
  \lambda_{\min}(-\bar H_{a,b,r})
  \label{eq:p40-curvature-statistic}
\end{equation}
and reports its mean, median, quantiles, and positive-definite fraction.
Eigenvalues of a mean Hessian may be reported as explanatory diagnostics only,
because averaging Hessians before eigendecomposition can hide replicate-level
curvature failures.

The main ladder should contain the following tables:
\begin{enumerate}[leftmargin=0.28in]
  \item a value table with mean average log likelihood, standard error, and
  failure rate;
  \item a componentwise score artifact with signed mean score, standard error,
  confidence interval, and standardized mean for each coordinate;
  \item a score-summary table with norm, maximum component, minimum component,
  and maximum absolute standardized component;
  \item a curvature table based on
  \(\lambda_{\min}(-\bar H_{a,b,r})\) and positive-definite fraction;
  \item a stochastic-filter uncertainty table separating data standard error
  from filter Monte Carlo standard error and recording ESS, resampling, and
  degeneracy diagnostics;
  \item a truth-robustness table showing between-truth variation, worst-truth
  diagnostics, and failure rates by truth draw;
  \item an LGSSM exact-reference table with exact value, score, and Hessian
  errors where the Kalman reference applies.
\end{enumerate}

The interpretation rule is deliberately conservative.  A higher approximate
log likelihood at the generating truth is not a win if the expected-score,
curvature, failure-rate, or Monte Carlo-uncertainty diagnostics veto the
comparison.  Conversely, a near-zero expected score is evidence of plausible
local likelihood geometry, not a proof that the approximate filter computes the
exact nonlinear likelihood.  This is the boundary that makes the benchmark
useful without making it dishonest.
...
synthetic-truth benchmark geometry & HMM likelihood asymptotics in \citet{cappe2005hmm}, \citet{douc2004markovregime}, \citet{douc2011consistencyhmm}; HAC variance in \citet{neweywest1987}; particle score variance in \citet{poyiadjis2011particle,nemeth2013particle} & Section~\ref{sec:p40-benchmark-verification} \\
\bottomrule
\end{tabular}
\end{center}
...
synthetic-truth benchmark design & Section~\ref{sec:p40-benchmark-verification}, \eqref{eq:p40-average-loglik}--\eqref{eq:p40-average-hessian}, \eqref{eq:p40-horizon-rule}, \eqref{eq:p40-mcse-rule} \\
\bottomrule
\end{tabular}
\end{center}
