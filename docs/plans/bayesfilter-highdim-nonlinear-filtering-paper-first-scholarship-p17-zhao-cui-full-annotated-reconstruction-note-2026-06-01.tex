...
\title{A Full Annotated Equation Reconstruction of Zhao--Cui Tensor-Train Sequential Filtering}
\author{BayesFilter P17 Technical Note}
\date{2026-06-01}
...
\begin{abstract}
This note reconstructs Zhao and Cui's Sections 1--3 and 5 in BayesFilter
notation.  The goal is not to summarize the paper, but to slow down the
displayed mathematics until the method can be followed by a reader who knows
Bayes' rule and calculus but has not studied tensor trains or transport maps.
The exposition follows the paper's order: state-space model and marginal
learning tasks, recursive posterior approximation, functional tensor trains,
squared-TT nonnegativity, marginalization and conditional
Knothe--Rosenblatt maps, particle correction, path smoothing, and
preconditioning.  Only after that reconstruction does the note introduce the
BayesFilter fixed-branch extension needed for a same-scalar analytical
derivative.
\end{abstract}
...
To turn the bridge approximation into a preconditioning map, Zhao--Cui first
...
Algorithm 5 then explains how to retain the filter after preconditioning.  Use
...
\[
    ...
    \tag{P14}
\]
Steps (b.1)--(b.3) build \(\widehat\rho_t\), \(T_t^\ell\), and
\(\widehat\nu_t^\sharp\).  Step (c.1) integrates the last block
\((u_{t-1})\) of \(\widehat\nu_t^\sharp\) to obtain
\[
...
\[
    ...
    \tag{P16}
\]
the retained marginal density for the next filtering step is
\[
