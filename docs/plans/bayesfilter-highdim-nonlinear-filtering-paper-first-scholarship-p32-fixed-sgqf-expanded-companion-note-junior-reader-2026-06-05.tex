...
\title{An Expanded Mathematical Companion To Fixed Sparse-Grid Quadrature Filtering And Its Analytical Gradient}
\author{Chak Wong}
\date{2026-06-03}
...
\begin{abstract}
This note develops fixed sparse-grid quadrature filtering as a self-contained
high-dimensional filtering proposal for the regime where exact nonlinear
filtering is unavailable but a deterministic value-and-gradient surrogate is
still scientifically useful.  The construction starts from the sparse-grid
quadrature nonlinear filter of Jia, Xin, and Cheng
\cite{jia2012sparsegrid}, reconstructs the Gaussian approximation and
sparse-grid formulas in source order, and then defines a fixed approximate
likelihood scalar with an analytical same-scalar gradient.  The resulting
scalar is a deterministic Gaussian-projection likelihood: it is a candidate
fixed target for gradient-based parameter learning and HMC experiments when
branch diagnostics remain stable, but it is not the exact nonlinear filtering
likelihood and it does not store the full posterior density.  The report is
written to answer two questions at once: whether a scientifically strong but
non-specialist review chair can judge the proposal coherent and persuasive, and
whether an implementation engineer can reconstruct the full value-and-gradient
method from the document alone.  The final sections position this proposal
against neighboring Gaussian filters, particle methods, and the Zhao--Cui
squared tensor-train lane \cite{zhao2024ttsequential}: fixed SGQF is the
transparent deterministic Gaussian-surrogate lane, while squared TT is the
richer non-Gaussian density-approximation lane.
\end{abstract}
...
Gaussian expectations.  The next section therefore narrows from the abstract
moment equations of the Gaussian approximation filter to the concrete
one-dimensional and tensor-product quadrature ingredients from which the fixed
...
The sparse-grid construction answers a specific question.  Suppose the filter
needs a Gaussian expectation
\begin{equation}
  \mathcal I_b[F]
  =
...
\end{equation}
Section 20 therefore is not a general survey.  It asks a narrower mathematical
