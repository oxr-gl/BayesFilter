...
\title{An Integrated Annotated Companion to Zhao--Cui Tensor-Train Sequential Filtering and Fixed-Branch Gradients}
\author{BayesFilter P20 Technical Note}
\date{2026-06-01}
...
\begin{abstract}
This integrated note reconstructs Zhao and Cui's Sections 1--3 and 5 in
BayesFilter notation and then carries the fixed-branch analytical-gradient
extension far enough for a skeptical mixed panel.  The first part preserves the
source-order annotated companion: state-space model and marginal learning
tasks, recursive posterior approximation, functional tensor trains,
squared-TT nonnegativity, marginalization and conditional Knothe--Rosenblatt
maps, particle correction, path smoothing, and preconditioning.  The second
part imports the chair-readable fixed-branch gradient derivation: normalizing
constants, squared approximations, tensor-train mass contractions, fixed linear
solves, carried filters, same-scalar propositions, and a finite-difference
protocol.  The result is one reader-facing document rather than a short
supplement.
\end{abstract}

\tableofcontents

\section{Reader Contract And Notation}
...
\[
    ...
    \tag{P7k}
\]
These are not cosmetic checks.  They test whether the density chain still
...
BayesFilter fixed-branch setup.  The next part replaces the earlier compressed
derivative tail with the accepted chair-readable expansion.  It deliberately
starts again from scalar normalizers before returning to tensor trains, because
the panel must be able to see that the gradient differentiates the same
approximate scalar computed by the fixed branch.
...
test and a later implementation must obey.

\section{Chair-Readable Fixed-Branch Gradient: Motivation}
...
\section{Integrated Reader-Facing Conclusion}
