...
\title{A Voice-And-Emphasis Rewrite of the Tensor-Train Sequential Filtering Companion, Fixed-Branch Gradients, Large-Scale Validation, And An Expanded Algorithm 5(c.2) Derivation}
\author{Chak Wong}
\date{2026-06-12}
...
Some chapters answer it by unpacking the filtering target; some by explaining the
tensor-train approximation family; some by following the fixed-branch derivative
through the same computational graph; and the final chapters answer it by showing
what must be checked before the approximation is scientifically credible.

\paragraph{What the reader should focus on.}
The most important distinction in the whole note is between three things that are
easy to blur together if read too quickly:
\begin{enumerate}[leftmargin=0.28in]
  \item the exact Bayesian filtering object one would ideally propagate,
  \item the adaptive Zhao--Cui approximation family that numerically replaces it,
  \item the fixed-branch scalar and derivative studied later in this note.
\end{enumerate}
The chapter sequence is organized so that those three layers become visible one
...
The earlier chapters define the approximation family and the sequential filtering
object.  This chapter now asks the implementation question that matters most for
a same-scalar likelihood lane: once the adaptive approximation exists, what
exactly must be frozen in order for the forward pass to define one reproducible
scalar?  The point of the fixed-branch construction is not merely to list stored
fields.  The point is to identify which numerical and structural choices belong
to the scalar itself, which are allowed to vary with the external parameter, and
which apparently small implementation changes would silently define a different
mathematical object.
...
first, then studies the scalar computed by the resulting fixed computational
route.
...
\subsection{Fixed-Branch Objects And Array Shapes}
...
excursions.  Their role is to expose the few elementary derivative ideas from
which the full fixed-branch gradient is assembled.
