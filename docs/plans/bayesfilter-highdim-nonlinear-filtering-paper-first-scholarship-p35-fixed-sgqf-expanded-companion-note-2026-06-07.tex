...
\end{equation}
In Toy case A, the role of \(\psi\) is played by the scalar nonlinear integrand
\begin{equation}
  \psi(\xi)=(m+C\xi)^2,
  \label{eq:p35-scalar-psi}
\end{equation}
so the one-dimensional rule simply says: replace the standard-normal integral in
\eqref{eq:p34-standardized-integral} by a weighted sum of a few standardized
abscissae.  This is the simplest possible picture of Gaussian quadrature in the
present note.
...
\paragraph{How to read this section with the toy cases in mind.}
Keep returning mentally to Toy case B.  There, the direct tensor-product rule
uses the full nine-point grid in \eqref{eq:p35-nine-point-grid}.  Sparse-grid
combination should be read as a structured way to approximate the same Gaussian
moment target without paying for every point of the full tensor product at every
level.  The formal index sets, coefficients, and duplicate-node merges below are
not separate ideas; they are the bookkeeping machinery that turns that intuition
into a reproducible fixed cloud.
...
\end{equation}
In Toy case A, this is just the scalar standard-normal integral in
\eqref{eq:p34-standardized-integral}.  In Toy case B with two coordinates, it is
the two-dimensional Gaussian-weighted integral approximated by the nine-point
tensor grid in \eqref{eq:p35-nine-point-grid}.  The higher-dimensional notation
\(\mathcal I_b[F]\) is therefore the common umbrella covering those simple
examples and the later filtering moments.
...
To keep the 2D toy case in view, imagine \(b=2\).  Then the active band says:
not every tensor-product level is kept.  Instead, only a narrow range of total
multi-index levels is retained.  This is the first place where sparse-grid
construction departs from the full nine-point tensor-product picture.  The rule
is no longer ``use every product level equally''; it is ``keep a structured band
of tensor levels whose total index lies in a narrow admissible range.''
...
In the 2D toy case, these signed coefficients are the formal reason the sparse
rule is not just a smaller positive-weight subset of the nine-point tensor grid.
It is a signed combination of several lower-level tensor rules.  Some points are
added more than once, some overlap, and some overlapping contributions must be
subtracted.  This is exactly why duplicate-node merging matters later: after the
signed combination is expanded into explicit tensor points, the same
standardized location can arise from more than one tensor component.
...
Each tensor rule \(I_{\mathbf i}\) has points and weights
\begin{align}
  \xi^{\mathbf i,\alpha}
  &=
  (\zeta_{i_1,\alpha_1},\ldots,\zeta_{i_b,\alpha_b}),
...
Seen through Toy case B, this is the final conceptual step.  The tensor-product
picture starts with explicit point sets such as the nine-point grid in
\eqref{eq:p35-nine-point-grid}.  Sparse-grid construction then says: combine
selected tensor rules with signed coefficients, collect all resulting point
contributions in one dictionary, merge repeated standardized nodes, and only then
store the final fixed cloud.  The cloud used later by the filter is therefore not
a raw tensor component and not a symbolic Smolyak formula.  It is the merged,
sorted, weighted list \(\mathcal C_{b,L}\) produced after all of that
bookkeeping has been completed.
