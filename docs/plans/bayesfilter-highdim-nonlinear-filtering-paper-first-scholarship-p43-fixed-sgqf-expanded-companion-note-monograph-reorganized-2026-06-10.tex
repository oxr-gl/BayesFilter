...
\begin{enumerate}[leftmargin=0.28in]
  ...
  coherent, bounded, and scientifically well motivated.
\end{enumerate}
...
When this note later says a moderate sparse-grid level can be plausible, that
claim is therefore about the standardized-coordinate moment integrands that the
fixed cloud actually evaluates.  It is not a general claim that low interaction
written in physical coordinates automatically survives every dense covariance
factor.
...
sparse.  
The mapping \(x=m+C\xi\) means that a sparse-grid plausibility argument must be
stated in the standardized coordinates actually seen by the quadrature rule.  A
block-local or low-order-interaction structure written directly in physical
coordinates can be mixed across many \(\xi\)-coordinates when the factor \(C\) is
dense.  So the useful teaching assumption here is narrower: after
standardization, the integrands entering the required Gaussian moments should
still have low effective interaction order in \(\xi\)-space, or at least be well
approximated by low-order interaction structure there.
...
\subsubsection{Toy case A: one nonlinear Gaussian moment}
...
\subsubsection{Toy case B: a two-dimensional Gaussian placement picture}
...
\subsubsection{One-Dimensional Gaussian Quadrature And Tensor Products}
\label{sec:p31-ghq}
...
\subsubsection{What problem is equation \texorpdfstring{\eqref{eq:p31-univariate-rule}}{9.3} solving?}
...
\subsubsection{What changes in two dimensions?}
...
\subsubsection{A concrete three-dimensional preview of why sparse grids help}
