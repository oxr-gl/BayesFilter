...
\title{A Chapter-Discipline Rewrite of the Tensor-Train Sequential Filtering Companion, Fixed-Branch Gradients, Large-Scale Validation, And An Expanded Algorithm 5(c.2) Derivation}
\author{Chak Wong}
\date{2026-06-12}
...
This note studies a high-dimensional nonlinear filtering problem through the lens
of one specific computational object: a tensor-train approximation to the
sequential filtering densities used by Zhao and Cui \cite{zhao2024ttsequential},
together with a fixed-branch likelihood specialization whose gradient
refers to the same declared approximate scalar.  The note is therefore not a
general introduction to tensor trains, not a general introduction to filtering,
and not a generic derivative note.  Its purpose is narrower and more concrete:
to explain how the Zhao--Cui construction works, how the fixed-branch likelihood is
defined, why the derivative is mathematically meaningful only after the branch is
frozen, and how that whole lane should be validated and interpreted.
...
non-Gaussian density-approximation lane based on tensor trains, squared-density
repair, conditional KR maps, and preconditioning.  The first task of this note is
to make that construction readable in source order.  The second task is to define
a fixed-branch specialization that freezes the discrete approximation choices so
that one obtains a named approximate scalar and an analytical derivative of that
same scalar, not of the adaptive algorithm in general.

The note therefore has one guiding question: what exactly is the mathematical
object being carried, normalized, differentiated, and validated at each step of
the Zhao--Cui style construction?  Everything that follows serves that question.
The reader does not need to keep every object in mind immediately.  It is enough
at the outset to distinguish the exact Bayesian target, the adaptive tensor-train
approximation family that numerically replaces it, and the fixed-branch scalar
and derivative that are studied later.  The chapter sequence is arranged so that
those three layers become visible one at a time.

\paragraph{What this note does not claim.}
The note does not claim that the adaptive algorithm is globally differentiable,
that fixed-branch differentiation recovers the exact Bayesian likelihood
derivative, or that moderate TT rank is always sufficient in high dimension.  It
only claims that, after the approximation family is fixed and the branch is
frozen, one can define a coherent approximate scalar and differentiate that same
scalar analytically.

\tableofcontents
\section{Running Example And Notation}

This chapter introduces the running nonlinear example and the notation that will
carry the rest of the note.  The purpose is not to catalog every symbol at once,
but to fix one live filtering object that makes the later TT, transport, and
fixed-branch constructions easier to follow.  The underlying method follows Zhao
and Cui \cite{zhao2024ttsequential}; tensor trains themselves follow Oseledets
\cite{oseledets2011tt}, and the transport maps used for conditional sampling are
Knothe--Rosenblatt maps in the sense of Rosenblatt's triangular transformation
\cite{rosenblatt1952remarks}.
...
\subsection{Threaded Nonlinear Example Used Throughout The Note}
...
\subsection{State-Space Model And Notation Used In This Note}
...
\subsection{The Four Marginal Learning Problems}
...
\subsection{The Exact Recursive Bottleneck}
...
This chapter introduces the approximation family itself.  Its guiding question is
simple: what class of structured functions is rich enough to replace a full
high-dimensional grid while still remaining computationally usable?  The answer in
this note is the tensor train.  Everything else in the chapter --- the split-rank
interpretation, the plausibility argument, and the worked exponential example ---
should be read as support for understanding that one approximation family.

Start from the object a numerical reader can picture: a table of function values.
If \(D=2\), a function \(h(r_1,r_2)\) sampled on a grid is a matrix.  Low-rank
matrix approximation writes
\[
...
\section{Coordinate Systems, Conditional Maps, And Preconditioning}
...
\begin{equation}
    ...
    \label{eq:p25-walk-5}
\end{equation}
	and then defines the source-scale SIRT density and its unshifted
	evidence-scale version
\begin{equation}
    q_{t,{\rm src}}(z)
    =
    \phi_t(z)^2+\tau_t\lambda_t(z),
    \qquad
    \widehat q_t(z)
    =
    e^{-c_t}q_{t,{\rm src}}(z).
    \label{eq:p25-walk-6}
\end{equation}
Here \(\tau_t\) is the source-scale defensive mass, so the whole
source-scale density, including the defensive part, is multiplied by
\(e^{-c_t}\) when returning to the unshifted evidence scale.
...
\subsection{Conditional Densities And KR Maps From Marginal Ratios}
...
\subsection{Forward Conditional Map And Particle-Filter Correction}
...
\subsection{Backward Conditional Map, Path Estimation, And Smoothing}
...
\section{Squared-TT Filtering Object And Operational Recursion}
...
\subsection{Why TT Marginalization Is Cheap Once The Representation Exists}
...
\subsection{Algorithm 1, Fully Annotated}
...
\subsection{Why Nonnegativity Fails And Why Square-Root TT Repairs It}
...
\subsection{Squared-TT Density, Defensive Reference, And Normalizer}
...
\subsection{Squared-TT Marginalization And Mass Matrices}
...
\subsection{Algorithm 2, Fully Annotated}
...
\label{sec:p50-fixed-branch-likelihood}

\paragraph{Mathematical question.}
Zhao and Cui's sequential tensor-train filter is an adaptive approximation
method.  At each time it constructs a local coordinate system, chooses
approximation points, fits a tensor train to a square-root density, controls the
rank, and then uses the squared approximation plus a defensive density to form a
nonnegative filtering density \cite[Algorithm~2 and Eq.~(13)]{zhao2024ttsequential}.
This adaptivity is mathematically natural: the region of posterior mass and the
rank needed to approximate it are not known before the data and the previous
filter have been propagated.

For a differentiable likelihood, however, one must distinguish the adaptive
construction of an approximation from the evaluation of a fixed approximate
scalar.  A gradient is meaningful only after the discrete choices that determine
the approximation have been declared.  The object below is therefore a
\emph{fixed branch}: it keeps Zhao and Cui's square-root density, defensive
term, affine change of variables, and normalizer convention, but treats the
design points, local coordinates, ranks, and stopping rule as part of the
definition of the approximate scalar.

\begin{definition}[Fixed branch]
\label{def:p50-fixed-branch}
For one time step \(t\), a fixed branch is the tuple
\begin{equation}
\begin{aligned}
B_t=\{&
\mathcal D_t,T_t,\Omega_t,c_t,
\mathcal V_t,\mathcal R_t,\mathcal A_t,
\lambda_t,\tau_t,\eta_t\}.
\end{aligned}
\label{eq:p50-fixed-branch-tuple}
\end{equation}
Here \(\mathcal D_t=\sum_{j=1}^{m_t}w_j\delta_{z_j}\) is a finite positive
design measure in local coordinates; \(T_t(z)=\mu_t+L_tz\) is an affine map from
the local domain \(\Omega_t\) to the physical filtering variables; \(c_t\in\R\)
is an additive log-scale shift; \(\mathcal V_t\) is the tensor-train
approximation space, with rank tuple \(\mathcal R_t\); \(\mathcal A_t\) is the
deterministic fitting rule used to choose the tensor-train coefficients inside
\(\mathcal V_t\); \(\lambda_t\) is a probability density on \(\Omega_t\);
\(\tau_t>0\) is the defensive mass on the shifted scale; and \(\eta_t\) denotes
the fixed admissibility tolerances used to reject ill-conditioned or
defensive-only approximations.

The branch is not an external annotation.  It is part of the definition of the
approximate likelihood.  Changing \(\mathcal D_t\), \(T_t\), \(c_t\),
\(\mathcal V_t\), \(\mathcal R_t\), or \(\mathcal A_t\) changes the scalar being
evaluated.
\end{definition}

\begin{proposition}[Fixed square-root step and normalizer]
\label{prop:p50-fixed-square-root-normalized}
Fix \(B_t\) in Definition \ref{def:p50-fixed-branch}.  Suppose:
\begin{enumerate}[label=(\roman*)]
\item the design measure \(\mathcal D_t\) is finite and has positive total
mass;
\item \(L_t\) is square and nonsingular, so \(\log|\det L_t|\) is finite;
\item if \(q_t^{\rm phys}\) is the unnormalized physical target at time \(t\)
and \(\ell_t(r)=-\log q_t^{\rm phys}(r)\), then the local negative log-density
\[
    U_t(z)=\ell_t(\mu_t+L_tz)-\log|\det L_t|
\]
is finite at the points used by \(\mathcal D_t\);
\item the fitting rule \(\mathcal A_t\), applied in the fixed space
\(\mathcal V_t\) with ranks \(\mathcal R_t\), returns a finite tensor train
\(\phi_t\) approximating \(s_t^{\rm sh}\), where
\[
\begin{gathered}
    \pi_t^{\rm sh}(z)=\exp\{-(U_t(z)-c_t)\};
    \qquad
    s_t^{\rm sh}(z)=\sqrt{\pi_t^{\rm sh}(z)}
    =\exp\{-(U_t(z)-c_t)/2\};
\end{gathered}
\]
\item \(\phi_t\in L^2(\Omega_t)\), equivalently
\(\int_{\Omega_t}\phi_t(z)^2\dd z<\infty\);
\item \(\tau_t>0\), \(\lambda_t\ge0\), and
\(\int_{\Omega_t}\lambda_t(z)\dd z=1\).
\end{enumerate}
Write \(z=(u,v)\), where \(u\) denotes the retained current-state coordinates
and \(v\) denotes the previous-state coordinates to be marginalized.
Define
\begin{equation}
    q_t^{B,{\rm sh}}(z)
    =
    ...
    \zeta_t^B
    =
    \int_{\Omega_t}q_t^{B,{\rm sh}}(z)\dd z,
    \label{eq:p50-fixed-shifted-density}
\end{equation}
...
\begin{equation}
    ...
    =
    e^{-c_t}\zeta_t^B.
    \label{eq:p50-fixed-original-density}
\end{equation}
Then \(q_t^{B,{\rm sh}}\ge0\), \(0<\zeta_t^B<\infty\), and
\(\overline Z_t^B>0\).  The retained density obtained by marginalizing either
\(q_t^{B,{\rm sh}}/\zeta_t^B\) or
\(\overline q_t^B/\overline Z_t^B\) over \(v\) is normalized as a density in
\(u\).
The log-evidence increment of this fixed branch is
\[
    \log\overline Z_t^B=\log\zeta_t^B-c_t,
\]
with the same shifted-density convention as Zhao and Cui's square-root
construction \cite[Algorithm~2 and Eq.~(13)]{zhao2024ttsequential}.
\end{proposition}

\begin{proof}
The affine change of variables gives the local unnormalized density
\(\exp\{-U_t(z)\}\).  The shift \(c_t\) replaces this density during fitting by
\(\exp\{-(U_t(z)-c_t)\}=e^{c_t}\exp\{-U_t(z)\}\), which changes numerical scale
but not the shape of the normalized target.  By assumption the deterministic
fitting rule returns a finite \(\phi_t\), hence \(\phi_t(z)^2\ge0\).  Since
\(\tau_t>0\), \(\lambda_t\ge0\), and \(\int_{\Omega_t}\lambda_t=1\),
\[
    \zeta_t^B
    =
    \int_{\Omega_t}\phi_t(z)^2\dd z+\tau_t
    \ge \tau_t>0.
\]
The \(L^2\) assumption gives \(\zeta_t^B<\infty\).  Thus
\(q_t^{B,{\rm sh}}/\zeta_t^B\) is a normalized density on the local target
block.  Multiplying numerator and denominator by the same positive factor
\(e^{-c_t}\) gives the original-scale density
\(\overline q_t^B/\overline Z_t^B\) without changing the normalized retained
filter.  Since \(q_t^{B,{\rm sh}}\ge0\) and \(0<\zeta_t^B<\infty\), Tonelli's
theorem implies that marginalizing \(q_t^{B,{\rm sh}}/\zeta_t^B\) over \(v\)
yields a density of total mass one in \(u\).  Finally,
\(\overline Z_t^B=e^{-c_t}\zeta_t^B\), so
\(\log\overline Z_t^B=\log\zeta_t^B-c_t\).
\end{proof}

\paragraph{Defensive-mass convention.}
The symbol \(\tau_t\) in Proposition
\ref{prop:p50-fixed-square-root-normalized} is measured on the shifted scale,
because it is added after squaring the fitted approximation of
\(s_t^{\rm sh}\).  Equivalently, \(\phi_t^2\) approximates the shifted density
\(\pi_t^{\rm sh}\).  On the original evidence scale, the defensive
contribution is \(e^{-c_t}\tau_t\lambda_t\).  Therefore a formulation that
wants to hold an original-scale defensive mass \(\overline\tau_t\) fixed must use
\(\tau_t=e^{c_t}\overline\tau_t\) in \eqref{eq:p50-fixed-shifted-density}.  If
one instead inserts the same numerical value of \(\overline\tau_t\) into the
shifted density, then the resulting likelihood is a different approximate
scalar.

\begin{proposition}[Relation to the adaptive Zhao--Cui construction]
\label{prop:p50-fixed-adaptive-relation}
For a fixed \(B_t\), the density in
\eqref{eq:p50-fixed-shifted-density} has the same square-root-plus-defensive
form as Zhao and Cui's Algorithm 2.  What is changed is not the density formula
but the status of the choices used to construct it: the empirical design
measure, affine localization, rank tuple, fitting rule, and stopping rule are
conditioned on rather than reselected during each likelihood evaluation.
Consequently the derivative of the fixed branch is a derivative conditional on
\(B_t\), not a derivative through the adaptive algorithm that selected \(B_t\).
\end{proposition}

\begin{proof}
Algorithm 2 of Zhao and Cui fits a square-root approximation and then squares it
with a defensive term to obtain a nonnegative density
\cite[Eq.~(13) and Algorithm~2]{zhao2024ttsequential}.  Proposition
\ref{prop:p50-fixed-square-root-normalized} uses the same mathematical form.
However, the map from the previous filtering approximation and the data to the
objects \(\mathcal D_t,T_t,c_t,\mathcal V_t,\mathcal R_t,\mathcal A_t\) is no
longer part of the differentiated computation.  These objects are treated as
parameters of the approximate scalar.  Differentiating the fixed branch
therefore differentiates the scalar produced after those choices have been
fixed.  It does not differentiate a random sampling rule, a rank-selection rule,
or an adaptive stopping rule.
\end{proof}
...
\toprule
Adaptive element in Zhao--Cui & Mathematical purpose & Fixed-branch treatment\\
\midrule
Empirical design points for the next target &
The next filtering target is not available as a closed-form low-dimensional
density.  Design points indicate where the propagated predictive and likelihood
mass lies. &
Condition on a finite positive design measure \(\mathcal D_t\).\\
\addlinespace
Adaptive increase of approximation information &
If the empirical representation is too concentrated, the fitted tensor train may
describe the wrong region of state space.  Adaptivity is a safeguard against
degenerate or uninformative approximation data. &
Use only a declared \(\mathcal D_t\).  If it fails coverage or conditioning
checks, reject the branch rather than silently changing the likelihood.\\
\addlinespace
Affine localization \(T_t(z)=\mu_t+L_tz\) &
The target may be translated, scaled, or anisotropic.  A local affine coordinate
system makes the square-root approximation numerically plausible in high
dimension. &
Treat \(T_t\) as part of \(B_t\).  Its Jacobian contribution is the
\(-\log|\det L_t|\) term in \(U_t\).\\
\addlinespace
Additive log-scale shift \(c_t\) &
The shift changes only the numerical scale of the fitted density.  The original
evidence scale is recovered by subtracting \(c_t\) from the logarithmic
normalizer. &
Fix \(c_t\) and use
\(\log\overline Z_t^B=\log\zeta_t^B-c_t\).\\
\addlinespace
Rank and approximation-space selection &
The rank controls the expressive power and cost of the tensor train.  The
adaptive algorithm changes rank or enrichment to find an adequate approximation
without knowing that rank in advance. &
Fix \(\mathcal V_t\), \(\mathcal R_t\), and \(\mathcal A_t\).  Rank comparisons
are valid only between admitted branches.\\
\addlinespace
Squared density with defensive component &
Squaring the square-root approximation enforces nonnegativity.  The defensive
density preserves support and avoids a zero proposal density in regions where
the fit is poor \cite[Eq.~(13)]{zhao2024ttsequential}. &
Use \(q_t^{B,{\rm sh}}=\phi_t^2+\tau_t\lambda_t\), and scale the whole density,
including the defensive term, by \(e^{-c_t}\) when returning to the original
evidence scale.\\
\addlinespace
Conditional maps and marginal contractions &
Zhao and Cui use conditional transport maps and marginal contractions to sample
and evaluate the fitted filtering object
\cite[Proposition~2 and Section~3.2]{zhao2024ttsequential}. &
Compute these contractions from the normalized density determined by \(B_t\).
They inherit the same fixed-branch conditioning as the normalizer.\\
\bottomrule
\end{tabular}
\caption{Mathematical changes made when an adaptive tensor-train filtering step
is converted into a fixed branch.}
\label{tab:p50-adaptive-vs-fixed-branch}
\end{table}

\begin{proposition}[Defensive-only fitted branch is not a rank comparator]
\label{prop:p50-defensive-only-rank-veto}
Let two fixed branches \(B^{\rm low}\) and \(B^{\rm high}\) differ only in a
declared rank or degree setting.  Let \(\epsilon_{\rm def}>0\) denote the
declared defensive-only tolerance, one component of \(\eta_t\), and let
\[
    S(B)=\int_{\Omega_t}\phi_B(z)^2\dd z,\qquad
    D(B)=\tau_t\int_{\Omega_t}\lambda_t(z)\dd z.
\]
If \(S(B^{\rm high})\le\epsilon_{\rm def}\) while
\(D(B^{\rm high})>0\), then \(B^{\rm high}\) is a defensive-only fitted
branch for the purposes of a rank ladder.  It cannot be used as evidence
that the higher rank is a stronger approximation, and the rank comparison must
be declared inadmissible.
\end{proposition}

\begin{proof}
The approximate density of the high branch is
\[
    q_{B^{\rm high}}^{\rm sh}(z)
    =
    \phi_{B^{\rm high}}(z)^2+\tau_t\lambda_t(z).
\]
If \(S(B^{\rm high})\le\epsilon_{\rm def}\), then the fitted square-root
component contributes no resolved mass at the declared tolerance.  The
normalizer is therefore dominated by \(D(B^{\rm high})\), the defensive
fallback inserted for support and stability.  A rank ladder is meant to compare
fitted approximation capacity.  A candidate whose mass is only the defensive
fallback is not a fitted higher-rank approximation of the target density, so the
comparison would rank a real fit against a support floor.  That is a category
error, not a convergence result.
\end{proof}

\paragraph{Consequence for fixed-rank experiments.}
A small fixed branch may be useful as a minimal algebraic diagnostic for the
normalizer and marginal contractions.  It is not, by itself, evidence that the
rank is adequate for a high-dimensional filtering problem.  Rank-capacity
claims require the admitted branch to have nonzero fitted square-root mass,
finite residuals, acceptable conditioning, adequate local-domain coverage, and
a stable retained normalizer.

\subsection*{Stabilizing A Frozen High-Rank Fit Without Changing The Target}

The preceding veto separates a mathematical failure of a fixed branch from a
failure of Zhao and Cui's adaptive method.  Zhao and Cui's paper constructs, at
each filtering time, a square-root tensor-train approximation and then forms the
nonnegative density in Eq.~(13) and Algorithm~2
\cite{zhao2024ttsequential}.  Their implementation does not merely solve one
predeclared least-squares problem.  In the audited source, the forward pass
forms local coordinates from propagated samples, evaluates the shifted target,
splits local samples into fitting and diagnostic sets, and then calls
\texttt{TTSIRT} to construct the square-root approximation
(\texttt{models/full\_sol.m:84--124}).  The local coordinate map is based on a
weighted empirical center and covariance with a small positive covariance
regularization; when the effective sample size is large enough, the same source
routine also applies a diagonal stretch based on weighted quantiles in the
standardized coordinates (\texttt{models/computeL.m:24--47}).  The
\texttt{TTSIRT} constructor delegates approximation to \texttt{TTFun} and then
may round the tensor train when the AMEN route is active
(\texttt{deep-tensor.dev/src/@TTSIRT/TTSIRT.m:238--248}).  The shared SIRT
constructor stores the defensive parameter and then marginalizes the
approximation (\texttt{deep-tensor.dev/src/SIRT.m:74--85}); the TTSIRT
marginalization routine computes the squared approximation mass and sets the
total normalizer to that mass plus the defensive term
(\texttt{deep-tensor.dev/src/@TTSIRT/marginalise.m:81--85}).  This is the source
counterpart of the squared-density normalizer in Zhao and Cui's Proposition~2
and Section~3.2 \cite{zhao2024ttsequential}.

The fixed branch changes the status of these choices.  It freezes the design
measure, coordinate map, rank tuple, basis, sweep count, and fitting rule so
that a differentiable approximate scalar is well-defined.  That freezing is
useful for a likelihood calculation, but it removes safeguards that the
adaptive method uses because the posterior region, approximation rank, and
useful sample set are not known in advance.  Therefore the fixed branch needs
its own admissibility and stabilization rules.  These rules are part of the
branch \(B_t\); they do not make the branch an unmodified implementation of the
adaptive algorithm.

\begin{proposition}[Zero-environment cascade in a frozen ALS update]
\label{prop:p50-zero-environment-cascade}
Consider one core update in a fixed tensor-train least-squares fit.  Let
\[
    y_j=s_t^{\rm sh}(z_j)>0,\qquad
    w_j>0,\qquad j=1,\ldots,m,
\]
be the square-root target values and fitting weights.  Holding all cores except
core \(k\) fixed gives the weighted ridge normal equation
\begin{equation}
    (A_k^\top W A_k+\rho I)g_k=A_k^\top W y,
    \qquad \rho>0,
    \label{eq:p50-zero-env-normal}
\end{equation}
where \(g_k\) is the vector of coefficients of the \(k\)-th core.  If either
the left environment or the right environment of core \(k\) is zero at every
design point, then \(A_k=0\).  In that case the unique solution of
\eqref{eq:p50-zero-env-normal} is \(g_k=0\).  If this zero core is accepted in
an interior position of the tensor train, then the represented square-root
function is identically zero, and every subsequent core update whose design
matrix uses the zero environment also has zero right-hand side.
\end{proposition}

\begin{proof}
The design row for core \(k\) is the tensor product of the left environment,
the one-dimensional basis vector at coordinate \(k\), and the right
environment.  If one of the two environments is zero for every design point,
then every design row is zero, hence \(A_k=0\).  Since \(\rho>0\), the normal
matrix is \(\rho I\), so \eqref{eq:p50-zero-env-normal} has the unique solution
\(g_k=0\).  A tensor train evaluates by multiplying its core matrices.  If an
interior core matrix is zero for every input, every product containing that
core is zero, so the whole square-root function is zero.  The same zero product
then appears as a left or right environment in later updates, giving the same
zero-design conclusion for those updates.
\end{proof}

Proposition~\ref{prop:p50-zero-environment-cascade} is not a theorem that every
high-rank fixed fit must collapse.  It identifies a concrete mechanism by which
a deterministic one-sweep, small-design fit can report finite ridge solves and
benign condition numbers while still producing a zero square-root tensor.  A
condition-number check alone is therefore not an admissibility check for a
fixed high-rank branch.

\begin{proposition}[Constant-path initialization for a fixed branch]
\label{prop:p50-constant-path-initialization}
Assume the one-dimensional bases are normalized so that their first basis
function is the constant \(1\), and the product reference measure has total
mass one.  Let \(y_j=s_t^{\rm sh}(z_j)>0\) and \(w_j>0\), and define the
weighted mean
\begin{equation}
    \bar y_t=
    \frac{\sum_{j=1}^{m}w_jy_j}{\sum_{j=1}^{m}w_j}.
    \label{eq:p50-constant-path-mean}
\end{equation}
For a fixed rank tuple \((1,R_1,\ldots,R_{D-1},1)\), initialize the tensor-train
cores by
\begin{equation}
    C_{1}[1,0,1]=\bar y_t,\qquad
    C_{k}[1,0,1]=1\quad (k=2,\ldots,D),
    \label{eq:p50-constant-path-init}
\end{equation}
with all other coefficient entries equal to zero.  The resulting initial
square-root tensor train represents the constant function
\(\phi_t^{(0)}(z)=\bar y_t\), and its squared mass is
\begin{equation}
    S(\phi_t^{(0)})=
    \int_{\Omega_t}\phi_t^{(0)}(z)^2\dd\lambda_t(z)
    =
    \bar y_t^2>0.
    \label{eq:p50-constant-path-mass}
\end{equation}
Consequently this initialization is a deterministic nonzero starting branch for
any declared rank tuple.  If later ALS updates are accepted only when the final
square-root mass is finite and exceeds the defensive-only tolerance
\(\epsilon_{\rm def}\), then an admitted high-rank branch cannot be
defensive-only in the sense of Proposition
\ref{prop:p50-defensive-only-rank-veto}.
\end{proposition}

\begin{proof}
Because \(y_j>0\) and \(w_j>0\), the weighted mean
\(\bar y_t\) in \eqref{eq:p50-constant-path-mean} is strictly positive.  In
the core product, only the first rank channel and the constant basis channel
are nonzero.  Each core after the first contributes the scalar \(1\), and the
first core contributes \(\bar y_t\).  Thus the represented function is the
constant \(\bar y_t\).  Since the reference measure has total mass one,
\[
    \int_{\Omega_t}\phi_t^{(0)}(z)^2\dd\lambda_t(z)
    =
    \int_{\Omega_t}\bar y_t^2\dd\lambda_t(z)
    =
    \bar y_t^2>0.
\]
If the completed fit is admitted only when its square-root mass is finite and
larger than \(\epsilon_{\rm def}\), then its fitted component contributes
resolved positive mass.  By Proposition
\ref{prop:p50-defensive-only-rank-veto}, such a branch is not defensive-only at
the declared tolerance.
\end{proof}

\paragraph{What changes relative to Zhao and Cui.}
The density formula does not change: the fixed branch still uses
\(\phi_t^2+\tau_t\lambda_t\), as in Zhao and Cui's square-root construction
\cite[Eq.~(13) and Algorithm~2]{zhao2024ttsequential}.  The marginal and
normalizer contractions also remain the squared-core contractions described in
their Proposition~2 and Section~3.2.  What changes is the finite-dimensional
fitting rule inside \(B_t\).  Instead of allowing the adaptive software route
to choose or enrich the approximation from its available samples, the fixed
branch declares a deterministic rank, a deterministic fitting sample set, a
deterministic sweep order, a deterministic initialization
\eqref{eq:p50-constant-path-init}, and an admissibility veto based on the
resolved square-root mass.  This is a fixed-branch adaptation for a
differentiable likelihood.  It must be recorded in the branch identity and
cannot be cited as an unqualified source-faithful Zhao--Cui operation.

\paragraph{Why the author route uses adaptivity.}
The adaptive choices in Zhao and Cui's algorithm are not incidental coding
preferences.  They are needed because the next filtering target is known only
through propagated particles, likelihood evaluations, and the previous
approximate filter.  The algorithm must discover a useful local coordinate
system, sample region, rank, and tensor-train representation from the data at
that time step.  A frozen HMC branch has the opposite requirement: every
discrete choice must be fixed before the likelihood and its derivative are
defined.  The constant-path initialization and the square-root-mass
admissibility check are therefore not replacements for the adaptive algorithm;
they are the extra mathematical bookkeeping required after the adaptive route
has been converted into one fixed approximate scalar.

\section{Fixed-Branch Likelihood Construction}

The earlier chapters define the approximation family and the sequential
filtering object.  We now isolate the mathematical question that matters for a
differentiable approximate likelihood: once the adaptive approximation has
selected its finite-dimensional structure, which choices are part of the scalar
being differentiated?  The fixed-branch construction identifies the
coordinate system, design measure, approximation space, ranks, fitted
coefficients, reference density, defensive mass, and log-scale convention as
components of one approximate likelihood.  Changing one of these components
generally defines a different scalar.

The adaptive Zhao--Cui algorithm contains rank choices, fitting choices,
preconditioning choices, and branch choices that should not be read as globally
differentiable.  The fixed-branch specialization freezes those discrete choices
first, then studies the derivative of the resulting approximate likelihood.
...
\mapsto
\text{fixed ranks, design points, and coordinate maps}.
\end{gathered}
\label{eq:p24-r3}
\end{equation}
...
Thus the fixed-branch construction does not differentiate the adaptive Zhao and
Cui \cite{zhao2024ttsequential} selection procedure itself.  It differentiates a
lower-dimensional scalar constructed by applying the same squared-TT filtering
algebra after the discrete approximation choices have been fixed.  If one wants
a fixed-branch derivative while also learning a static parameter as a random
coordinate, one can append that coordinate to the state; the external
derivative variable must still be distinguished from the integrated coordinate.

\subsection{Fixed-Branch Mathematical Objects}

The fixed branch contains more than the displayed density formula.  Its
finite-dimensional data are part of the scalar identity: changing the coordinate
domains, bases, rank tuple, tensor cores, reference density, defensive mass, or
log-scale convention changes the approximate likelihood.  At time \(t\), write
the squared-TT filter branch as
\[
    \mathcal B_t
    =
    \{
...
approximate likelihood, the realized branch must be fixed before
differentiation.
...
The next subtlety is what actually has to survive from one time step to the next.
If too little is saved, the next target cannot be reconstructed on the same branch.
If the wrong thing is saved, the implementation may appear consistent while no
longer evaluating the same scalar.  After time \(t\), the next step therefore needs to evaluate
\[
...
\subsection{Consolidated Fixed Least-Squares Formulation}
...
than Zhao and Cui \cite{zhao2024ttsequential}'s adaptive method.  Its purpose is
to remove ambiguity by specifying one approximate scalar completely.  More
adaptive or higher-performance variants should be treated as different branches
unless they preserve the same declared mathematical objects.
...
\begin{equation}
    ...
    \label{eq:p24-c7}
\end{equation}
Choose a source-scale defensive floor \(\epsilon_\tau>0\) and set
\begin{equation}
    \tau_t=2^D\epsilon_\tau.
    \label{eq:p24-c8}
\end{equation}
Then \(\tau_t\lambda(z)=\epsilon_\tau\) in the shifted SIRT density and
\(e^{-c_t}\tau_t\lambda(z)=e^{-c_t}\epsilon_\tau\) on the unshifted evidence
scale.  Choose the stabilizing shift at the base parameter \(\beta_0\):
\begin{equation}
...
&\text{Source-scale SIRT density:}\qquad
  q_{t,{\rm src}}(z;\beta)
  =
  \phi_t(z;\beta)^2+\tau_t\lambda_t(z),\\
&\text{Unshifted evidence-scale density:}\qquad
  \overline q_t(z;\beta)
  =
  e^{-c_t}q_{t,{\rm src}}(z;\beta),\\
&\text{Retained numerator:}\qquad
  a_t(z_t;\beta)
  =
  \int
  e^{-c_t}\left[\phi_t(z_t,z_{t-1};\beta)^2
        +\tau_t\lambda_t(z_t,z_{t-1})\right]\dd z_{t-1},\\
&\text{Source-scale normalizer:}\qquad
  \zeta_t(\beta)
  =
  \int q_{t,{\rm src}}(z;\beta)\dd z
  =
  \int\phi_t(z;\beta)^2\dd z+\tau_t,\\
&\text{Unshifted normalizer:}\qquad
  \overline Z_t(\beta)
  =
  \int \overline q_t(z;\beta)\dd z
  =
  e^{-c_t}\zeta_t(\beta).
\end{aligned}}
\label{eq:p24-c11}
\end{equation}
Every carried filter, proposition, derivative, and finite-difference check in
this fixed variant uses \eqref{eq:p24-c11}.  The convention is mathematical:
\(\zeta_t\) normalizes the shifted density, while the filtering scalar on the
original evidence scale uses
\(\log\overline Z_t=\log\zeta_t-c_t\).  If one instead chooses an unshifted
defensive mass, that choice defines a different fixed branch unless the
defensive mass is rescaled before forming the shifted density.
...
objects.  The dependency flow is
\begin{equation}
\begin{array}{rcl}
...
\subsection{Fixed-Branch TT Filtering Recursion}

The fixed-branch filtering variant chooses all discrete approximation objects
before differentiating.  It is narrower than the full adaptive method of
Zhao and Cui \cite{zhao2024ttsequential}, but it is the variant for which an
analytical derivative can be stated without changing the scalar under the
derivative.
...
place.  It is a statement of the fixed finite-dimensional recurrence, not a
prescription for a particular program.
...
Build
\(q_{t,{\rm src}}=\phi_t^2+\tau_t\lambda_t\) and
\(\overline q_t=e^{-c_t}q_{t,{\rm src}}\).
Contract mass matrices to compute
\(\zeta_t=\int\phi_t^2\dd z+\tau_t\) and
\(\overline Z_t=e^{-c_t}\zeta_t\).
Set
\(a_t(z_t)=\int \overline q_t(z_t,z_{t-1})\dd z_{t-1}\) and
\(\widehat p_t(z_t)=a_t(z_t)/\overline Z_t\).
Update \(\widehat\ell_t=\widehat\ell_{t-1}+\log\zeta_t-c_t\).
...
\[
    ...
    \zeta_t=R_t+\tau_t,\qquad
    \overline Z_t=e^{-c_t}\zeta_t,\qquad
    \dot{\overline Z}_t=e^{-c_t}\dot R_t.
\]
\\[1mm]
...
\[
    P_t=Q_t/\overline Z_t,\qquad
    \dot P_t=\dot Q_t/\overline Z_t-Q_t\dot{\overline Z}_t/\overline Z_t^2,
\]
and save the evaluator pair needed by time \(t+1\).
\\[1mm]
\textbf{Scalar update.}
\[
    \widehatell_t=\widehatell_{t-1}+\log\zeta_t-c_t,\qquad
...
\begin{equation}
    \overline q_t(z;\beta)
    =
    e^{-c_t}\left[\phi_t(z;\beta)^2+\tau_t\lambda_t(z)\right],
    \qquad
...
\begin{equation}
    ...
    \int\overline q_t(z;\beta)\dd z
    =
    e^{-c_t}\left[\int\phi_t(z;\beta)^2\dd z+\tau_t\right].
    \label{eq:p24-fb4}
\end{equation}
...
\begin{equation}
    ...
    =
    \frac{
    \int \overline q_t(z_t,z_{t-1};\beta)\dd z_{t-1}}
    {\overline Z_t(\beta)}
    \left|\det\nabla z_t/\nabla x_t\right|.
    \label{eq:p24-fb5}
\end{equation}
...
\(\overline q_t(\cdot;\beta)\) defined by \eqref{eq:p24-fb3} is nonnegative, integrable, and
has \(0<\overline Z_t(\beta)<\infty\).  Define \(\widehat p_t\) by \eqref{eq:p24-fb5},
...
approximate joint density \(\overline q_t\), not to the exact joint density.
\end{proposition}

\begin{proof}
Nonnegativity follows from
\(\phi_t^2\ge0\), \(\tau_t>0\), \(\lambda_t\ge0\), and \(e^{-c_t}>0\).  Since
\(\overline Z_t=\int\overline q_t\) is positive and finite, the normalized joint
density
\[
    \widehat p_t^{\rm joint}(z)
    =
    \overline q_t(z)/\overline Z_t
\]
...
the declared approximation \(\overline q_t\).  No step asserts that
\(\overline q_t=q_t\), so the result is a normalized approximate filtering
recursion rather than an exact-posterior theorem.
\end{proof}


\section{Fixed-Branch Derivative Theory}

The fixed-branch likelihood chapter defines the scalar.  This chapter follows the
dependent question: what exactly is differentiated when the branch is held fixed,
and why does the resulting derivative refer to the same scalar rather than to a
nearby adaptive variant?  The warmups that follow are not separate teaching
excursions.  Their role is to isolate the few derivative moves that are reused in
the full fixed-branch graph, so that the later proposition and implementation
logic feel like one ascent rather than several separate derivations.
...
\subsection{Fixed-Branch Gradient Motivation}
...
\subsection{Warmup 1: A Normalizing Constant}
...
\subsection{Warmup 2: A Squared Approximation}

The fixed-branch squared tensor-train approximation has the form
\begin{equation}
    q_{\rm src}(z;\beta)
    =
    \phi(z;\beta)^2+\tau\lambda(z),
    \qquad
    \widehat q(z;\beta)=e^{-c}q_{\rm src}(z;\beta).
    \label{eq:p24-p19-14}
\end{equation}
Here:
\begin{equation}
...
\tau>0 & \text{is a frozen source-scale defensive mass},\\
...
with \(\beta\) is \(\phi\).  Differentiating the unshifted evidence-scale
density gives
\begin{equation}
\begin{aligned}
    ...
    \frac{\partial}{\partial\beta}
    \left[
    e^{-c}\left(\phi(z;\beta)^2+\tau\lambda(z)\right)
    \right]\\
    &=
    e^{-c}
...
\begin{aligned}
    ...
    &=
    e^{-c}\left[
    \int \phi(z;\beta)^2\dd z
    +
    \tau\int\lambda(z)\dd z
    \right]\\
    &=
    e^{-c}\left[\int \phi(z;\beta)^2\dd z+\tau\right].
\end{aligned}
\label{eq:p24-p19-17}
\end{equation}
...
\begin{equation}
    ...
    =
    \frac{
    2\int \phi(z;\beta)\dot\phi(z;\beta)\dd z}
    {\int\phi(z;\beta)^2\dd z+\tau}.
    \label{eq:p24-p19-19}
\end{equation}
...
\subsection{Warmup 3: Two Coordinates, Rank One}
...
\subsection{Warmup 5: A Fixed Linear Solve}
...
\subsection{The Fixed-Branch Forward Pass}
...
and its normalizer
\begin{equation}
    \widehat Z_t(\beta)=\int \overline q_t(z;\beta)\dd z.
    \label{eq:p24-p19-45}
\end{equation}
...
Cores \(C_{t,k}(\beta)\) & Parameter-dependent fitted coefficient arrays & Evaluate \(\phi_t\) and \(\overline q_t\)\\
Mass contractions & \(M_{t,>k}(\beta)\), \(M_{t,<k}(\beta)\) & Compute integrals and marginals\\
Normalizer & \(\widehat Z_t=e^{-c_t}(R_t+\tau_t)\) & Adds \(\log\widehat Z_t=\log\zeta_t-c_t\) to the scalar\\
...
\subsection{Gradient Teaching Layer: What Is Differentiated}
...
\boxed{
    ...
    =
    \sum_{t=1}^{T}\{\log\zeta_t(\beta;B_t)-c_t\}
}
    \label{eq:p25-gradteach-2}
\end{equation}
Here \(\zeta_t=R_t+\tau_t\) is the source-scale SIRT normalizer and
\(\widehat Z_t=e^{-c_t}\zeta_t\) is the unshifted evidence increment.  The
branch \(B=(B_1,\ldots,B_T)\) is saved.  It contains frozen structural fields:
\begin{equation}
    B_{t,\rm frozen}
    =
...
\(N_kg_k=d_k\) &
\(N_k\dot g_k=\dot d_k-\dot N_kg_k\)\\
\(g_k\to C_k\to M_{>k}\to\zeta_t\to\widehat Z_t\) &
\(\dot g_k\to\dot C_k\to\dot M_{>k}\to\dot\zeta_t\to\dot{\widehat Z}_t\)\\
...
\subsection{The Fixed-Branch Derivative Pass}
...
\eqref{eq:p24-p19-57}--\eqref{eq:p24-p19-65}.  In the warmup, \(M_1[r,s]\) stored the inner products of the
...
By (17)--(18),
\begin{equation}
    \zeta_t(\beta)=R_t(\beta)+\tau_t,
    \qquad
    \widehat Z_t(\beta)=e^{-c_t}\zeta_t(\beta),
    \label{eq:p24-p19-71}
\end{equation}
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:p24-p22-k2}
\end{equation}
The defensive term in \eqref{eq:p24-p22-k1} gives
\(e^{-c_t}\tau_t/2\) on the unshifted evidence scale because the normalized
Legendre constant satisfies \(b_1(z)[0]=1/\sqrt2\), hence
\begin{equation}
    e^{-c_t}\tau_t b_1(z_1)[0]^2=e^{-c_t}\tau_t/2.
    \label{eq:p24-p22-k3}
\end{equation}
...
\end{equation}
The fitted retained matrix and the retained defensive contribution share the
same evidence-scale factor in \(Q_t\):
\begin{equation}
    Q_t[\ell,\ell']
    =
    e^{-c_t}\left[
...
\subsection{Proposition 1: The Fixed-Branch Recursion Is A Normalized Approximate Filter}
...
\subsection{The Story Of The Fixed-Branch Derivative}
...
\begin{equation}
    \zeta_t=\int\phi_t^2\dd z+\tau_t,\qquad
    \widehat Z_t=e^{-c_t}\zeta_t,
    \qquad
...
\subsection{Proposition 2: The Fixed-Branch Derivative Differentiates The Same Scalar}
...
\subsection{What Must Be Held Fixed For The Derivative}
...
\end{equation}
The equality is a mathematical identity of declared fields; the object being
compared is the field tuple in \eqref{eq:p24-p23-man2}.
...
\subsection{Four Checklists For Reading The Derivative}
...
\begin{array}{ll}
\text{established} &
    \overline q_t=e^{-c_t}(\phi_t^2+\tau_t\lambda_t)\ge0,\\[1mm]
\text{stored} &
    \phi_t \text{ through } C_{t,k}
    \text{ and } \dot\phi_t \text{ through } \dot C_{t,k},\\[1mm]
\text{integrated} &
    e^{-c_t}\left[\phi_t(z)^2+\tau_t\lambda_t(z)\right],\\[1mm]
\text{differentiated} &
    \overline q_t(z)
...
The note closes its mathematical construction by asking what evidence would make
the declared approximate scalar and its derivative credible.  This chapter is not
an appended protocol ledger.  It is the chapter in which the note tests whether
the object defined earlier is the same object that is evaluated, differentiated,
and interpreted in practice.  The finite-difference diagnostic is the first such
test because it checks same-scalar consistency most directly.

Choose a base parameter \(\beta_0\).  Build the branch
...
\subsection{Minimal Mathematical Example}
...
\begin{equation}
    \widehat Z_1(\beta)=e^{-c_1}\left[
    \int \phi_1(z_1,z_0;\beta)^2
    \dd z_1\dd z_0+\tau_1
    \right],
...
\begin{equation}
    ...
    \frac{\int
    e^{-c_1}\left[
    \phi_1(z_1,z_0;\beta)^2+\tau_1\lambda_1(z_1,z_0)
    \right]
    \dd z_0}
    {\widehat Z_1(\beta)}.
...
\subsection{Large-Scale Validation Models And Test Protocol}
...
\section{Positioning, Limits, And Relation To Zhao And Cui}

At this stage the main object of the note has been fully constructed: a TT-based
approximate filtering lane, a fixed-branch scalar built from that lane, and an
analytical derivative of that same scalar.  The final task is therefore not to
restart the derivation, but to state what that construction does and does not
entitle us to conclude, and how it relates back to Zhao and Cui's original
...
\subsection{Relation To Zhao And Cui}
...
\IfFileExists{docs/references.bib}
