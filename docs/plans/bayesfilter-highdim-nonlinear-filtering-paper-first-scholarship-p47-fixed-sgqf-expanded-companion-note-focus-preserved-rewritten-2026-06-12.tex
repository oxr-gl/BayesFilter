...
That task has several layers.  One must identify the exact filtering object being
approximated, state what the Gaussian surrogate keeps and discards, show how the
sparse-grid cloud is actually built, define the deterministic scalar computed on a
fixed branch, and then explain why the implementation details matter to the
identity of that scalar.  The purpose of the chapter is therefore not only to
state formulas, but to keep the reader oriented about which object is being
constructed at each stage and why each later convention matters.
