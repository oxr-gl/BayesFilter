...
\title{A Chemist-Readable And Implementation-Specified Companion to Zhao--Cui Tensor-Train Sequential Filtering and Fixed-Branch Gradients}
\author{BayesFilter P23 Technical Note}
\date{2026-06-02}
...
\begin{abstract}
This P23 note is a chemist-readable and implementation-specified companion
built by copying the P22 mathematical spine and adding eleven gap-closure
blocks.  It does not summarize, shorten, or replace P22.  Instead, it
preserves P22's Zhao--Cui annotation and fixed-branch derivations, then adds a
threaded nonlinear example, a worked two-dimensional KR example,
preconditioning flattening algebra, rank-count plausibility, deterministic
fitting and stabilization policies, multidimensional retained-filter storage,
and a branch-manifest schema.  The document is intentionally generous: it
favors explicit equations, dimensions, and implementation meaning over
compactness.
\end{abstract}
...
The construction rule for P22 was
\[
    \text{P22}
    =
...
The construction rule for P23 is
\[
    \text{P23}
    =
    \text{P22 mathematical spine}
    +
    \text{eleven chemist and implementation gap closures},
    \tag{P23-C1}
\]
not
\[
    \text{P23}
    =
    \text{a shorter cross-reference to P22}.
    \tag{P23-C2}
\]
No inherited P22 derivation is replaced by a shorter reference.  Additions in
P23 are additive or locally extending.

The scope boundary is also fixed:
\[
    \text{P23 is a mathematical document and specification, not executable code}.
    \tag{P23-C3}
\]
It introduces no Python, MATLAB, Octave, Julia, shell, production
BayesFilter, DPF, student-baseline, controlled-DPF, or public-API edit.
...
\section{Threaded Nonlinear Example Used Throughout P23}
...
\hline
\text{filtering target} & \text{P23-E5, P23-E9} &
    \text{Bayes numerator before fitting}\\
\text{squared-TT density} & \text{P23-E6--P23-E8} &
    \text{nonnegative fitted approximation}\\
\text{preconditioning} & \text{P23-E10} &
    \text{bridge removes dominant geometry}\\
\text{gradient} & \text{P23-E11--P23-E12} &
...
\subsection*{P23 Rank-Count Plausibility Calculation}
...
\subsection*{P23 Deterministic Fixed-Rank Sweep Protocol}
...
\subsection*{P23 Worked Two-Dimensional KR Example}
...
\subsection*{P23 Operational KR Construction Details}
...
\subsection*{P23 Before/After Flattening Calculation}
...
\[
    ...
    \tag{P23-PREC9}
\]
For the running example (P23-E1)--(P23-E10), the unpreconditioned target is
\[
    q_t(r)
    =
...
\[
    ...
    \tag{P23-PREC11}
\]
This is (P23-PREC5) with the concrete factors from the running example
substituted.  The bridge helps only if the remaining multiplier in
(P23-PREC11) is flatter and less coupled than the original target.
...
\subsection*{P23 Domain-Selection Policy}
...
\subsection*{P23 Stabilization Defaults}
...
\subsection*{P23 Multidimensional Retained-Filter Contract}
...
provide the same evaluator and derivative evaluator as (P23-MD7)--(P23-MD8).
...
\subsection*{P23 Equation Dependency Diagram For Proposition 2}
...
(P23-STAB7), and a branch with unstable solves may differentiate the same
...
is correct.  This is why rescaling in (P23-SWEEP7), stabilization diagnostics
in (P23-STAB7)--(P23-STAB9), and bridge diagnostics in (P23-PREC9) are
...
\[
    ...
    \tag{P23-GDAG7}
\]
Proposition 2 proves that the derivative path (P23-GDAG2),
(P23-GDAG4), (P23-GDAG6), and (P23-GDAG7) differentiates the same scalar
computed by (P23-GDAG1), (P23-GDAG3), (P23-GDAG5), and (P23-GDAG7).
For the running example, the first link in (P23-GDAG2) is
\[
\begin{aligned}
    \dot{\widetilde q}_{t,j}
    &=
...
\section{P23 Saved Branch Manifest Schema}
...
object being compared is the field tuple in (P23-MAN2).
