...
level 2 matched the recursively updated dense numerical reference in total log
...
- The chosen scalar quadratic row is unusually favorable to SGQF level 2; later
  phases should not assume this perfect agreement persists on more difficult
  fixtures.

## Empirical evidence
- Three-step scalar quadratic SGQF level-2 and recursive dense reference agree
  to floating-point tolerance on the tested row.
- The multistep gap G1 is therefore closed at the selected 1D fixture scope.
...
|---|---|---|---|---|---|
| pass P1 | satisfied | no same-target or exactness-label veto triggered | agreement may not transfer to higher dimension or harder nonlinear rows | use this row as the dense-reference anchor for sparse-level and score phases | no claim beyond the tested 1D fixture/horizon |

## Next step
- Continue to P3 and P5 using the new multistep scalar quadratic row as the
  dense-reference anchor where appropriate.
