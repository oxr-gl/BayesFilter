...
\title{A Chair-Readable And Implementation-Ready Companion to Tensor-Train Sequential Filtering, Fixed-Branch Gradients, Large-Scale Validation, And An Expanded Algorithm 5(c.2) Derivation}
\author{Chak Wong}
\date{2026-06-03}
...
algorithm itself.  The final part gives a mathematical validation protocol for
large-scale state-space applications: the core test models follow Zhao and
Cui's numerical section, and the additional tests record accuracy, memory,
runtime, stability, and fixed-branch derivative behavior without claiming that
these diagnostics prove exact posterior accuracy.
\end{abstract}
...
\section{State-Space Model And Notation Used In This Note}
...
evaluator.  Outputs are nonnegative densities or log densities.  Quantities to
retain are the model specification and the observed \(y_t\).  Diagnostics are
...
Conditioning on data gives Zhao and Cui \cite{zhao2024ttsequential}, Eq. (4) in the notation of this note:
\begin{equation}
...
pointwise evaluator for the three-block target.  The quantities to retain after
...
\subsection*{A Fully Worked Three-Variable Exponential Example}
\label{subsec:p30-worked-tt-exponential}

The abstract formula \eqref{eq:p24-tt-1} becomes much less mysterious when one
multiplies through a small example.  Consider the three-variable function
\begin{equation}
    h(x,y,z)
    =
    \exp\{\alpha xy+\beta yz\},
    \qquad (x,y,z)\in\R^3.
    \label{eq:p30-tt-example-1}
\end{equation}
This function is not separable as \(h_1(x)h_2(y)h_3(z)\).  The middle
coordinate \(y\) couples to both \(x\) and \(z\).  The interaction graph is a
chain,
\[
    x\ --\ y\ --\ z,
\]
which is exactly the kind of structure a tensor train can exploit.

Start by approximating each exponential edge by its degree-\(M\) Taylor
polynomial:
\begin{equation}
    E_{12}^{(M)}(x,y)
    =
    \sum_{a=0}^{M}\frac{\alpha^a}{a!}x^ay^a,
    \qquad
    E_{23}^{(M)}(y,z)
    =
    \sum_{b=0}^{M}\frac{\beta^b}{b!}y^bz^b.
    \label{eq:p30-tt-example-2}
\end{equation}
The truncated target is
\begin{equation}
    h_M(x,y,z)
    =
    E_{12}^{(M)}(x,y)E_{23}^{(M)}(y,z)
    =
    \sum_{a=0}^{M}\sum_{b=0}^{M}
    \frac{\alpha^a\beta^b}{a!\,b!}
    x^a y^{a+b} z^b.
    \label{eq:p30-tt-example-3}
\end{equation}
Equation \eqref{eq:p30-tt-example-3} already has the tensor-train structure.
Use ranks
\[
    R_0=R_3=1,\qquad R_1=R_2=M+1,
\]
with rank labels \(a,b\in\{0,\ldots,M\}\).  Define
\begin{align}
    H_1(x)[1,a]
    &=
    \frac{\alpha^a}{a!}x^a,
    \label{eq:p30-tt-example-4}\\
    H_2(y)[a,b]
    &=
    y^{a+b},
    \label{eq:p30-tt-example-5}\\
    H_3(z)[b,1]
    &=
    \frac{\beta^b}{b!}z^b.
    \label{eq:p30-tt-example-6}
\end{align}
Then
\begin{equation}
    H_1(x)H_2(y)H_3(z)
    ...
    =
    h_M(x,y,z).
    \label{eq:p30-tt-example-7}
\end{equation}
The rank index \(a\) remembers how much \(xy\)-interaction has entered from
the left edge.  The rank index \(b\) remembers how much \(yz\)-interaction will
leave through the right edge.  The middle core is where the two interaction
orders meet.

Take a concrete numerical case:
\[
    M=2,\qquad \alpha=\frac12,\qquad \beta=-\frac13,\qquad
    (x,y,z)=\left(2,\frac12,-1\right).
\]
The three cores are
\begin{equation}
    H_1(2)
...
\begin{equation}
    ...
    1/18
    \end{bmatrix}.
    \label{eq:p30-tt-example-10}
\end{equation}
First multiply the first two cores:
\begin{equation}
    H_1(2)H_2(1/2)
    =
    \begin{bmatrix}
    13/8 & 13/16 & 13/32
    \end{bmatrix}.
    \label{eq:p30-tt-example-11}
\end{equation}
Then multiply by the final core:
\begin{equation}
\begin{aligned}
    H_1(2)H_2(1/2)H_3(-1)
    &=
    ...
    \approx 1.9184.
\end{aligned}
    \label{eq:p30-tt-example-12}
\end{equation}
The true exponential value is
\begin{equation}
    \exp\left\{
      \frac12\cdot2\cdot\frac12
      -\frac13\cdot\frac12\cdot(-1)
    \right\}
    =
    e^{2/3}
    \approx 1.9477.
    \label{eq:p30-tt-example-13}
\end{equation}
The difference is not a tensor-train multiplication error.  It is the Taylor
truncation error in replacing \(h\) by \(h_M\).  Raising \(M\) increases the TT
ranks and reduces this particular approximation error.  This is the same
tradeoff that later appears in filtering: richer cores and higher ranks can
represent more interaction, but they cost more and can become harder to fit.

\subsection*{How The Least-Squares Core Regression Looks In The Same Example}
\label{subsec:p30-worked-tt-regression}

The fitting sections later in the note use rows of the form
\[
    \widehat h(r^{(j)})=A_{j,k}g_k,
\]
where \(g_k=\vecop(C_k)\) is one unknown core and the other cores are held
fixed.  The exponential example shows exactly what this row means.

Use the first-order truncation \(M=1\):
\begin{equation}
    h_1(x,y,z)
    =
    (1+\alpha xy)(1+\beta yz)
    =
    1+\alpha xy+\beta yz+\alpha\beta xy^2z.
    \label{eq:p30-tt-example-14}
\end{equation}
The TT ranks are \(R_1=R_2=2\).  Use local basis vectors
\begin{equation}
    b_1(x)=\begin{bmatrix}1&x\end{bmatrix},\qquad
    b_2(y)=\begin{bmatrix}1&y&y^2\end{bmatrix},\qquad
    b_3(z)=\begin{bmatrix}1&z\end{bmatrix}.
    \label{eq:p30-tt-example-15}
\end{equation}
One exact set of cores is
\begin{equation}
    H_1(x)=\begin{bmatrix}1&\alpha x\end{bmatrix},
    \qquad
    H_3(z)=\begin{bmatrix}1\\ \beta z\end{bmatrix},
    \label{eq:p30-tt-example-16}
\end{equation}
and
\begin{equation}
    H_2(y)
    ...
    y & y^2
    \end{bmatrix}.
    \label{eq:p30-tt-example-17}
\end{equation}
If the middle core is unknown, write it as
\[
    C_2[a,\ell,b],
    \qquad
    a,b\in\{0,1\},\quad \ell\in\{0,1,2\},
\]
so that
\begin{equation}
    H_2(y)[a,b]
    =
    \sum_{\ell=0}^{2} C_2[a,\ell,b]\,y^\ell.
    \label{eq:p30-tt-example-18}
\end{equation}
The exact middle core for \eqref{eq:p30-tt-example-17} has four nonzero
entries:
\begin{equation}
    C_2[0,0,0]=1,\quad
    C_2[0,1,1]=1,\quad
    C_2[1,1,0]=1,\quad
    C_2[1,2,1]=1.
    \label{eq:p30-tt-example-19}
\end{equation}

At a fitting point \(r^{(j)}=(x_j,y_j,z_j)\), the left and right environments
for the middle core are
\begin{equation}
    L_j=H_1(x_j)=\begin{bmatrix}1&\alpha x_j\end{bmatrix},
    \qquad
    R_j=H_3(z_j)=\begin{bmatrix}1\\ \beta z_j\end{bmatrix}.
    \label{eq:p30-tt-example-20}
\end{equation}
The least-squares coefficient multiplying \(C_2[a,\ell,b]\) is
\begin{equation}
    A_{j,2}[a,\ell,b]
    =
    L_j[a]\,y_j^\ell\,R_j[b].
    \label{eq:p30-tt-example-21}
\end{equation}
This is the concrete version of the environment formula used later in
\eqref{eq:p24-a1-2b} and \eqref{eq:p24-p23-sweep1f}.

Use the same numerical point as above with \(M=1\):
\[
    \alpha=\frac12,\qquad
    \beta=-\frac13,\qquad
    (x_j,y_j,z_j)=\left(2,\frac12,-1\right).
\]
Then
\begin{equation}
    L_j=\begin{bmatrix}1&1\end{bmatrix},
    \qquad
    b_2(y_j)=\begin{bmatrix}1&1/2&1/4\end{bmatrix},
    \qquad
    R_j=\begin{bmatrix}1\\1/3\end{bmatrix}.
    \label{eq:p30-tt-example-22}
\end{equation}
The row \(A_{j,2}\) is the list of all products
\[
    L_j[a]\,b_2(y_j)[\ell]\,R_j[b].
\]
When dotted with the exact middle-core vector from
\eqref{eq:p30-tt-example-19}, only four entries contribute:
\begin{equation}
\begin{aligned}
    A_{j,2}g_2
    &=
    ...
    +\frac12\cdot\frac13
    +1\cdot\frac12
    +1\cdot\frac14\cdot\frac13\\
    &=
    \frac74.
\end{aligned}
    \label{eq:p30-tt-example-23}
\end{equation}
Direct multiplication gives the same value:
\begin{equation}
    H_1(2)H_2(1/2)H_3(-1)
    =
    (1+\alpha xy)(1+\beta yz)
    =
    \left(1+\frac12\right)\left(1+\frac16\right)
    =
    \frac74.
    \label{eq:p30-tt-example-24}
\end{equation}
Now suppose the target values in the least-squares problem are not the
first-order truncation \(h_1\), but the true exponential \(h\).  With fitting
points \(r^{(j)}\) and weights \(w_j\), the middle-core update solves
\begin{equation}
    \min_{g_2}
    \sum_j w_j\bigl(A_{j,2}g_2-h(r^{(j)})\bigr)^2+\rho\|g_2\|_2^2.
    \label{eq:p30-tt-example-25}
\end{equation}
The regression is therefore not mysterious interpolation jargon.  It is the
ordinary problem of choosing the middle-core coefficients so that the TT
product best matches the target values at the fitting points, while the left
and right cores are held fixed.  Alternating regression repeats this operation
for core \(1\), then core \(2\), then core \(3\), and then sweeps back.  In the
full filtering algorithm, the target values \(h(r^{(j)})\) are replaced by
unnormalized filtering-density values or square-root density values evaluated
at the chosen fitting points.
...
Here is the same rank idea in a physical high-dimensional example.  Imagine
measurements from a one-dimensional sensor array, a polymer chain, or a
reaction coordinate discretized into neighboring sites.  Let \(x_j\) be the
unknown local state at site \(j\).  A common local energy model has the form
\begin{equation}
    \log q(x_1,\ldots,x_m)
    =
    \sum_{j=1}^m a_j(x_j)
    +
    \sum_{j=1}^{m-1} b_j(x_j,x_{j+1}),
    \label{eq:p26-rank-physical-1}
\end{equation}
where \(a_j\) is the single-site evidence and \(b_j\) is the nearest-neighbor
interaction.  Across the split after coordinate \(k\), all terms in
\eqref{eq:p26-rank-physical-1} are entirely left or entirely right except the
one bridge term \(b_k(x_k,x_{k+1})\):
\begin{equation}
\begin{aligned}
    q(x_1,\ldots,x_m)
    &=
    \exp\!\left[
    \sum_{j\le k}a_j(x_j)+\sum_{j<k}b_j(x_j,x_{j+1})
    \right]\\
    &\quad\times
    \exp\!\left[b_k(x_k,x_{k+1})\right]\\
    \exp\!\left[
    \sum_{j>k}a_j(x_j)+\sum_{j>k}b_j(x_j,x_{j+1})
    \right].
\end{aligned}
    \label{eq:p26-rank-physical-2}
\end{equation}
Only the middle factor crosses the split.  If that bridge factor has a
separated expansion
\begin{equation}
    \exp[b_k(x_k,x_{k+1})]
    \approx
    \sum_{a=1}^{R_{\rm edge}} u_{k,a}(x_k)v_{k,a}(x_{k+1}),
    \label{eq:p26-rank-physical-3}
\end{equation}
then the whole density has a split-separated representation with rank at most
\(R_{\rm edge}\) at that cut:
\begin{equation}
    q(x_{\le k},x_{>k})
    \approx
    \sum_{a=1}^{R_{\rm edge}}
    U_{k,a}(x_{\le k})V_{k,a}(x_{>k}).
    \label{eq:p26-rank-physical-4}
\end{equation}
For a two-dimensional grid ordered by rows, a vertical cut crosses several
nearest-neighbor edges rather than one.  If \(w_k\) edges cross the split and
each edge needs at most \(R_{\rm edge}\) channels, the crude product bound is
\begin{equation}
    R_k \lesssim R_{\rm edge}^{\,w_k}.
    \label{eq:p26-rank-physical-5}
\end{equation}
This bound is not a sharp theorem, but it makes the hypothesis vivid:
moderate TT rank is plausible when the coordinate order keeps the number and
complexity of cross-split physical interactions moderate.  It is much less
plausible when one observation creates a sharp constraint involving nearly all
coordinates at once.
...
definition, and core tensor \(C_k\).  The operation is basis evaluation followed
by a contraction over the basis index:
\begin{equation}
    H_k(r_k)=\sum_{\ell=0}^{p_k-1}\psi_{k,\ell}(r_k)\,C_k[:,\ell,:].
    \label{eq:p24-tt-3a}
\end{equation}
The quantities to retain is the list of cores, basis objects, and coordinate
...
\subsection{Basis Families, Selection, And Diagnostic Equations}
\label{subsec:p30-basis-defense}

\paragraph{Mathematical question.}
The basis is not a cosmetic choice.  It determines which one-coordinate
features can be represented cheaply inside each TT core.  The question is
therefore:
\begin{equation}
    \begin{aligned}
    &\text{which finite-dimensional univariate spaces should be used}\\
    &\text{before the TT ranks carry cross-coordinate dependence?}
    \end{aligned}
    \label{eq:p30-basis-q}
\end{equation}
For coordinate \(k\), a basis choice declares a space
\begin{equation}
    V_{k,p_k}
    =
    \operatorname{span}\{\psi_{k,0},\ldots,\psi_{k,p_k-1}\},
    \qquad
    \psi_{k,\ell}:\mathcal I_k\to\R,
    \label{eq:p30-basis-space}
\end{equation}
and the TT approximation lives in the nonlinear product family
\begin{equation}
    \mathcal T(p_{1:D},R_{0:D})
    =
    \left\{
    z\mapsto H_1(z_1)\cdots H_D(z_D):
    H_k^{a,b}\in V_{k,p_k},\
    R_0=R_D=1
    \right\}.
    \label{eq:p30-basis-tt-family}
\end{equation}
The basis controls within-coordinate approximation.  The ranks control
between-coordinate interaction.  A poor basis can force high degree \(p_k\);
an unfortunate coordinate order or preconditioner can force high rank \(R_k\).
These are different failure modes and should be diagnosed separately.

This subsection gives the intuition and the minimal equations.  The formal
selection rule, fitted scalar, mass contractions, quadrature checks, and
gradient boundary are given in
\S\ref{subsec:p33-complete-basis-audit}.  The later subsection is the
authoritative specification; the present paragraphs are the readable preview.
The concrete Legendre example above is written in physical interval
coordinates \(r_k\), while the formal specification below is written in basis
coordinates \(z_k\).

To make the dependence explicit, write the complete approximation design as
\begin{equation}
    \mathfrak D
    =
    \bigl(
    \Psi,\nu,\omega_{1:D},T,\mathcal B_{1:D},p_{1:D},
    R_{0:D},
    Z_{\rm fit},Z_{\rm hold},W_{\rm fit},W_{\rm hold},\rho,S_{\max}
    \bigr).
    \label{eq:p30-basis-hyper}
\end{equation}
Here \(\Psi\) maps basis coordinates to physical coordinates,
\(\nu(\dd z)=\prod_{k=1}^D\omega_k(z_k)\dd z_k\) is the declared reference
measure, \(T\) is any preconditioning map, \(\mathcal B_k\) is the family from
which the basis \(\{\psi_{k,\ell}\}\) is drawn, \(p_k\) is its local size,
\(R_k\) is the TT rank at the \(k\)-th split, \(Z_{\rm fit}\) and
\(Z_{\rm hold}\) are fitting and holdout points, \(W_{\rm fit}\) and
\(W_{\rm hold}\) are weighted empirical inner products, \(\rho\) is the ridge
regularization, and \(S_{\max}\) is the sweep budget.  The same object is
expanded again in \eqref{eq:p33-design-tuple}; the later display is not a
second framework, but the fully specified version of \eqref{eq:p30-basis-hyper}.
Thus basis choice is a numerical
hyperparameter in the precise sense that it is selected before the fitted
coefficients are computed.  It is not a posterior parameter, and it is not a
quantity learned by Bayes' rule unless an additional outer model-selection
layer is explicitly introduced.

\paragraph{No universal optimal basis is being claimed.}
For a target \(h\) and a measure \(\mu\), the ideal fixed-rank, fixed-degree
choice would solve
\begin{equation}
    \inf_{\mathfrak D}
    \inf_{\widehat h\in\mathcal T_{\mathfrak D}}
    \|h-\widehat h\|_{L^2(\mu)}.
    \label{eq:p30-basis-ideal}
\end{equation}
Equation \eqref{eq:p30-basis-ideal} is useful as a definition of the desire,
but it is not the algorithm.  It is usually too expensive because it searches
over coordinate maps, basis families, degrees, ranks, and fitting designs at
the same time.  The defensible claim is narrower:
\begin{equation}
    \begin{aligned}
    &\text{choose a basis family by known structure,}\\
    &\text{then test the fitted approximation by residual, stability,}\\
    &\text{marginal, and downstream checks.}
    \end{aligned}
    \label{eq:p30-basis-claim}
\end{equation}
This is the same philosophy used in spectral approximation more broadly:
smooth functions on simple domains are often well represented by global
polynomials, while localized nonsmooth features often need local bases or
adaptivity \cite{trefethen2019approximation}.  In stochastic computation,
Hermite and related polynomial-chaos bases are standard when the reference
measure is Gaussian or belongs to the Wiener--Askey family
\cite{ghanem1991stochastic,xiu2002wieneraskey}.  Wavelets are standard sparse
representations for localized multiscale features \cite{daubechies1992ten,mallat2009wavelet}.
Dictionary-learning methods show how bases can be trained from data in a
sparse-representation setting \cite{aharon2006ksvd}.  Low-rank and sparse
approximations for parametric high-dimensional problems are also commonly
understood as competing structure-exploiting approximation designs, not as one
universally dominant basis rule \cite{bachmayr2018parametric}.
These citations are background for candidate-family heuristics and
approximation-space language only.  They do not prove correctness, convergence,
or optimality for nonlinear filtering posteriors.

\paragraph{The basis family is chosen from the coordinate law and expected
regularity.}
Let \(z_k\) be the coordinate in which the basis is evaluated and let
\(\omega_k(z_k)\dd z_k\) be the one-dimensional reference measure used for
mass matrices.  A stable basis should make the Gram matrix
\begin{equation}
    M_k[\ell,m]
    =
    \int_{\mathcal I_k}
    \psi_{k,\ell}(z)\psi_{k,m}(z)\omega_k(z)\dd z
    \label{eq:p30-basis-gram}
\end{equation}
well conditioned and preferably close to the identity.  The common choices are
therefore tied to the support and measure:
\begin{equation}
\begin{array}{@{}lll@{}}
\begin{array}{l}\text{bounded interval,}\\ \text{smooth target}\end{array} &
    z_k\in[-1,1] &
    \text{Legendre or Chebyshev},\\[1mm]
\begin{array}{l}\text{Gaussian reference}\\ \text{coordinate}\end{array} &
    z_k\in\R,\ \omega_k=\varphi &
    \text{Hermite / polynomial chaos},\\[1mm]
\text{periodic coordinate} &
    z_k\in[0,2\pi) &
    \text{Fourier modes},\\[1mm]
\begin{array}{l}\text{localized kink, threshold,}\\ \text{or sharp feature}\end{array} &
    z_k\in\mathcal I_k &
    \text{piecewise, spline, or wavelet},\\[1mm]
\begin{array}{l}\text{repeated solved examples}\\ \text{from one model class}\end{array} &
    z_k\in\mathcal I_k &
    \text{pilot dictionary or reduced basis}.
\end{array}
    \label{eq:p30-basis-table}
\end{equation}
This table is not a ranking.  It states matching conditions.  If the domain is
bounded and the transformed density is smooth, global polynomial bases are
reasonable.  If the coordinate is a Gaussianized preconditioned coordinate,
Hermite functions or polynomial chaos may fit the reference measure more
naturally.  If the posterior has local discontinuity-like behavior from a
floor, truncation, or threshold observation, local bases can be more efficient
than increasing a global polynomial degree.  If many training problems from
the same model class are available, a learned dictionary can be explored, but
then it must be frozen before fixed-branch differentiation.

\paragraph{Basis degree is tuned by a deterministic ladder.}
A complete method should not say merely ``choose \(p\).''  It should specify
a small ladder.  For each family \(\mathcal B\), define
\begin{equation}
    \mathcal P_{\mathcal B}
    =
    \{p^{(1)}<p^{(2)}<\cdots<p^{(L)}\}.
    \label{eq:p30-basis-degree-ladder}
\end{equation}
For example, a bounded smooth coordinate might use
\begin{equation}
    \mathcal P_{\rm Legendre}=\{4,6,8,10,12\},
    \label{eq:p30-basis-legendre-ladder}
\end{equation}
whereas a piecewise basis might refine by subinterval count \(J\) and local
degree \(s\):
\begin{equation}
    \mathcal P_{\rm piecewise}
    =
    \{(J,s):(J,s)\in\{(2,2),(4,2),(4,3),(8,3)\}\}.
    \label{eq:p30-basis-piecewise-ladder}
\end{equation}
For a fixed candidate \((\mathcal B,p,R)\), fit the TT and compute the training
and holdout residuals
\begin{equation}
    \varepsilon_{\rm train}
    =
    \frac{\|A_{\rm train}g-y_{\rm train}\|_{W_{\rm train}}}
         {\max\{\|y_{\rm train}\|_{W_{\rm train}},\epsilon_{\rm abs}\}},
    \qquad
    \varepsilon_{\rm hold}
    =
    \frac{\|A_{\rm hold}g-y_{\rm hold}\|_{W_{\rm hold}}}
         {\max\{\|y_{\rm hold}\|_{W_{\rm hold}},\epsilon_{\rm abs}\}}.
    \label{eq:p30-basis-residuals}
\end{equation}
The holdout points are not used in the core update.  They are a check that the
basis and ranks are representing the target rather than only interpolating the
selected fitting points.  A candidate basis-degree-rank choice is rejected if
\begin{equation}
    \varepsilon_{\rm hold}>\epsilon_{\rm hold}
    \quad\text{or}\quad
    \frac{\varepsilon_{\rm hold}}
         {\max\{\varepsilon_{\rm train},\epsilon_{\rm abs}\}}
    >\gamma_{\rm overfit}.
    \label{eq:p30-basis-holdout-reject}
\end{equation}
The second inequality is an overfitting diagnostic: the target can be fit on
the sampled rows, but the chosen basis/rank combination is not stable away
from them.

\paragraph{Mass, evidence, and marginal checks test the basis in the
filtering task.}
For a density approximation, pointwise residuals are not enough.  The fitted
object is trusted only if its integrals are stable.  Let
\(\widehat Z(p,R)\) be the normalizer computed from the fitted squared TT.
Basis enrichment should not move the evidence significantly after the
approximation has stabilized:
\begin{equation}
    \Delta_Z(p,R)
    =
    \left|
    \log \widehat Z(p^+,R^+)-\log \widehat Z(p,R)
    \right|
    \le \epsilon_Z,
    \label{eq:p30-basis-evidence-stability}
\end{equation}
where \((p^+,R^+)\) is the next richer candidate in the ladder.  Similarly,
one-dimensional and block marginals computed from the TT should agree with
direct quadrature or Monte Carlo spot checks on low-dimensional projections:
\begin{equation}
    \Delta_{\rm marg}
    =
    \max_{B\in\mathcal C_{\rm check}}
    \left\|
    \widehat p_B^{\rm TT}
    -
    \widehat p_B^{\rm check}
    \right\|_{L^1(B)}
    \le \epsilon_{\rm marg}.
    \label{eq:p30-basis-marginal-check}
\end{equation}
The set \(\mathcal C_{\rm check}\) contains low-dimensional coordinate blocks
that are computationally cheap to verify: single coordinates, neighboring
pairs, retained state components, or parameters that drive the likelihood.
These checks answer the panel's practical question: if the basis were a bad
choice, how would we know before using the filter?

\paragraph{Conditioning checks distinguish a rich basis from an unstable
basis.}
Adding basis functions is not automatically good.  The normal matrix in a core
least-squares update is
\begin{equation}
    N_k=A_k^\top W A_k+\rho I.
    \label{eq:p30-basis-normal-matrix}
\end{equation}
Even when the approximation family is expressive, the fitted coefficients are
not trustworthy if
\begin{equation}
    \kappa_k=\operatorname{cond}(N_k)
    \label{eq:p30-basis-condition}
\end{equation}
is too large or if small perturbations of the fitting points produce large
coefficient changes:
\begin{equation}
    \Delta_C
    =
    \frac{\|C^{\rm perturbed}-C^{\rm base}\|_F}
         {\max\{\|C^{\rm base}\|_F,\epsilon_{\rm abs}\}}.
    \label{eq:p30-basis-coeff-stability}
\end{equation}
The basis ladder therefore stops not at the largest possible \(p\), but at the
first candidate satisfying approximation and stability criteria:
\begin{equation}
\begin{array}{ll}
\text{fit criterion} & \varepsilon_{\rm hold}\le\epsilon_{\rm hold},\\[1mm]
\text{evidence criterion} & \Delta_Z\le\epsilon_Z,\\[1mm]
\text{marginal criterion} & \Delta_{\rm marg}\le\epsilon_{\rm marg},\\[1mm]
\text{conditioning criterion} & \max_k\kappa_k\le\kappa_{\max},\\[1mm]
\text{rank criterion} & \max_kR_k\le R_{\max}\ \text{without saturation}.
\end{array}
    \label{eq:p30-basis-pass-table}
\end{equation}
If all basis candidates fail, the conclusion is not that the posterior has
been solved badly; the conclusion is that this declared TT approximation
family is inadequate for the target under the current map, ordering, and
budget.

\paragraph{Can the basis be learned?}
Yes, but it changes the mathematical contract.  Suppose a pilot collection of
targets \(h^{(1)},\ldots,h^{(M_{\rm pilot})}\) is available.  A learned
dictionary basis can be written as
\begin{equation}
    \psi_{k,\ell}^{\rm learn}(z)
    =
    \sum_{m=1}^{m_{\rm raw}}
    U_{k,m\ell}\,\varphi_{k,m}(z),
    \qquad \ell=0,\ldots,p_k-1,
    \label{eq:p30-basis-learned}
\end{equation}
where \(\{\varphi_{k,m}\}\) is a raw library, such as polynomials, splines, or
wavelets, and \(U_k\) is learned from the pilot problems.  The pilot objective
might be
\begin{equation}
    \min_{U,C^{(1:M_{\rm pilot})}}
    \sum_{i=1}^{M_{\rm pilot}}
    \left\|
    h^{(i)}-\widehat h^{(i)}_{U,C^{(i)}}
    \right\|_{W_i}^2
    +\lambda_U\mathcal R(U).
    \label{eq:p30-basis-learn-objective}
\end{equation}
This can be useful when the same state-space model class is solved repeatedly.
However, this is an optional, non-default extension unless a separate pilot
study has produced an accepted learned basis.  The learned basis must then be
treated as part of the approximation design \(\mathfrak D\).  For the
fixed-branch derivative developed later, the basis is frozen:
\begin{equation}
    \dot\psi_{k,\ell}(z)=0.
    \label{eq:p30-basis-frozen-derivative}
\end{equation}
If the basis itself is re-trained as \(\beta\) changes, then
\begin{equation}
    \frac{\partial}{\partial\beta}
    H_k(z_k;\beta)
    =
    \sum_{\ell}
    \dot C_k[:,\ell,:]\psi_{k,\ell}(z_k;\beta)
    +
    \sum_{\ell}
    C_k[:,\ell,:]\dot\psi_{k,\ell}(z_k;\beta),
    \label{eq:p30-basis-moving-derivative}
\end{equation}
and the derivative is no longer the fixed-branch derivative in this note.  It
is a different, larger problem: differentiating through the basis-training
algorithm and all of its branch choices.

\paragraph{Default mathematical convention.}
The reader should therefore read the Legendre choice in
\eqref{eq:p24-tt-7} as a transparent baseline, not as a dogma.  The convention
used in the formal specification is
\begin{equation}
\begin{aligned}
    &\text{start with a measure-matched standard basis;}\\
    &\text{run the deterministic degree/rank ladder;}\\
    &\text{use the first candidate satisfying the residual, evidence, marginal,
      conditioning, and derivative criteria;}\\
    &\text{hold that basis fixed before fixed-branch differentiation.}
\end{aligned}
    \label{eq:p30-basis-policy}
\end{equation}
This is the substantive answer to the basis objection.  The method does
depend on the basis, just as spectral, sparse-grid, and reduced-basis methods
depend on their approximation spaces.  The defense is not to pretend
otherwise; it is to make the basis a declared part of the numerical method
whose consequences are visible in residuals, integrals, marginals,
conditioning, and derivative checks.

\subsection{A Complete Basis-Choice Specification In Equations}
\label{subsec:p33-complete-basis-audit}

The preceding section gives the main idea.  This section gives the complete
mathematical specification.  The central object is not a single named basis,
such as Legendre or Hermite.  The central object is the
whole approximation design:

\subsubsection{The Approximation Design Tuple}
...
\begin{aligned}
    ...
    (&\Psi,\nu,\omega_{1:D},T,\mathcal B_{1:D},p_{1:D},R_{0:D},
      Z_{\rm fit},Z_{\rm hold},\\
     &W_{\rm fit},W_{\rm hold},\rho,S_{\max}).
\end{aligned}
    \label{eq:p33-design-tuple}
\end{equation}
Each symbol in \eqref{eq:p33-design-tuple} is part of the approximation:
\begin{equation}
\begin{array}{ll}
\Psi:\mathcal Z\to\mathcal R & \text{maps basis coordinates to physical coordinates},\\
\nu(\dd z)=\prod_{k=1}^D \omega_k(z_k)\dd z_k
    & \text{is the tensor-product reference measure},\\
T & \text{is an optional preconditioning map},\\
\mathcal B_k,p_k & \text{give the family and number of basis functions in coordinate }k,\\
R_{0:D} & \text{are TT ranks with }R_0=R_D=1,\\
Z_{\rm fit},Z_{\rm hold} & \text{are fitting and holdout points},\\
W_{\rm fit},W_{\rm hold} & \text{are weighted empirical inner products},\\
\rho,S_{\max} & \text{are ridge and sweep-budget choices}.
\end{array}
    \label{eq:p33-design-table}
\end{equation}
Thus the basis choice is inseparable from the coordinate map, reference
measure, rank budget, fitting points, and diagnostics.

Suppose the physical target density is \(q(r)\) with respect to Lebesgue
measure \(\dd r\).  If \(r=\Psi(z)\), then the pulled-back density with
respect to Lebesgue measure in \(z\) is
\begin{equation}
    \widetilde q_{\rm Leb}(z)
    =
    q(\Psi(z))\,\left|\det\nabla\Psi(z)\right|.
    \label{eq:p33-pullback-leb}
\end{equation}
If the fitting basis is orthogonal with respect to
\(\nu(\dd z)=\omega(z)\dd z\), where
\(\omega(z)=\prod_{k=1}^D\omega_k(z_k)\), then the corresponding density with
respect to \(\nu\) is
\begin{equation}
    \widetilde q_{\nu}(z)
    =
    \frac{\widetilde q_{\rm Leb}(z)}{\omega(z)}
    =
    \frac{q(\Psi(z))|\det\nabla\Psi(z)|}
         {\prod_{k=1}^D\omega_k(z_k)}.
    \label{eq:p33-pullback-nu}
\end{equation}
Equations \eqref{eq:p33-pullback-leb}--\eqref{eq:p33-pullback-nu} explain why
the named basis alone is not enough.  Legendre, Hermite, Fourier, or wavelet
families have their standard conditioning properties only after the coordinate
map and reference measure have been declared.

The scalar fitted by the squared-TT regression is the square root of this
declared reference-measure target.  At fitting point \(z^{(j)}\),
\begin{equation}
    y_j
    =
    e^{c/2}\sqrt{\widetilde q_\nu(z^{(j)})},
    \qquad
    j=1,\ldots,N_{\rm fit}.
    \label{eq:p33-fitted-scalar}
\end{equation}
The regression target is therefore the square root of the density expressed in
the same coordinates and measure used by the basis functions and mass
matrices.
Thus the least-squares rows below do not fit the physical density \(q(r)\)
directly.  They fit the pulled-back and reference-measure-corrected square-root
target in \eqref{eq:p33-fitted-scalar}.  If the implementation instead chooses
to fit \(\sqrt{\widetilde q_{\rm Leb}}\), then the mass matrices and
normalizers must also be written with Lebesgue measure in \(z\).  Mixing the
target from one measure with the mass matrices from another measure is a
mathematical error.

\subsubsection{Basis Functions Inside Tensor-Train Cores}

For coordinate \(k\), choose
\begin{equation}
    V_{k,p_k}
    =
    \operatorname{span}\{\psi_{k,0},\ldots,\psi_{k,p_k-1}\}
    \subset L^2(\omega_k).
    \label{eq:p33-univariate-space}
\end{equation}
The \(k\)-th TT core is a coefficient tensor
\begin{equation}
    C_k\in\mathbb R^{R_{k-1}\times p_k\times R_k},
    \qquad R_0=R_D=1,
    \label{eq:p33-core-shape}
\end{equation}
and it evaluates to the matrix
\begin{equation}
    H_k(z_k)[a,b]
    =
    \sum_{\ell=0}^{p_k-1}
    C_k[a,\ell,b]\psi_{k,\ell}(z_k).
    \label{eq:p33-core-eval}
\end{equation}
The displayed equations use one-based rank indices
\[
    a=1,\ldots,R_{k-1},\qquad b=1,\ldots,R_k,
\]
and zero-based basis indices
\[
    \ell=0,\ldots,p_k-1.
\]
The TT approximation is
\begin{equation}
    \widehat h(z)
    =
    H_1(z_1)H_2(z_2)\cdots H_D(z_D)\in\mathbb R.
    \label{eq:p33-tt-value}
\end{equation}
Written out, \eqref{eq:p33-tt-value} is
\begin{equation}
\begin{aligned}
    \widehat h(z)
    =
    ...
    \left[
    \sum_{\ell_k=0}^{p_k-1}
    C_k[\alpha_{k-1},\ell_k,\alpha_k]\psi_{k,\ell_k}(z_k)
    \right],
\end{aligned}
    \label{eq:p33-tt-expanded}
\end{equation}
with \(\alpha_0=\alpha_D=1\).

Now fix all cores except \(C_k\).  At fitting point
\(z^{(j)}=(z_1^{(j)},\ldots,z_D^{(j)})\), define
\begin{equation}
    L_{j,k}
    =
    H_1(z_1^{(j)})\cdots H_{k-1}(z_{k-1}^{(j)})
    \in\mathbb R^{1\times R_{k-1}},
    \label{eq:p33-left-env}
\end{equation}
and
\begin{equation}
    R_{j,k}
    ...
    \in\mathbb R^{R_k\times1}.
    \label{eq:p33-right-env}
\end{equation}
Then
\begin{equation}
\begin{aligned}
    \widehat h(z^{(j)})
    &=
    \sum_{a=1}^{R_{k-1}}\sum_{b=1}^{R_k}
    L_{j,k}[a]\,H_k(z_k^{(j)})[a,b]\,R_{j,k}[b]\\
    &=
    \sum_{a=1}^{R_{k-1}}
    \sum_{\ell=0}^{p_k-1}
    \sum_{b=1}^{R_k}
    L_{j,k}[a]\psi_{k,\ell}(z_k^{(j)})R_{j,k}[b]
    C_k[a,\ell,b].
\end{aligned}
    \label{eq:p33-row-derivation}
\end{equation}
Thus the row multiplying \(C_k[a,\ell,b]\) is
\begin{equation}
    A_{j,k}[a,\ell,b]
    =
    L_{j,k}[a]\psi_{k,\ell}(z_k^{(j)})R_{j,k}[b].
...
\begin{equation}
    ...
    \label{eq:p33-vectorization}
\end{equation}
Because \(a=1,\ldots,R_{k-1}\), \(\ell=0,\ldots,p_k-1\), and
\(b=1,\ldots,R_k\), the image of \(\iota_k\) is exactly
\begin{equation}
    \iota_k(a,\ell,b)\in\{1,\ldots,R_{k-1}p_kR_k\}.
    \label{eq:p33-vectorization-range}
\end{equation}
Then \(A_k\in\mathbb R^{N_{\rm fit}\times R_{k-1}p_kR_k}\), and
\begin{equation}
    \widehat h(z^{(j)})=A_k[j,:]\,g_k.
    \label{eq:p33-ls-linear}
\end{equation}

Given target values \(y_j\) and weights \(W_{\rm fit}\), the local core update
is the ridge problem
\begin{equation}
    \min_{g_k}
    \|A_kg_k-y\|_{W_{\rm fit}}^2+\rho\|g_k\|_2^2,
    \qquad
    \|v\|_{W_{\rm fit}}^2=v^\top W_{\rm fit}v.
    \label{eq:p33-ridge-objective}
\end{equation}
Differentiating the scalar objective gives
\begin{equation}
    2A_k^\top W_{\rm fit}(A_kg_k-y)+2\rho g_k=0,
    \label{eq:p33-ridge-gradient}
\end{equation}
and therefore
\begin{equation}
    (A_k^\top W_{\rm fit}A_k+\rho I)g_k
    =
    A_k^\top W_{\rm fit}y.
    \label{eq:p33-normal-equation}
\end{equation}
This proves the normal equations used in the core update.

\subsubsection{Mass Matrices, Normalizers, And Marginals}

For each coordinate, the basis mass matrix is
\begin{equation}
    M_k[\ell,m]
    =
    \int_{\mathcal I_k}
    \psi_{k,\ell}(z)\psi_{k,m}(z)\omega_k(z)\dd z.
    \label{eq:p33-mass-matrix}
\end{equation}
If the basis is orthonormal with respect to the declared
\(\omega_k(z)\dd z\), then \(M_k=I\).  If the coordinate map or reference
measure changes, the same named polynomial family need not remain
orthonormal; the mass matrix must be recomputed.

Let \(\phi(z)=H_1(z_1)\cdots H_D(z_D)\) be the square-root TT.  The squared
mass is
\begin{equation}
    Z_\phi
    =
    \int \phi(z)^2\nu(\dd z).
    \label{eq:p33-square-mass}
\end{equation}
Define \(G_0[1,1]=1\) and update
\begin{equation}
    G_k[b,b']
    =
    \sum_{a,a'=1}^{R_{k-1}}
    ...
    M_k[\ell,m]\,
    C_k[a',m,b'].
    \label{eq:p33-mass-recursion}
\end{equation}
Then
\begin{equation}
    Z_\phi=G_D[1,1].
    \label{eq:p33-mass-final}
\end{equation}
The proof is induction.  For \(k=1\), \eqref{eq:p33-mass-recursion}
integrates the product \(H_1(z_1)^\top H_1(z_1)\) using
\eqref{eq:p33-mass-matrix}.  If \(G_{k-1}\) is the integrated square-mass
interface for the first \(k-1\) coordinates, multiplying by two copies of
core \(k\) and integrating \(z_k\) gives exactly \eqref{eq:p33-mass-recursion}.
At \(k=D\), both boundary ranks are one, so the remaining interface is the
scalar integral.

For the squared-TT density with defensive floor,
\begin{equation}
    \widehat q(z)=e^{-c}\phi(z)^2+\tau\lambda(z),
    \label{eq:p33-density-with-floor}
\end{equation}
the normalizer is
\begin{equation}
    \widehat Z=e^{-c}G_D[1,1]+\tau\int\lambda(z)\nu(\dd z).
    \label{eq:p33-full-normalizer}
\end{equation}
When \(\lambda\) is normalized with respect to \(\nu\),
\begin{equation}
    \widehat Z=e^{-c}G_D[1,1]+\tau.
    \label{eq:p33-normalized-floor}
\end{equation}
Marginalization is the same contraction with some coordinates left open.  If
coordinate block \(S\) is retained and \(S^c\) is integrated out, then
\begin{equation}
    a_S(z_S)
    =
    e^{-c}
    \int \phi(z_S,z_{S^c})^2\nu_{S^c}(\dd z_{S^c})
    +\tau\int\lambda(z_S,z_{S^c})\nu_{S^c}(\dd z_{S^c}).
    \label{eq:p33-retained-marginal}
\end{equation}
The normalized retained density is
\begin{equation}
    \widehat p_S(z_S)=\frac{a_S(z_S)}{\widehat Z}.
    \label{eq:p33-retained-normalized}
\end{equation}
Thus a basis choice affects not only pointwise approximation; it affects the
computed normalizer and all retained marginals through \(M_k\).

If \(M_k\) is computed by quadrature, write
\begin{equation}
    \widehat M_k[\ell,m]
    =
    \sum_{r=1}^{Q_k}
    w_{k,r}^{\rm quad}
    \psi_{k,\ell}(z_{k,r}^{\rm quad})
    \psi_{k,m}(z_{k,r}^{\rm quad}).
    \label{eq:p33-quadrature-mass}
\end{equation}
For the normalized Legendre example in \eqref{eq:p24-tt-7}, the product
\(\psi_{k,\ell}(z)\psi_{k,m}(z)\) is a polynomial of degree at most
\begin{equation}
    \deg(\psi_{k,\ell}\psi_{k,m})\le 2p_k-2.
    \label{eq:p33-legendre-mass-degree}
\end{equation}
A Gauss--Legendre rule with \(Q_k\) points is exact for polynomials of degree
at most \(2Q_k-1\).  Therefore exact mass integration for this example is
guaranteed when
\begin{equation}
    2Q_k-1\ge 2p_k-2,
    \qquad\text{equivalently}\qquad
    Q_k\ge p_k.
    \label{eq:p33-legendre-quadrature-threshold}
\end{equation}
This example does not make Gauss--Legendre quadrature universal.  It shows the
kind of exactness statement that must accompany any declared basis/measure
pair.
The quadrature residual is
\begin{equation}
    \delta_{M,k}
    =
    \frac{\|\widehat M_k-M_k^{\rm ref}\|_F}
         {\max\{\|M_k^{\rm ref}\|_F,\epsilon_{\rm abs}\}},
    \label{eq:p33-quadrature-residual}
\end{equation}
where \(M_k^{\rm ref}\) is an analytic mass matrix when available or a richer
reference quadrature otherwise.  The normalizer aliasing diagnostic is
\begin{equation}
    \delta_Z^{\rm quad}
    =
    \left|
    \log\widehat Z_{\rm rich\ quad}
    -
    \log\widehat Z_{\rm working\ quad}
    \right|.
    \label{eq:p33-quadrature-z}
\end{equation}
For candidate \(q\), the aggregate quadrature diagnostic used later is
\begin{equation}
    \delta_{\rm quad}^{(q)}
    =
    \max\left\{
    \max_{k=1,\ldots,D}\delta_{M,k}^{(q)},
    \delta_Z^{{\rm quad},(q)}
    \right\}.
    \label{eq:p33-quadrature-aggregate}
\end{equation}
If \(\delta_{M,k}\) or \(\delta_Z^{\rm quad}\) is large, the basis cannot be
judged: the numerical integration layer is itself unresolved.

\subsubsection{What Optimal Basis Can And Cannot Mean}

For a fixed design \(\mathfrak D\), define the best approximation error
\begin{equation}
    E_{\mathfrak D}(h)
    =
    \inf_{\widehat h\in\mathcal T_{\mathfrak D}}
    \|h-\widehat h\|_{L^2(\nu)}.
    \label{eq:p33-design-error}
\end{equation}
This is the right mathematical meaning of ``best within the declared family.''
It is not an algorithmic guarantee that a sweep will find the global infimum.

For a fixed linear space \(V\subset L^2(\nu)\), the orthogonal projection
\(\Pi_Vh\) is best in \(V\):
\begin{equation}
    \|h-v\|_{L^2(\nu)}^2
    =
    ...
    \|\Pi_Vh-v\|_{L^2(\nu)}^2,
    \qquad v\in V.
    \label{eq:p33-projection-pythagoras}
\end{equation}
...
\begin{equation}
    ...
    \le
    \|h-v\|_{L^2(\nu)}.
    \label{eq:p33-projection-best}
\end{equation}
If \(V_p\subset V_{p'}\) with \(p<p'\), then
\begin{equation}
    \inf_{v\in V_{p'}}\|h-v\|_{L^2(\nu)}
    \le
    \inf_{v\in V_p}\|h-v\|_{L^2(\nu)}.
    \label{eq:p33-nested-monotone}
\end{equation}
This proves that enriching a nested linear basis cannot worsen the ideal
linear best-approximation error.

It does not prove that there is a universally optimal basis.  To see why,
consider \(L^2[-1,1]\).  The one-dimensional space \(V_1=\operatorname{span}\{1\}\)
represents the constant target \(h_1(z)=1\) exactly:
\begin{equation}
    \inf_{v\in V_1}\|h_1-v\|_{L^2}=0.
    \label{eq:p33-no-universal-1}
\end{equation}
The one-dimensional space \(V_2=\operatorname{span}\{z\}\) represents
\(h_2(z)=z\) exactly:
\begin{equation}
    \inf_{v\in V_2}\|h_2-v\|_{L^2}=0.
    \label{eq:p33-no-universal-2}
\end{equation}
Neither one-dimensional space is best for both targets.  A filtering posterior
changes with model, observation path, coordinate map, and preconditioner; the
basis must therefore be chosen and tested for the declared target family, not
asserted as universal.

The empirical fitting norms are
\begin{equation}
    \|v\|_{W_{\rm fit}}^2=v^\top W_{\rm fit}v,
    \qquad
    \|v\|_{W_{\rm hold}}^2=v^\top W_{\rm hold}v.
    \label{eq:p33-empirical-norms}
\end{equation}
These norms approximate \(L^2(\nu)\) only to the extent that the fitting and
holdout points, weights, and quadrature design approximate \(\nu\).  Therefore
training residual is evidence about the sampled problem; it is not by itself
a theorem about \eqref{eq:p33-design-error}.

\subsubsection{Basis-Family Taxonomy}

The following matching rules are approximation-guided heuristics, not
theorem-level guarantees for nonlinear filtering posteriors.  They are useful
because each basis family makes a different assumption about support,
regularity, and measure:
\begin{center}
\begin{tabular}{p{0.16\linewidth}p{0.18\linewidth}p{0.22\linewidth}p{0.27\linewidth}}
\toprule
Family & Natural setting & Mass behavior & Typical failure sign\\
\midrule
Legendre/Chebyshev & bounded smooth coordinate & diagonal or well-conditioned after matched weighting & slow convergence near kinks or unbounded tails\\
Hermite / polynomial chaos & Gaussianized reference coordinate & diagonal under Gaussian weight & poor fit for bounded discontinuities or heavy tails\\
Fourier & periodic coordinate & diagonal under uniform periodic measure & ringing for nonperiodic or nonsmooth boundaries\\
Piecewise polynomial / spline / wavelet & local sharp feature or threshold & block/local mass matrices & too many cells or levels if feature is global\\
Pilot dictionary / reduced basis & repeated model class with training targets & must be orthonormalized under declared measure & overfits pilot targets or moves with parameters\\
\bottomrule
\end{tabular}
\end{center}
The orthogonality claim belongs to the triplet
\begin{equation}
    (\text{coordinate map},\text{reference measure},\text{basis family}),
    \label{eq:p33-orthogonality-triplet}
\end{equation}
not to the basis name alone.  A Hermite basis is naturally matched to a
Gaussianized coordinate; a Legendre basis is naturally matched to a bounded
coordinate with uniform weight.  If the map \(T\) or \(\Psi\) changes the
effective measure, the mass matrix and conditioning must be checked again.

\subsubsection{Deterministic Basis And Rank Ladder}

The selection protocol uses an ordered ladder
\begin{equation}
    \mathcal L
    =
    \{(\mathcal B^{(q)},p^{(q)},R^{(q)},\rho^{(q)})\}_{q=1}^{Q}.
    \label{eq:p33-ladder}
\end{equation}
The ordering is declared before fitting.  A conservative ordering is
\begin{equation}
    \text{measure-matched global basis}
    \prec
    \text{higher degree}
    \prec
    \text{higher TT rank}
    \prec
    \text{local/piecewise basis}
    \prec
    \text{new preconditioner}.
    \label{eq:p33-ladder-order}
\end{equation}
The numerical claim comes from comparing candidates under common diagnostics,
not from asserting that the first named family is correct.

For candidate \(q\), fit on \(Z_{\rm fit}\) and compute
\begin{equation}
    \varepsilon_{\rm fit}^{(q)}
    =
    \frac{\|A_{\rm fit}^{(q)}g^{(q)}-y_{\rm fit}\|_{W_{\rm fit}}}
         {\max\{\|y_{\rm fit}\|_{W_{\rm fit}},\epsilon_{\rm abs}\}},
    \label{eq:p33-fit-residual}
\end{equation}
and on holdout points
\begin{equation}
    \varepsilon_{\rm hold}^{(q)}
    =
    \frac{\|A_{\rm hold}^{(q)}g^{(q)}-y_{\rm hold}\|_{W_{\rm hold}}}
         {\max\{\|y_{\rm hold}\|_{W_{\rm hold}},\epsilon_{\rm abs}\}}.
    \label{eq:p33-hold-residual}
\end{equation}
The overfit ratio is
\begin{equation}
    \Gamma^{(q)}
    =
    \frac{\varepsilon_{\rm hold}^{(q)}}
         {\max\{\varepsilon_{\rm fit}^{(q)},\epsilon_{\rm abs}\}}.
    \label{eq:p33-overfit-ratio}
\end{equation}
The normal matrix checked for conditioning is
\begin{equation}
    N_k^{(q)}
    =
    (A_k^{(q)})^\top W_{\rm fit}A_k^{(q)}+\rho^{(q)}I,
    \qquad
    \kappa^{(q)}=\max_k\operatorname{cond}(N_k^{(q)}).
    \label{eq:p33-conditioning}
\end{equation}
Ridge \(\rho\) may be fixed or laddered, but the choice must be declared in
\(\mathcal L\).  A candidate satisfies the numerical criteria only if
\begin{equation}
\begin{array}{ll}
\varepsilon_{\rm hold}^{(q)}\le\epsilon_{\rm hold},&
\Gamma^{(q)}\le\Gamma_{\max},\\[1mm]
\Delta_Z^{(q)}\le\epsilon_Z,&
\Delta_{\rm marg}^{(q)}\le\epsilon_{\rm marg},\\[1mm]
\kappa^{(q)}\le\kappa_{\max},&
\max_k R_k^{(q)}<R_{\max},\\[1mm]
\delta_{\rm quad}^{(q)}\le\epsilon_{\rm quad},&
\Delta_{\rm FD}^{(q)}\le\epsilon_{\rm FD}\quad\text{if gradients are claimed}.
\end{array}
    \label{eq:p33-pass-criteria}
\end{equation}
The veto precedence is
\begin{equation}
    \text{finite values}
    \succ \text{quadrature validity}
    \succ \text{conditioning}
    \succ \text{holdout accuracy}
    \succ \text{integral stability}
    \succ \text{derivative parity}.
    \label{eq:p33-veto-order}
\end{equation}
The selected candidate is the first candidate satisfying the declared criteria:
\begin{equation}
    q_\star=\min\{q:\text{candidate }q\text{ satisfies \eqref{eq:p33-pass-criteria}}\}.
    \label{eq:p33-selected-candidate}
\end{equation}
If several candidates are equivalent under the declared order, the tie-break is
\begin{equation}
    \min
    \left(
    \sum_{k=1}^D R_{k-1}^{(q)}p_k^{(q)}R_k^{(q)}
    \right),
    \label{eq:p33-tie-break}
\end{equation}
that is, choose the smaller stored representation.  The stored record is
\begin{equation}
    B_{\rm basis}
    =
    \{\mathcal B^{(q_\star)},p^{(q_\star)},R^{(q_\star)},\rho^{(q_\star)},
      M_{1:D}^{(q_\star)},Z_{\rm fit},Z_{\rm hold},\text{diagnostics}\}.
    \label{eq:p33-basis-record}
\end{equation}
If no candidate satisfies the criteria, the selected index is empty:
\begin{equation}
    q_\star=\varnothing.
    \label{eq:p33-no-candidate}
\end{equation}
The next mathematical alternatives are to enlarge the domain, change the
coordinate map or preconditioner, change the basis family, increase the
declared maximum rank, or state that the TT basis/rank budget did not resolve
the target.  Derivative
parity \(\Delta_{\rm FD}\le\epsilon_{\rm FD}\) is a veto when the fixed-branch
gradient is reported or used.  If the run is value-only, the derivative row in
\eqref{eq:p33-pass-criteria} is marked not applicable rather than silently
passed.

\subsubsection{How A Bad Basis Fails}

A bad basis is not a philosophical concern; it leaves numerical traces.  The
main traces are
\begin{equation}
\begin{array}{ll}
\varepsilon_{\rm hold}\text{ large} & \text{poor approximation away from fitting points},\\
\Gamma\text{ large} & \text{overfitting to selected rows},\\
\Delta_Z\text{ large} & \text{unstable evidence under enrichment},\\
\Delta_{\rm marg}\text{ large} & \text{wrong low-dimensional retained behavior},\\
\kappa\text{ large} & \text{unstable coefficient solve},\\
\max_kR_k=R_{\max} & \text{rank budget saturated},\\
\Delta_{\rm FD}\text{ large} & \text{derivative does not match the declared scalar}.
\end{array}
    \label{eq:p33-bad-basis-traces}
\end{equation}
The last two rows must be read relative to the declared budget and geometry.
Rank saturation means saturation at the chosen \(R_{\max}\); it is not a proof
that no TT representation could work at a different budget.  A basis failure
means poor one-coordinate approximation after the current map.  A coordinate
or preconditioner failure means the target geometry is difficult before the
basis even begins to approximate it.
To distinguish one-coordinate basis insufficiency from TT-rank insufficiency,
run the local discrimination ladder
\begin{equation}
\begin{array}{ll}
\text{basis enrichment test} &
    (\mathcal B,p,R)\mapsto(\mathcal B,p^+,R),\\
\text{rank enrichment test} &
    (\mathcal B,p,R)\mapsto(\mathcal B,p,R^+),\\
\text{map/preconditioner test} &
    (\Psi,T)\mapsto(\Psi^+,T^+).
\end{array}
    \label{eq:p33-discrimination-ladder}
\end{equation}
If basis enrichment reduces \(\varepsilon_{\rm hold}\), \(\Delta_Z\), and
\(\Delta_{\rm marg}\) while rank enrichment does not, diagnose basis
insufficiency.  If rank enrichment helps at fixed basis, diagnose insufficient
interaction capacity.  If neither helps until the coordinate map or
preconditioner changes, diagnose poor geometry rather than a local basis
problem.

\subsubsection{Learned Bases And The Freeze Boundary}

A learned basis is an optional extension, not the default basis path in this
note.
A learned basis starts from a raw library
\begin{equation}
    \varphi_k(z)
    =
    (\varphi_{k,1}(z),\ldots,\varphi_{k,m_{\rm raw}}(z))^\top
    \label{eq:p33-raw-library}
\end{equation}
and learns
\begin{equation}
    \psi_{k,\ell}^{U}(z)
    =
    \sum_{m=1}^{m_{\rm raw}}U_{k,m\ell}\varphi_{k,m}(z),
    \qquad \ell=0,\ldots,p_k-1.
    \label{eq:p33-learned-basis}
\end{equation}
For pilot targets \(h^{(i)}\), one possible training objective is
\begin{equation}
    \min_{U,C^{(1:M)}}
    \sum_{i=1}^{M}
    \|h^{(i)}-\widehat h_{U,C^{(i)}}^{(i)}\|_{W_i}^2
    +
    \lambda_U\mathcal R(U).
    \label{eq:p33-learned-objective}
\end{equation}
After learning, orthonormalize under the declared mass:
\begin{equation}
    G_k(U)=\int \psi_k^{U}(z)\psi_k^{U}(z)^\top\omega_k(z)\dd z,
    \qquad
    \psi_k^{\rm fr}(z)=G_k(U)^{-1/2}\psi_k^{U}(z).
    \label{eq:p33-learned-orthonormalize}
\end{equation}
The freeze boundary is the end of pilot training and orthonormalization.  The
frozen basis record is
\begin{equation}
    B_{\rm learn}
    =
    \{\varphi_k,U_k,G_k^{-1/2},\omega_k,\Psi,T,\text{pilot data id},
      \text{seed},\text{acceptance diagnostics}\}_{k=1}^D.
    \label{eq:p33-learned-artifact}
\end{equation}
All downstream filtering and fixed-branch derivatives condition on
\(B_{\rm learn}\).  Re-training \(U_k\) as \(\beta\) changes defines a
different scalar.

\subsubsection{Relation To Neural-Operator Basis Learning}

The phrase ``neural operator'' should not be read as a sudden replacement for
the tensor-train filter.  The issue is more modest.  The preceding sections
choose a one-dimensional basis in each coordinate and then fit TT
coefficients.  A panelist may ask whether the basis itself could be chosen by
a trained rule, rather than by hand.  Neural-operator language is one way to
make that question precise, but it needs two steps of background before it is
useful.

First, recall the ordinary approximation problem.  If a one-dimensional target
has the form
\begin{equation}
    g(z)\approx c_0+c_1z+c_2z^2,
    \label{eq:p33-ordinary-polynomial-approximation}
\end{equation}
then the basis \((1,z,z^2)\) is fixed and only the coefficients
\((c_0,c_1,c_2)\) are fitted.  If the same problem is written with Gaussian
bumps,
\begin{equation}
    g(z)\approx
    \sum_{\ell=1}^{p}c_\ell
    \exp\left\{-\frac{(z-m_\ell)^2}{2s_\ell^2}\right\},
    \label{eq:p33-gaussian-bump-basis}
\end{equation}
then the coefficients \(c_\ell\) are still fitted, but the centers
\(m_\ell\) and widths \(s_\ell\) are part of the basis design.  A poor choice
of centers and widths wastes degrees of freedom.  A good choice puts local
features where the target changes rapidly.  The same distinction appears in a
TT approximation: the TT cores fit coefficients, while the coordinate
functions \(\psi_{k,\ell}\), their degrees, and their reference measures
define the approximation space in which the cores are allowed to work.

Second, basis choice can depend on the model parameter.  In a filtering
problem, changing \(\theta\) can change posterior scale, skewness, tail
thickness, and coupling strength.  A basis that is efficient near one
parameter value may be inefficient near another.  For example, if a local
posterior factor is approximately
\begin{equation}
    h_\theta(z)
    =
    \exp\left\{-\frac{(z-\mu(\theta))^2}{2s(\theta)^2}\right\}
    \{1+\alpha(\theta)z\},
    \label{eq:p33-parameter-dependent-local-shape}
\end{equation}
then centers near \(\mu(\theta)\), scales comparable to \(s(\theta)\), and a
few functions able to represent the linear tilt \(\alpha(\theta)z\) are more
natural than a parameter-blind basis on a large domain.  The learned-basis
question is therefore not mysterious: can one train a rule that reads
\(\theta\) and proposes a numerically useful approximation space?

A scalar function maps a number or vector to a number:
\begin{equation}
    f:\mathbb R^d\to\mathbb R,
    \qquad
    x\mapsto f(x).
    \label{eq:p33-function-vs-operator-1}
\end{equation}
An operator maps one function, field, or model input to another function.  For
example, a parametric simulator or filtering solver can be viewed schematically
as
\begin{equation}
    \mathcal G:
    a(\cdot)\mapsto u(\cdot),
    \label{eq:p33-function-vs-operator-2}
\end{equation}
where \(a\) might describe coefficients, forcing, parameters, or observation
information, and \(u\) is the resulting solution, density, or value function.
In a finite-dimensional parameterized family, the input may simply be
\(\theta\), and the output may be a function of state:
\begin{equation}
    \mathcal G:
    \theta\mapsto h_\theta(\cdot).
    \label{eq:p33-function-vs-operator-3}
\end{equation}
A neural operator is a neural-network approximation to a map like
\eqref{eq:p33-function-vs-operator-2} or
\eqref{eq:p33-function-vs-operator-3}.  Operator-learning methods such as
DeepONet, Fourier neural operators, and the general neural-operator framework
learn maps between function spaces or parameterized families of functions
\cite{lu2021deeponet,li2021fourier,kovachki2023neuraloperator}.

This is relevant because a basis is also a mathematical object that can be the
output of a learned rule.  In the fixed-basis TT method, we choose
\(\{\psi_{k,\ell}\}\) first and only fit coefficients.  In a learned-basis
method, we can instead learn a map from a model parameter to the numerical
objects that define a useful TT approximation space.  Thus the neural operator
is not a new filtering algorithm in this paragraph; it is a possible basis
generator:
\begin{equation}
    B_\eta:\Theta\to\mathcal B_{\rm TT},
    \qquad
    \theta\mapsto B_\eta(\theta),
    \label{eq:p33-neural-basis-map}
\end{equation}
where \(B_\eta(\theta)\) denotes the coordinate maps, one-dimensional basis
functions, degree choices, mass matrices, preconditioner parameters, and
fixed-shape arrays needed by the TT approximation.
The answer to the reviewer question is therefore yes at the level of
approximation spaces: learning \(\theta\mapsto B_\eta(\theta)\) is an
operator-learning-style problem.  The filtering contract below is narrower
because \(B_\eta\) is used only to choose an approximation space, not to replace
the posterior filter itself.

The important distinction is what the learned map is allowed to do during
Bayesian estimation.  In an HMC run the observation path \(y_{1:T}\) is fixed,
and each proposed \(\theta\) requires a value and gradient of the same
approximate likelihood.  A data-dependent learned basis map
\begin{equation}
    (\theta,y_{1:T})\mapsto B_\eta(\theta,y_{1:T})
    \label{eq:p33-data-dependent-basis-map}
\end{equation}
is mathematically possible, but it asks the network to learn posterior
geometry across observation paths.  That is close to learning the filtering
problem itself and is not the baseline contract here.  The more defensible
use is to train a parameter-conditioned basis proposal offline, evaluate it at
a representative parameter value such as the approximate MAP, and then freeze
it:
\begin{equation}
    \widehat\theta_{\rm MAP}
    =
    \arg\max_\theta\{\log p(\theta)+\widehat\ell_{\rm pilot}(\theta)\},
    \qquad
    B_{\rm MAP}=B_\eta(\widehat\theta_{\rm MAP}),
    \label{eq:p33-map-basis}
\end{equation}
\begin{equation}
    B(\theta)\equiv B_{\rm MAP}
    \quad\text{for all subsequent HMC proposals.}
    \label{eq:p33-map-basis-frozen}
\end{equation}
The HMC scalar is then
\begin{equation}
    \widehat\ell_T^{\rm MAPbasis}(\theta)
    =
    \widehat\ell_T(\theta;B_{\rm MAP},y_{1:T}),
    \label{eq:p33-map-basis-scalar}
\end{equation}
and its derivative is the fixed-basis derivative
\begin{equation}
    \frac{d}{d\theta}\widehat\ell_T^{\rm MAPbasis}(\theta)
    =
    \partial_\theta \widehat\ell_T(\theta;B_{\rm MAP},y_{1:T}).
    \label{eq:p33-map-basis-gradient}
\end{equation}
There is no moving-basis term in \eqref{eq:p33-map-basis-gradient}, because
\(B_{\rm MAP}\) is held fixed after \eqref{eq:p33-map-basis}.

This also explains why a one-layer neural network with fixed polynomial
features is not different from the fixed-basis method.  If
\begin{equation}
    f(x)=\sum_{\ell=0}^{p}c_\ell\phi_\ell(x),
    \qquad
    \phi_\ell(x)=x^\ell,
    \label{eq:p33-fixed-feature-network}
\end{equation}
then \(f\) is a polynomial basis expansion.  In the TT core,
\begin{equation}
    H_k(z_k)=
    \sum_{\ell=0}^{p_k-1}
    C_k[:,\ell,:]\psi_{k,\ell}(z_k)
    \label{eq:p33-fixed-feature-tt-core}
\end{equation}
is the same fixed-feature idea with matrix-valued coefficients.  A neural
basis becomes different only when hidden weights, feature maps, or basis
mixing matrices are learned.

One concrete parameterization keeps the learned part finite-dimensional.  Let
\(\varphi_k(z)\in\mathbb R^{m_{\rm raw}}\) be a raw library of polynomials,
splines, Fourier modes, or wavelets.  Let the neural operator output fixed
shape mixing matrices
\begin{equation}
    U_k(\theta)=B_{\eta,k}(\theta)\in\mathbb R^{m_{\rm raw}\times p_k}.
    \label{eq:p33-neural-mixing}
\end{equation}
The proposed basis is
\begin{equation}
    \psi_k^\theta(z)
    =
    U_k(\theta)^\top \varphi_k(z).
    \label{eq:p33-neural-raw-basis}
\end{equation}
Its Gram matrix under the declared reference measure is
\begin{equation}
    G_k(\theta)
    =
    \int
    \psi_k^\theta(z)\psi_k^\theta(z)^\top\omega_k(z)\dd z
    =
    U_k(\theta)^\top M_{\varphi,k}U_k(\theta),
    \label{eq:p33-neural-gram}
\end{equation}
where
\begin{equation}
    M_{\varphi,k}
    =
    \int\varphi_k(z)\varphi_k(z)^\top\omega_k(z)\dd z.
    \label{eq:p33-neural-raw-mass}
\end{equation}
If \(G_k(\theta)\) is positive definite, an orthonormalized basis is
\begin{equation}
    \psi_k^{\rm orth}(z;\theta)
    =
    G_k(\theta)^{-1/2}U_k(\theta)^\top\varphi_k(z).
    \label{eq:p33-neural-orth-basis}
\end{equation}
For the MAP-frozen path, all matrices in
\eqref{eq:p33-neural-mixing}--\eqref{eq:p33-neural-orth-basis} are evaluated
at \(\widehat\theta_{\rm MAP}\) and then treated as constants.

Training should judge the learned map by downstream filtering approximation,
not by arbitrary closeness to a nonunique teacher basis.  For training
parameters \(\theta_i\) and simulated or representative observation paths
\(y^{(i,m)}_{1:T}\), define square-root targets
\begin{equation}
    h_{i,m}(z)
    =
    e^{c_{i,m}/2}
    \sqrt{\widetilde q_{\theta_i,y^{(i,m)}}(z)}.
    \label{eq:p33-neural-training-target}
\end{equation}
For the proposed basis \(B_\eta(\theta_i)\), fit the inner TT coefficients
\begin{equation}
    C_{i,m}^\star(\eta)
    =
    \arg\min_C
    \left\|
    h_{i,m}-\widehat h_{B_\eta(\theta_i),C}
    \right\|_{W_{\rm fit}}^2
    +
    \rho\|C\|_2^2.
    \label{eq:p33-neural-inner-fit}
\end{equation}
Then train \(\eta\) by the same diagnostics that will later be used to accept
or reject the basis:
\begin{equation}
\begin{aligned}
    \min_\eta
    \sum_{i,m}\Big[
    &(\varepsilon_{\rm hold}^{(i,m)})^2
    +\lambda_Z(\Delta_Z^{(i,m)})^2
    +\lambda_{\rm marg}(\Delta_{\rm marg}^{(i,m)})^2\\
    &+\lambda_{\rm cond}\,
      \operatorname{penalty}(\kappa^{(i,m)})
    +\lambda_{\rm orth}
      \sum_k\|G_k(\theta_i)-I\|_F^2
    \Big].
\end{aligned}
    \label{eq:p33-neural-training-objective}
\end{equation}
This objective says that the neural operator is not replacing the filter.  It
is proposing an approximation space that must make the actual squared-TT
filter accurate, normalized, marginally stable, and well conditioned.

After \(B_{\rm MAP}\) is frozen, it must still be validated across the
posterior region explored by HMC.  Let
\(\theta^{(s)}\) be a pilot cloud around the MAP.  The MAP-frozen basis passes
only if
\begin{equation}
    \max_s
    \varepsilon_{\rm hold}(\theta^{(s)};B_{\rm MAP})
    \le \epsilon_{\rm hold},
    \label{eq:p33-map-cloud-holdout}
\end{equation}
\begin{equation}
    \max_s
    \left|
    \log\widehat Z(\theta^{(s)};B_{\rm MAP})
    -
    \log\widehat Z_{\rm enriched}(\theta^{(s)})
    \right|
    \le \epsilon_Z,
    \label{eq:p33-map-cloud-evidence}
\end{equation}
and
\begin{equation}
    \max_s
    \Delta_{\rm marg}(\theta^{(s)};B_{\rm MAP})
    \le \epsilon_{\rm marg}.
    \label{eq:p33-map-cloud-marginal}
\end{equation}
If these checks fail, the learned basis is not used for the HMC run; the
method falls back to the deterministic basis/rank ladder in
\eqref{eq:p33-ladder}.  The methodological point is therefore narrow but
useful: a neural operator may help choose the approximation space at the MAP,
while the reported posterior computation still uses a fixed basis and the
fixed-branch derivative of a declared scalar.

\subsubsection{Frozen-Basis And Moving-Basis Gradients}

For the frozen-basis scalar, \(\dot\psi_{k,\ell}=0\), so
\begin{equation}
    \dot H_k(z_k)
    =
    \sum_{\ell=0}^{p_k-1}
    \dot C_k[:,\ell,:]\psi_{k,\ell}(z_k).
    \label{eq:p33-frozen-hdot}
\end{equation}
If the basis moves with \(\beta\), then
\begin{equation}
    \dot H_k(z_k)
    =
    \sum_{\ell=0}^{p_k-1}
    ...
    \sum_{\ell=0}^{p_k-1}
    C_k[:,\ell,:]\dot\psi_{k,\ell}(z_k;\beta).
    \label{eq:p33-moving-hdot}
\end{equation}
The least-squares row becomes
\begin{equation}
\begin{aligned}
    \dot A_{j,k}[a,\ell,b]
    &=
    \dot L_{j,k}[a]\psi_{k,\ell}(z_k^{(j)})R_{j,k}[b]
    +L_{j,k}[a]\psi_{k,\ell}(z_k^{(j)})\dot R_{j,k}[b]\\
    &\quad+
    L_{j,k}[a]\dot\psi_{k,\ell}(z_k^{(j)})R_{j,k}[b].
\end{aligned}
    \label{eq:p33-moving-row}
\end{equation}
The mass matrix derivative is
\begin{equation}
    \dot M_k[\ell,m]
    =
    \int
    \dot\psi_{k,\ell}\psi_{k,m}\omega_k
    +
    \psi_{k,\ell}\dot\psi_{k,m}\omega_k
    +
    \psi_{k,\ell}\psi_{k,m}\dot\omega_k
    \dd z.
    \label{eq:p33-moving-mass}
\end{equation}
Consequently a one-core square mass term differentiates as
\begin{equation}
    \frac{d}{d\beta}(C^\top M C)
    =
    \dot C^\top MC+C^\top M\dot C+C^\top\dot M C.
    \label{eq:p33-moving-square-derivative}
\end{equation}
The fixed-branch derivative in this note deliberately uses the frozen-basis
case \(\dot\psi=0\) and \(\dot M=0\).  The moving-basis equations above define
the extra obligations if one later chooses to differentiate through basis
adaptation.

\subsubsection{Eleven Basis-Choice Questions}

\begin{center}
\begin{tabular}{p{0.04\linewidth}p{0.25\linewidth}p{0.21\linewidth}p{0.28\linewidth}p{0.12\linewidth}}
\toprule
No. & Question & Section & Decisive equations & Diagnostic\\
\midrule
1 & Why basis choice matters & Design tuple & \eqref{eq:p33-design-tuple}--\eqref{eq:p33-pullback-nu} & declared design\\
2 & Full design tuple & Design tuple & \eqref{eq:p33-design-tuple}, \eqref{eq:p33-design-table} & stored record\\
3 & Basis in cores and LS rows & Core evaluation & \eqref{eq:p33-core-eval}--\eqref{eq:p33-normal-equation} & row/shape check\\
4 & Mass, normalization, marginalization & Mass contractions & \eqref{eq:p33-mass-matrix}--\eqref{eq:p33-retained-normalized} & mass/evidence check\\
5 & Meaning of optimal basis & Projection scope & \eqref{eq:p33-design-error}--\eqref{eq:p33-no-universal-2} & scoped claim\\
6 & Basis taxonomy & Taxonomy & \eqref{eq:p33-orthogonality-triplet} and taxonomy table & matched measure\\
7 & Basis/rank ladder & Selection ladder & \eqref{eq:p33-ladder}--\eqref{eq:p33-selected-candidate} & first satisfying candidate\\
8 & Diagnostics & Selection ladder & \eqref{eq:p33-fit-residual}--\eqref{eq:p33-pass-criteria} & veto table\\
9 & Bad-basis failure & Failure traces & \eqref{eq:p33-bad-basis-traces}--\eqref{eq:p33-discrimination-ladder} & discrimination ladder\\
10 & Learned and neural-operator basis & Learned/MAP-frozen basis & \eqref{eq:p33-raw-library}--\eqref{eq:p33-map-cloud-marginal} & frozen record and MAP-cloud checks\\
11 & Frozen vs moving gradients & Gradient split & \eqref{eq:p33-frozen-hdot}--\eqref{eq:p33-moving-square-derivative} & derivative parity\\
\bottomrule
\end{tabular}
\end{center}

\subsubsection{Why The Basis Choice Is A Controlled Approximation}

The basis is an approximation assumption, but it is a controlled one.
The method declares the coordinate map, reference measure, basis family,
degree, ranks, points, weights, ridge, and sweeps before fitting.  It compares
candidate basis/rank designs under the same residual, holdout, evidence,
marginal, quadrature, conditioning, rank-saturation, and derivative checks.  A
candidate that does not satisfy these checks is not used for the reported
approximation.  A candidate that does satisfy them is held fixed before the
fixed-branch derivative is interpreted.  This is the same mathematical
discipline used by spectral, sparse-grid, and reduced-basis numerical methods:
approximation spaces are chosen by structure, then judged by their residual,
integral, marginal, conditioning, and derivative consequences.
...
by \(\overline H_k\) and contracts adjacent matrices.  The retained object is a
...
\((x_t,\alpha,x_{t-1})\).  Output: \(q_t\) or \(\log q_t\).  Quantities to retain:
...
supplies a differentiable fixed-branch specialization.
...
For a concrete version of this row, see the three-variable exponential example
in Section~\ref{subsec:p30-worked-tt-regression}.  There the middle-core row
is written entry by entry as
\[
    A_{j,2}[a,\ell,b]=L_j[a]\,y_j^\ell\,R_j[b],
\]
and multiplying that row by the vectorized middle core reproduces the TT value
at the fitting point.  The general formula \eqref{eq:p24-a1-2b} is the same
calculation with arbitrary ranks and arbitrary local basis functions.
...
cores \(F,A,H\) for the fitted TT.  Quantities to retain: cores, rank vector, basis
definition, fit residual, and any scaling shift.  Operation: alternating core
...
\(\widehat p_t(x_t)\) and \(\widehat p_t(\alpha)\).  Quantities to retain: retained
TT/evaluator and contraction data.  Operation: endpoint TT core
...
\(\widehat p\).  Quantities to retain is \(\phi\)'s cores, \(\lambda\), \(\tau\),
...
\begin{equation}
    ...
    \label{eq:p24-m4}
\end{equation}
This is Zhao and Cui \cite{zhao2024ttsequential}, Eq. (14) in the notation of this note.
...
their Cholesky factors.  Quantities to retain is the list of mass matrices needed by
...
\subsection*{Why This Transport Is The Right Device}

The KR map is useful because it turns the question ``how do we sample from a
high-dimensional density?'' into a sequence of one-dimensional questions.
Start with the ordinary one-dimensional probability integral transform:
\begin{equation}
    U=F_X(X)=\int_{-\infty}^{X}p_X(s)\dd s,
    \qquad X\sim p_X,
    \label{eq:p26-kr-teach-1}
\end{equation}
which implies \(U\sim\operatorname{Unif}(0,1)\).  Conversely,
\begin{equation}
    X=F_X^{-1}(U),
    \qquad U\sim\operatorname{Unif}(0,1),
    \label{eq:p26-kr-teach-2}
\end{equation}
has density \(p_X\).  The multivariate construction repeats this idea after
conditioning on the coordinates that have already been chosen:
\begin{equation}
\begin{array}{rcl}
u_1 &=& F_1(r_1),\\[1mm]
u_2 &=& F_2(r_2\mid r_1),\\[1mm]
u_3 &=& F_3(r_3\mid r_1,r_2),\\
&\vdots&\\
u_D &=& F_D(r_D\mid r_1,\ldots,r_{D-1}).
\end{array}
    \label{eq:p26-kr-teach-3}
\end{equation}
The inverse construction is equally sequential:
\begin{equation}
\begin{array}{rcl}
r_1 &=& F_1^{-1}(u_1),\\[1mm]
r_2 &=& F_2^{-1}(u_2\mid r_1),\\[1mm]
r_3 &=& F_3^{-1}(u_3\mid r_1,r_2),\\
&\vdots&\\
r_D &=& F_D^{-1}(u_D\mid r_1,\ldots,r_{D-1}).
\end{array}
    \label{eq:p26-kr-teach-4}
\end{equation}
Thus a product draw \(u\sim\operatorname{Unif}([0,1]^D)\) becomes a dependent
draw \(r\sim\widehat p\).  This is the reason the map is a transport map:
it transports the simple product measure to the fitted dependent density.

The triangular form is the essential feature.  Its Jacobian matrix has zeros
above the diagonal,
\begin{equation}
    \nabla F(r)
    =
    \begin{bmatrix}
    \partial_{r_1}F_1 & 0 & \cdots & 0\\
    \partial_{r_1}F_2 & \partial_{r_2}F_2 & \cdots & 0\\
    \vdots & \vdots & \ddots & \vdots\\
    \partial_{r_1}F_D & \partial_{r_2}F_D & \cdots &
    \partial_{r_D}F_D
    \end{bmatrix},
    \label{eq:p26-kr-teach-5}
\end{equation}
so the determinant is just the product of one-dimensional conditional
densities:
\begin{equation}
    \det\nabla F(r)
    =
    \prod_{k=1}^D
    \partial_{r_k}F_k(r_k\mid r_{<k})
    =
    \prod_{k=1}^D
    \widehat p(r_k\mid r_{<k})
    =
    \widehat p(r).
    \label{eq:p26-kr-teach-6}
\end{equation}
For the fitted squared TT, the hard part is not this determinant identity; it
is obtaining the conditional densities in the middle product.  The squared-TT
mass contractions provide exactly those marginal ratios:
\begin{equation}
    \widehat p(r_k\mid r_{<k})
    =
    \frac{\widehat p(r_{\le k})}{\widehat p(r_{<k})}
    =
    \frac{\text{TT contraction retaining }r_{\le k}}
         {\text{TT contraction retaining }r_{<k}}.
    \label{eq:p26-kr-teach-7}
\end{equation}
This is why conditional/KR maps are not an ornament added after fitting.  They
are the natural way to convert the fitted nonnegative density into a sampler,
a particle proposal, or a backward smoother.
...
mass contractions and upper-triangular conditional CDFs.  Quantities to retain must
...
inverse-CDF samplers.  Quantities to retain includes integration direction, mass
...
mass contractions for conditional maps.  Quantities to retain: all of those objects.
...
\begin{equation}
    ...
    \label{eq:p24-f2}
\end{equation}
This is Zhao and Cui \cite{zhao2024ttsequential}, Eq. (21) in the fixed-branch notation used here.
...
Outputs: new particles and normalized weights.  Quantities to retain: particles,
...
samples.  Outputs: path samples and normalized smoothing weights.  Quantities
to retain: map sequence, samples, weights, and ESS.  Operation: final joint sample,
...
\subsection*{Why Preconditioning And KR Maps Belong Together}

The preconditioner and the KR map solve complementary problems.  The KR map
can sample from a fitted density once the density has been represented well.
The preconditioner tries to make the density easier to represent before the TT
fit is performed.  In equations, let the exact target be decomposed as
\begin{equation}
    q(r)=\rho(r)m(r),
    \qquad m(r)=q(r)/\rho(r),
    \label{eq:p26-prec-teach-1}
\end{equation}
and choose \(T\) so that \(\rho\) is mapped to a product reference \(\eta\):
\begin{equation}
    \rho(r)\dd r
    =
    \eta(u)\dd u,
    \qquad u=T(r).
    \label{eq:p26-prec-teach-2}
\end{equation}
...
\begin{equation}
    ...
    =
    m(T^{-1}(u))\eta(u)\dd u.
    \label{eq:p26-prec-teach-3}
\end{equation}
The TT therefore sees the residual multiplier \(m(T^{-1}(u))\), not the
entire original density geometry.  If \(\rho\) captures the main posterior
ellipsoid, ridge, or concentration region, then
\begin{equation}
    \operatorname{osc}\{\log m(T^{-1}(u)):u\in\Omega_u\}
    <
    \operatorname{osc}\{\log q(r):r\in\Omega_r\}
    \label{eq:p26-prec-teach-4}
\end{equation}
on the fitting domain, where \(\operatorname{osc}(f)=\sup f-\inf f\).  This is
the concrete mathematical sense in which the residual can be flatter.

After the residual has been fitted, a KR map can be built in the easier
\(u\)-coordinates:
\begin{equation}
    F^\sharp(u)
    =
    \bigl(F^\sharp_1(u_1),
    F^\sharp_2(u_2\mid u_1),\ldots,
    F^\sharp_D(u_D\mid u_{<D})\bigr).
    \label{eq:p26-prec-teach-5}
\end{equation}
If \(\xi\sim\operatorname{Unif}([0,1]^D)\), then the two-step sampler is
\begin{equation}
    u=(F^\sharp)^{-1}(\xi),
    \qquad
    r=T^{-1}(u).
    \label{eq:p26-prec-teach-6}
\end{equation}
The density calculation is the product of the two Jacobian identities:
\begin{equation}
    \left|\det\nabla F^\sharp(u)\right|
    \left|\det\nabla T(r)\right|
    =
    \widehat\nu^\sharp(u)
    \left|\det\nabla T(r)\right|
    \approx q(r).
    \label{eq:p26-prec-teach-7}
\end{equation}
This is the teach-back version of the method: the preconditioner removes the
dominant geometry, the squared TT fits the remaining density, and the KR map
turns the fitted density into sequential conditional inversions.

Equivalently, the bridge answers a coarse geometric question and the KR map
answers a sampling question:
\begin{equation}
    \rho \approx q
    \quad\Longrightarrow\quad
    q^\sharp(u)\approx \eta(u)\times\text{moderate residual},
    \qquad
    F^\sharp:\ q^\sharp \longrightarrow \operatorname{Unif}([0,1]^D).
    \label{eq:p26-prec-teach-8}
\end{equation}
The inverse proposal then has the readable form
\begin{equation}
    \xi\sim\operatorname{Unif}([0,1]^D)
    \quad\mapsto\quad
    u=(F^\sharp)^{-1}(\xi)
    \quad\mapsto\quad
    r=T^{-1}(u).
    \label{eq:p26-prec-teach-9}
\end{equation}
So the bridge captures the coarse posterior geometry, the squared TT captures
what the bridge missed, and the KR inverse converts that fitted residual
density into a sequential one-dimensional proposal construction.
...
marginal/conditional map objects.  Quantities to retain: map evaluators, inverse map
...
map \(T_t^\ell\), inverse map \(r=\mu_t+L_tu\), and log determinant.  Quantities
to retain: \(\mu_t,L_t\) and any covariance regularization.  Diagnostics:
...
its KR map.  Quantities to retain: exponents, bridge TT cores, defensive mass, and
...
Zhao and Cui's Algorithm 5(c.2) is compact because it uses the lower triangular
map \(T_t^\ell\) and the residual marginal from step (c.1) in one line.  We now
derive that line slowly.  The goal is to prove the physical retained marginal
\[
    \widehat\pi_t(x_t,\theta\mid y_{1:t})
\]
from the three-block pulled-back density, without losing the Jacobian direction.

First name the two retained physical variables and the old-state variable:
\begin{equation}
    a=(x_t,\theta),
    \qquad
    b=x_{t-1}.
    \label{eq:p30-c2-0}
\end{equation}
The lower bridge map is block triangular:
\begin{equation}
    T_t^\ell(a,b)
    =
    \bigl(T_A(a),T_B(b\mid a)\bigr),
    \qquad
    T_A(a)=
    \bigl(T_{t,t}^{\ell}(x_t),
    T_{t,\theta}^{\ell}(\theta\mid x_t)\bigr),
    \label{eq:p30-c2-1}
\end{equation}
where \(T_B(b\mid a)=T_{t,t-1}^{\ell}(x_{t-1}\mid x_t,\theta)\).  Write
\[
    u_A=(u_t,u_\theta),
    \qquad
    u_B=u_{t-1}.
\]
Thus \(u_A=T_A(a)\) and \(u_B=T_B(b\mid a)\).
...
\begin{equation}
    ...
    \label{eq:p24-p16a}
\end{equation}
The source formula to reach is
\begin{equation}
    \widehat\pi_t(a)
    =
    \frac{\widehat\nu_t^\sharp\!\left(T_A(a)\right)}
    {\eta_A\!\left(T_A(a)\right)}
    \widehat\rho_{t,A}(a),
    \label{eq:p30-c2-source-form}
\end{equation}
where
\begin{equation}
    \widehat\rho_{t,A}(a)=\int \widehat\rho_t(a,b)\dd b,
    \qquad
    \widehat\nu_t^\sharp(u_A)=
    \int \widehat\nu_t^\sharp(u_A,u_B)\dd u_B,
    \label{eq:p30-c2-2}
\end{equation}
and
\begin{equation}
    \eta_A(u_A)=\int \eta(u_A,u_B)\dd u_B.
    \label{eq:p30-c2-3}
\end{equation}
For Zhao and Cui's product reference,
\(\eta(u_A,u_B)=\eta_A(u_A)\eta_B(u_B)\), so \(\eta_A\) is just the
reference density of the retained transformed coordinates.

The only delicate point is how \(\widehat\rho_t(a,b)\dd b\) transforms when
\(a\) is fixed.  Because \(T_t^\ell\) pushes the bridge approximation to the
reference density,
\begin{equation}
    \widehat\rho_t(a,b)\dd a\dd b
    =
    C_\rho\,\eta(u_A,u_B)\dd u_A\dd u_B,
    \label{eq:p30-c2-4}
\end{equation}
for a bridge normalizing constant \(C_\rho>0\).  Marginalizing both sides over
\(b\) or \(u_B\) gives the retained bridge marginal identity
\begin{equation}
    \widehat\rho_{t,A}(a)\dd a
    =
    C_\rho\,\eta_A(u_A)\dd u_A.
    \label{eq:p30-c2-5}
\end{equation}
Dividing \eqref{eq:p30-c2-4} by \eqref{eq:p30-c2-5} gives the conditional
change-of-variables identity
\begin{equation}
    \widehat\rho_t(b\mid a)\dd b
    =
    \eta_B(u_B\mid u_A)\dd u_B.
    \label{eq:p30-c2-6}
\end{equation}
For a product reference, \(\eta_B(u_B\mid u_A)=\eta_B(u_B)\), but we keep the
conditional notation because it shows what must be true even for a
block-triangular non-product reference.
...
\begin{equation}
    ...
    \label{eq:p24-p16b.2}
\end{equation}
This is the conditional KR transport for the old-state block: once the retained
block \((x_t,\theta)\) is held fixed, the monotone lower-triangular map
\(x_{t-1}\mapsto u_{t-1}\) changes the conditional bridge law into the
conditional reference law.
...
\(\dd u_{t-1}\).  In the compact notation \((a,b,u_A,u_B)\), the same identity is
\begin{equation}
    \widehat\rho_t(a,b)\dd b
    =
    \frac{\widehat\rho_{t,A}(a)}
    {\eta_A(T_A(a))}
    \eta(T_A(a),u_B)\dd u_B.
    \label{eq:p30-c2-7}
\end{equation}
This is the cancellation identity.  It is where the Jacobian enters the
derivation; it is not an extra factor to be appended later.

Substituting \eqref{eq:p30-c2-7} into the pulled-back density \eqref{eq:p24-p16a}
and using \(T_t^\ell(a,b)=(T_A(a),u_B)\) gives
\begin{equation}
\begin{aligned}
    \widehat\pi_t(a,b)\dd b
    &=
    \frac{\widehat\nu_t^\sharp(T_A(a),u_B)}
    {\eta(T_A(a),u_B)}
    \widehat\rho_t(a,b)\dd b\\
    ...
    \frac{\widehat\rho_{t,A}(a)}
    {\eta_A(T_A(a))}
    \widehat\nu_t^\sharp(T_A(a),u_B)\dd u_B.
\end{aligned}
    \label{eq:p30-c2-8}
\end{equation}
The full three-block reference density cancels before the old-state variable
is integrated.  Integrating \eqref{eq:p30-c2-8} over \(u_B\) gives
\begin{equation}
    \widehat\pi_t(a)
    =
    \frac{\widehat\rho_{t,A}(a)}
    ...
    \label{eq:p30-c2-9}
\end{equation}
This is exactly step (c.1) followed by step (c.2): first compute the residual
marginal in \(u\)-space, then pull it back to the retained physical block using
the bridge marginal.

Returning from \(a=(x_t,\theta)\) notation to the paper's notation, integrating
over \(x_{t-1}\) gives
\begin{equation}
\begin{aligned}
 \widehat\pi_t(x_t,\theta\mid y_{1:t})
...
\begin{equation}
    ...
    \label{eq:p24-p17}
\end{equation}
Here the two-argument \(\eta\) in the denominator is the retained reference
marginal \(\eta_A(T_A(a))\), not the full three-block reference density
\(\eta(T_A(a),u_B)\).  This is the source formula in Algorithm 5(c.2) written
without relying on paper shorthand.  The implementation stores the two maps, their marginal
evaluators, and a retained evaluator for \(\widehat\pi_t(x_t,\theta)\).

\paragraph{Two-block sanity check.}
If the residual approximation is perfect in the sense that
\begin{equation}
    \widehat\nu_t^\sharp(u_A,u_B)
    =
    \frac{q_t(T_t^{-1}(u_A,u_B))}
    {\widehat\rho_t(T_t^{-1}(u_A,u_B))}
    \eta(u_A,u_B),
    \label{eq:p30-c2-10}
\end{equation}
then \eqref{eq:p24-p13} gives \(\widehat\pi_t(a,b)=q_t(a,b)\).  Formula
\eqref{eq:p30-c2-9} therefore reduces to
\begin{equation}
    \widehat\pi_t(a)
    =
    \int q_t(a,b)\dd b,
    \label{eq:p30-c2-11}
\end{equation}
which is ordinary marginalization.  This check does not prove numerical
accuracy, but it verifies the direction of the density ratio: a perfect
residual fit returns the exact retained marginal.
...
\[
\begin{array}{llll}
\text{Step} & \text{Evaluator signature} & \text{Quantities retained} & \text{Main diagnostic}\\
\hline
\text{b.1} &
...
\section{Fixed-Branch Likelihood Specialization}

The equations above explain Zhao and Cui \cite{zhao2024ttsequential}'s adaptive
sequential algorithm: tensor-train approximation, squared-TT nonnegativity,
conditional KR maps, particle correction, smoothing, and preconditioning.  The
fixed-branch likelihood specialization freezes approximation choices so that
an analytical derivative can differentiate the same scalar computed by the
forward pass.  Zhao and Cui \cite{zhao2024ttsequential}'s adaptive rank, pivot,
fitting, preconditioning, and branch choices should not be read as globally
differentiable.
...
\begin{equation}
    ...
    \label{eq:p24-r1}
\end{equation}
The fixed-branch likelihood specialization conditions on an external parameter value
...
\section{Fixed-Branch Objects And Array Shapes}
...
The implementable formulation used for the fixed-branch derivative is a fixed
least-squares formulation.  It is not adaptive TT-cross.  Adaptive TT-cross chooses
pivots, interpolation sets, and ranks from the target.  The construction here declares
the branch first, then uses deterministic ridge least squares to fit the TT
cores.  In symbols,
\begin{equation}
    \text{fixed least-squares derivative formulation}
    =
    \text{fixed points}
    +
    \text{fixed ranks}
    ...
    \label{eq:p25-ls-1}
\end{equation}
This narrower construction is sufficient to define a scalar and a derivative of that
same scalar.
...
static arrays during an alternating sweep.  They are recomputed from the
current cores at the moment a core is updated.  Let
\begin{equation}
    H_{j,k}=H_k(z_k^{(j)};C_k),
    \qquad
    \dot H_{j,k}=H_k(z_k^{(j)};\dot C_k),
    \label{eq:p26-sweep-recompute-1}
\end{equation}
where the basis values and fitting point are fixed and only the coefficients
change.  For a current core collection \(C^{\rm cur}\), define prefix and
suffix products by
\begin{equation}
    L_{j,1}^{\rm cur}=1,\qquad
    L_{j,k}^{\rm cur}=L_{j,k-1}^{\rm cur}H_{j,k-1}^{\rm cur},
    \qquad k=2,\ldots,D,
    \label{eq:p26-sweep-recompute-2}
\end{equation}
and
\begin{equation}
    R_{j,D}^{\rm cur}=1,\qquad
    R_{j,k}^{\rm cur}=H_{j,k+1}^{\rm cur}R_{j,k+1}^{\rm cur},
    \qquad k=D-1,\ldots,1.
    \label{eq:p26-sweep-recompute-3}
\end{equation}
The dotted prefix and suffix products are computed by the same recursions with
one product-rule term added:
\begin{equation}
    \dot L_{j,1}^{\rm cur}=0,\qquad
    \dot L_{j,k}^{\rm cur}
    =
    \dot L_{j,k-1}^{\rm cur}H_{j,k-1}^{\rm cur}
    +
    L_{j,k-1}^{\rm cur}\dot H_{j,k-1}^{\rm cur},
    \label{eq:p26-sweep-recompute-4}
\end{equation}
and
\begin{equation}
    \dot R_{j,D}^{\rm cur}=0,\qquad
    \dot R_{j,k}^{\rm cur}
    =
    \dot H_{j,k+1}^{\rm cur}R_{j,k+1}^{\rm cur}
    +
    H_{j,k+1}^{\rm cur}\dot R_{j,k+1}^{\rm cur}.
    \label{eq:p26-sweep-recompute-5}
\end{equation}

At a core update \(k\), the design matrix and normal equations are built from
the current environments:
\begin{equation}
    A_k^{\rm cur}
    =
    A_k(L_{\cdot,k}^{\rm cur},R_{\cdot,k}^{\rm cur}),
    \qquad
    N_k^{\rm cur}
    =
    (A_k^{\rm cur})^\top W A_k^{\rm cur}+\rho I,
    \qquad
    d_k^{\rm cur}=(A_k^{\rm cur})^\top W y.
    \label{eq:p26-sweep-recompute-6}
\end{equation}
Solve \(N_k^{\rm cur}g_k^{\rm new}=d_k^{\rm cur}\) and replace only the
current core:
\begin{equation}
    C_k^{\rm cur}\leftarrow C_k^{\rm new},
    \qquad
    C_\ell^{\rm cur}\ \text{unchanged for }\ell\ne k.
    \label{eq:p26-sweep-recompute-7}
\end{equation}
...
\begin{aligned}
    \dot A_k^{\rm cur}
    &=
    \dot A_k(\dot L_{\cdot,k}^{\rm cur},L_{\cdot,k}^{\rm cur},
             \dot R_{\cdot,k}^{\rm cur},R_{\cdot,k}^{\rm cur}),\\
    \dot N_k^{\rm cur}
    &=
    (\dot A_k^{\rm cur})^\top W A_k^{\rm cur}
    +
    (A_k^{\rm cur})^\top W\dot A_k^{\rm cur},\\
    \dot d_k^{\rm cur}
    &=
    (\dot A_k^{\rm cur})^\top W y
    +
    (A_k^{\rm cur})^\top W\dot y.
\end{aligned}
    \label{eq:p26-sweep-recompute-8}
\end{equation}
The derivative core is updated by
\begin{equation}
    N_k^{\rm cur}\dot g_k^{\rm new}
    =
    \dot d_k^{\rm cur}-\dot N_k^{\rm cur}g_k^{\rm new},
    \qquad
    \dot C_k^{\rm cur}\leftarrow \dot C_k^{\rm new}.
    \label{eq:p26-sweep-recompute-9}
\end{equation}

The next environment update depends on sweep direction.  In a left-to-right
pass, after replacing \(C_k\), the next core is \(k+1\), so
\begin{equation}
    L_{j,k+1}^{\rm cur}
    =
    L_{j,k}^{\rm cur}H_{j,k}^{\rm new},
    \qquad
    \dot L_{j,k+1}^{\rm cur}
    =
    \dot L_{j,k}^{\rm cur}H_{j,k}^{\rm new}
    +
    L_{j,k}^{\rm cur}\dot H_{j,k}^{\rm new}.
    \label{eq:p26-sweep-recompute-10}
\end{equation}
The suffix \(R_{j,k+1}^{\rm cur}\) is already valid because it involves only
cores \(k+2,\ldots,D\).  In a right-to-left pass, after replacing \(C_k\), the
next core is \(k-1\), so
\begin{equation}
    R_{j,k-1}^{\rm cur}
    =
    H_{j,k}^{\rm new}R_{j,k}^{\rm cur},
    \qquad
    \dot R_{j,k-1}^{\rm cur}
    =
    \dot H_{j,k}^{\rm new}R_{j,k}^{\rm cur}
    +
    H_{j,k}^{\rm new}\dot R_{j,k}^{\rm cur}.
    \label{eq:p26-sweep-recompute-11}
\end{equation}
The prefix \(L_{j,k-1}^{\rm cur}\) is already valid because it involves only
cores \(1,\ldots,k-2\).  If an implementation caches all prefixes and suffixes
for all \(k\), then after updating \(C_k\) every cached prefix with index
\(\ell>k\) and every cached suffix with index \(\ell<k\) is stale:
\begin{equation}
    C_k\ \text{updated}
    \quad\Longrightarrow\quad
    \{L_{j,\ell}:\ell>k\}\ \text{and}\ \{R_{j,\ell}:\ell<k\}
    \text{ must be recomputed before use}.
    \label{eq:p26-sweep-recompute-12}
\end{equation}
This is the concrete bookkeeping behind the statement that \(A_k\), \(N_k\),
and \(d_k\) are recomputed during each alternating sweep.
...
This section gives a single fixed-branch construction.  It is intentionally narrower
than Zhao and Cui \cite{zhao2024ttsequential}'s adaptive code.  Its purpose is to remove ambiguity.  A coder
can implement this construction first, then replace pieces with adaptive or higher
...
\midrule
\lambda_t(z) & 2^{-D}\ind\{z\in[-1,1]^D\} &
    \text{defensive reference density in reference coordinates}\\
\tau_t & 2^D\epsilon_{\rm floor} &
    \text{defensive mass giving floor }\tau_t\lambda_t=\epsilon_{\rm floor}\\
c_t & -\max_j\log \widetilde q_{t,j}^{\rm floor}(\beta_0) &
    \text{base-parameter square-root scaling shift}\\
...
\rho_t & \epsilon_{\rm ridge}\operatorname{trace}(A^\top W A)/\dim(g) &
    \text{ridge used in each local normal equation}\\
\delta_\tau & 10^{-2} &
...
\epsilon_{\rm fit} & 10^{-4} &
    \text{relative weighted square-root fit-residual tolerance}\\
\epsilon_{\rm sweep} & 10^{-5} &
    \text{relative core-change tolerance across sweeps}\\
\epsilon_Z & 10^{-8} &
    \text{relative normalizer cross-check tolerance}\\
\epsilon_{\rm norm} & 10^{-8} &
    \text{retained-filter normalization tolerance}\\
\epsilon_Q,\epsilon_{\dot Q} & 10^{-6},10^{-5} &
    \text{compressed retained-object validation tolerances}\\
...
H_{\rm FD} & \{10^{-2},10^{-3},10^{-4},10^{-5}\} &
    \text{finite-difference step schedule}\\
\bottomrule
\end{array}
...
objects, not executable code.  The dependency flow is
\begin{equation}
\begin{array}{rcl}
...
The fixed-branch filtering variant chooses all discrete approximation objects
...
The following boxed algorithm gathers the complete mathematical recurrence in one
...
\subsection{Boxed Algorithm: Fixed-Branch Filter And Derivative Together}

For implementation work, the forward scalar and its derivative should be read
as one coupled mathematical recurrence.  The branch is fixed first:
\begin{equation}
    B
    =
    \{\Psi_t,T_t,b_{t,k},Z_{{\rm fit},t},W_t,R_{t,k},S_t,
    \rho_{t,k},c_t,\tau_t,\lambda_t,\text{sweep order}\}_{t,k}.
    \label{eq:p26-box-deriv-1}
\end{equation}
For a parameter component \(\beta_i\), the coupled algorithm is
\begin{equation}
\boxed{
\begin{array}{p{0.94\linewidth}}
\textbf{Inputs.}
\(\beta\), model density evaluators
\((p_\beta,f_\beta,g_\beta)\), derivative evaluators
\((\partial_i p_\beta,\partial_i f_\beta,\partial_i g_\beta)\), observations
\(y_{1:T}\), and fixed branch \(B\).
\\[1mm]
\textbf{Initial retained object.}
Set \(P_0\) to the initial retained density representation and set
\(\dot P_0=\partial_i P_0\).  If the initial law is independent of
\(\beta_i\), then \(\dot P_0=0\).
\\[1mm]
\textbf{For \(t=1,\ldots,T\), target values.}
At each fitting point \(z^{(j)}\), evaluate
\[
  \widetilde q_{t,j}
  =
  P_{t-1}(z_{t-1}^{(j)})
  f_\beta(x_t^{(j)}\mid x_{t-1}^{(j)})
  g_\beta(y_t\mid x_t^{(j)})J_t(z^{(j)}),
\]
and
\[
  \dot{\widetilde q}_{t,j}
  =
  \dot P_{t-1} f_\beta g_\beta J_t
  +P_{t-1}(\partial_i f_\beta)g_\beta J_t
  +P_{t-1}f_\beta(\partial_i g_\beta)J_t
  +P_{t-1}f_\beta g_\beta \dot J_t .
\]
In the fixed-domain specialization, \(\dot J_t=0\).
\\[1mm]
\textbf{Square-root values.}
With the frozen shift and floor,
\[
  y_j=e^{c_t/2}\sqrt{\max(\widetilde q_{t,j},\epsilon_{\rm floor})},
  \qquad
  \dot y_j=
  \frac{e^{c_t/2}\dot{\widetilde q}_{t,j}}
       {2\sqrt{\widetilde q_{t,j}}}
\]
when the value is not floored; if the branch declares a nonsmooth floor hit,
the derivative check is failed rather than silently differentiated through
\(\max\).
\\[1mm]
\textbf{Fixed sweeps.}
For each declared core update, recompute \(L_{j,k},R_{j,k},A_k,N_k,d_k\),
solve \(N_kg_k=d_k\), reshape \(g_k\mapsto C_{t,k}\), then recompute the
dotted environments and solve
\[
    N_k\dot g_k=\dot d_k-\dot N_kg_k.
\]
The recomputation order is the one in
\eqref{eq:p26-sweep-recompute-1}--\eqref{eq:p26-sweep-recompute-12};
in particular, cached prefixes and suffixes on the wrong side of an updated
core are stale until recomputed.
\\[1mm]
\textbf{Mass and normalizer.}
Contract the fitted cores to obtain \(R_t=\int\phi_t^2\dd z\) and
\[
    \dot R_t=\int 2\phi_t\dot\phi_t\dd z,\qquad
    \widehat Z_t=e^{-c_t}R_t+\tau_t,\qquad
    \dot{\widehat Z}_t=e^{-c_t}\dot R_t.
\]
\\[1mm]
\textbf{Retained filter.}
Compute retained numerator matrices or compressed retained evaluators
\((Q_t,\dot Q_t)\), normalize
\[
    P_t=Q_t/\widehat Z_t,\qquad
    \dot P_t=\dot Q_t/\widehat Z_t-Q_t\dot{\widehat Z}_t/\widehat Z_t^2,
\]
and save the evaluator pair needed by time \(t+1\).
\\[1mm]
\textbf{Scalar update.}
\[
    \widehatell_t=\widehatell_{t-1}+\log\widehat Z_t,\qquad
    \dot{\widehatell}_t=\dot{\widehatell}_{t-1}
    +\dot{\widehat Z}_t/\widehat Z_t.
\]
\\[1mm]
\textbf{Outputs.}
\(\widehatell_T(\beta;B)\), \(\partial_i\widehatell_T(\beta;B)\),
retained evaluator pairs \((P_t,\dot P_t)\), fitted cores
\((C_{t,k},\dot C_{t,k})\), normalizers, diagnostics, and failure status.
\\[1mm]
\textbf{Invariants and exits.}
The branch \(B\) is unchanged; every retained filter integrates to one; every
normalizer is finite and positive; every derivative solve uses the same
forward normal matrix.  Exit with failure if floor hits are nonsmooth, ranks do
not pass the declared ladder, conditioned solves exceed \(\kappa_{\max}\),
defensive mass dominates, retained compression breaks normalization, or
finite differences do not use the same branch.
\end{array}}
\label{eq:p26-box-deriv-2}
\end{equation}
...
fixed-branch likelihood setup.  The next part expands the derivative path in
...
evaluates an approximate likelihood.  These are different roles:
\begin{equation}
...
\subsection*{Retained-Filter Storage When The Product Basis Is Too Large}

The full retained matrix has
\begin{equation}
    \#Q_t=p_{\rm ret}^2
    =
    \left(\prod_{j=1}^{m}p_j\right)^2
    =
    \prod_{j=1}^{m}p_j^2
    \label{eq:p26-retained-storage-1}
\end{equation}
entries.  If \(m=10\) and \(p_j=4\), then
\begin{equation}
    p_{\rm ret}=4^{10}=1{,}048{,}576,
    \qquad
    p_{\rm ret}^2\approx 1.1\times10^{12},
    \label{eq:p26-retained-storage-2}
\end{equation}
so full matrix storage is no longer a mathematical implementation plan.  The
retained object should then be stored as an evaluator, not necessarily as a
dense matrix.

One exact evaluator form is the marginal TT itself.  Let the retained
coordinates be \(z_{t,1:m}\).  Store cores \(G_{t,j}\) representing the
retained numerator:
\begin{equation}
    a_t(z_t)
    =
    G_{t,1}(z_{t,1})G_{t,2}(z_{t,2})\cdots G_{t,m}(z_{t,m}),
    \label{eq:p26-retained-storage-3}
\end{equation}
...
\begin{equation}
    ...
    \sum_{r=1}^{m}
    G_{t,1}(z_{t,1})\cdots
    \dot G_{t,r}(z_{t,r})\cdots
    G_{t,m}(z_{t,m}).
    \label{eq:p26-retained-storage-4}
\end{equation}
The normalized retained evaluator is then
\begin{equation}
    \widehat p_t(z_t)=a_t(z_t)/\widehat Z_t,
    \qquad
    \dot{\widehat p}_t(z_t)
    =
    \dot a_t(z_t)/\widehat Z_t
    -
    a_t(z_t)\dot{\widehat Z}_t/\widehat Z_t^2.
    \label{eq:p26-retained-storage-5}
\end{equation}

A second option is a low-rank quadratic basis representation:
\begin{equation}
    Q_t\approx U_t S_t U_t^\top,
    \qquad
    U_t\in\R^{p_{\rm ret}\times r_Q},
    \qquad
    r_Q\ll p_{\rm ret}.
    \label{eq:p26-retained-storage-6}
\end{equation}
Then
\begin{equation}
    a_t(z_t)
    \approx
    \left(U_t^\top b_t(z_t)\right)^\top
    S_t
    \left(U_t^\top b_t(z_t)\right),
    \label{eq:p26-retained-storage-7}
\end{equation}
...
\begin{aligned}
    \dot a_t(z_t)
    &\approx
    (\dot U_t^\top b_t)^\top S_t(U_t^\top b_t)
    +(U_t^\top b_t)^\top \dot S_t(U_t^\top b_t)\\
    &\quad+
    (U_t^\top b_t)^\top S_t(\dot U_t^\top b_t).
\end{aligned}
    \label{eq:p26-retained-storage-8}
\end{equation}
This compression is acceptable only if it is declared as part of the fixed
branch and the compression residual is tested:
\begin{equation}
    \varepsilon_Q
    =
    \frac{\|Q_t-U_tS_tU_t^\top\|_{\mathcal V}}
         {\max\{\|Q_t\|_{\mathcal V},\epsilon_{\rm abs}\}}
    \le \epsilon_Q,
    \label{eq:p26-retained-storage-9}
\end{equation}
where \(\|\cdot\|_{\mathcal V}\) may be evaluated on a fixed validation grid
or fixed quadrature rule.  The derivative compression is tested analogously:
\begin{equation}
    \varepsilon_{\dot Q}
    =
    \frac{\|\dot Q_t-\dot Q_t^{\rm comp}\|_{\mathcal V}}
         {\max\{\|\dot Q_t\|_{\mathcal V},\epsilon_{\rm abs}\}}
    \le \epsilon_{\dot Q}.
    \label{eq:p26-retained-storage-10}
\end{equation}
Thus the retained-filter contract is not ``store a dense matrix.''  It is
\begin{equation}
    \mathcal E_t
    =
    \{z_t\mapsto \widehat p_t(z_t),\
      z_t\mapsto \dot{\widehat p}_t(z_t),\
      \int\widehat p_t(z_t)\dd z_t=1,\
      \int\dot{\widehat p}_t(z_t)\dd z_t=0\}.
    \label{eq:p26-retained-storage-11}
\end{equation}
Any dense, TT, sparse, or low-rank storage format is admissible only if it
implements \(\mathcal E_t\) and passes the normalization and derivative-mass
checks in \eqref{eq:p26-retained-storage-11}.

The compressed retained object has its own failure diagnostics:
\begin{equation}
\begin{array}{ll}
\toprule
\text{diagnostic} & \text{failure test}\\
\midrule
\text{normalization drift} &
\left|\int \widehat p_t(z_t)\dd z_t-1\right|>\epsilon_{\rm norm}\\
\text{derivative-mass drift} &
\left|\int \dot{\widehat p}_t(z_t)\dd z_t\right|>\epsilon_{\rm norm}\\
\text{value compression residual} &
\varepsilon_Q>\epsilon_Q\\
\text{derivative compression residual} &
\varepsilon_{\dot Q}>\epsilon_{\dot Q}\\
\text{query mismatch} &
\max_j|\widehat p_t^{\rm comp}(z_j)-\widehat p_t^{\rm ref}(z_j)|>\epsilon_Q\\
\bottomrule
\end{array}
    \label{eq:p26-retained-storage-12}
\end{equation}
Normalization drift means the retained density no longer integrates to one.
Derivative-mass drift means the quotient derivative or compression is
inconsistent.  Compression residuals mean the value or derivative evaluator is
not accurate enough.  Query mismatch means the next-step target would use the
wrong retained factor.  The query mismatch test uses a fixed validation table
of retained-coordinate points.  If no dense reference is available, compare
two independently built evaluators or a higher-rank retained representation on
the same fixed table.
...
\section{The Story Of The Fixed-Branch Derivative}

Before the formal proposition, it is useful to separate the idea from the
notation.  The adaptive Zhao and Cui method chooses approximation structure as
it runs \cite{zhao2024ttsequential}.  A rank may change, a pivot may change, or
an interpolation set may change.  Such choices are excellent for approximation
quality, but they are discrete choices.  A discrete choice can jump when
\(\beta\) is perturbed:
\begin{equation}
    B_{\rm adaptive}(\beta_0-h)
    \ne
    B_{\rm adaptive}(\beta_0)
    \ne
    B_{\rm adaptive}(\beta_0+h).
    \label{eq:p26-deriv-story-1}
\end{equation}
Across such jumps, there is no single smooth scalar being differentiated.  The
fixed-branch construction deliberately chooses a narrower scalar:
\begin{equation}
    \widehatell_T(\beta;B)
    =
    \sum_{t=1}^{T}\log\widehat Z_t(\beta;B),
    \label{eq:p26-deriv-story-2}
\end{equation}
where
\begin{equation}
    B(\beta_0-h)=B(\beta_0)=B(\beta_0+h)=B.
    \label{eq:p26-deriv-story-3}
\end{equation}
This is why the derivative is useful but narrower.  It does not claim to
differentiate the adaptive rank-changing algorithm.  It differentiates the
declared approximate likelihood obtained after the branch has been fixed.
...
\begin{equation}
    ...
    y_j(\beta)
    \mapsto
    (A_k(\beta),N_k(\beta),d_k(\beta))
    \mapsto
    C_{t,k}(\beta)
    \mapsto
    \widehat Z_t(\beta)
    \mapsto
    \widehat p_t(\cdot;\beta).
    \label{eq:p26-deriv-story-4}
\end{equation}
The derivative pass follows the same chain with the product rule, chain rule,
linear-solve derivative, and quotient rule:
\begin{equation}
    \dot{\widetilde q}_{t,j}
    \mapsto
    \dot y_j
    \mapsto
    (\dot A_k,\dot N_k,\dot d_k)
    \mapsto
    \dot C_{t,k}
    \mapsto
    \dot{\widehat Z}_t
    \mapsto
    \dot{\widehat p}_t.
    \label{eq:p26-deriv-story-5}
\end{equation}
The only difficult-looking step is the fitted core derivative.  But each core
update is just a fixed linear solve:
\begin{equation}
    N_k(\beta)g_k(\beta)=d_k(\beta).
    \label{eq:p26-deriv-story-6}
\end{equation}
Differentiating the left side gives
\begin{equation}
    \dot N_k g_k+N_k\dot g_k=\dot d_k,
    \label{eq:p26-deriv-story-7}
\end{equation}
and therefore
\begin{equation}
    N_k\dot g_k=\dot d_k-\dot N_kg_k.
    \label{eq:p26-deriv-story-8}
\end{equation}
After this, the normalizer derivative is only
\begin{equation}
    \widehat Z_t=e^{-c_t}\int\phi_t^2\dd z+\tau_t,
    \qquad
    \dot{\widehat Z}_t=e^{-c_t}\int2\phi_t\dot\phi_t\dd z,
    \label{eq:p26-deriv-story-9}
\end{equation}
...
\begin{equation}
    ...
    \dot a_t/\widehat Z_t
    -
    a_t\dot{\widehat Z}_t/\widehat Z_t^2.
    \label{eq:p26-deriv-story-10}
\end{equation}

Thus Proposition 2 has a simple message.  If the branch is held fixed and all
objects are recomputed by the same declared equations, then the dotted
recursion is the derivative of the same scalar produced by the forward
recursion:
\begin{equation}
    \text{forward scalar } \widehatell_T(\beta;B)
    \quad\Longleftrightarrow\quad
    \text{dotted scalar } \partial_\beta\widehatell_T(\beta;B).
    \label{eq:p26-deriv-story-11}
\end{equation}
The proposition is not a claim that the approximate scalar equals the exact
Bayesian evidence, not a claim that the adaptive algorithm is globally smooth,
and not a claim that the numerical linear algebra is stable.  Those are
separate approximation and diagnostics questions.
...
The same constant-basis calculation now gives a second approximate
normalizer:
\begin{equation}
    \phi_2(z_2,z_1)=\frac{C_2}{2},
    \qquad
    C_2=2(1.417)\approx2.834,
    \label{eq:p26-trace-seq-1}
\end{equation}
so
\begin{equation}
    \widehat Z_2
    =
    \int_{[-1,1]^2}\phi_2(z_2,z_1)^2\dd z_2\dd z_1+\tau_2
    \approx
    C_2^2+\tau_2
    \approx
    8.032+\tau_2.
    \label{eq:p26-trace-seq-2}
\end{equation}
...
\begin{equation}
    ...
    \approx
    \log(2.784001)+\log(8.032001)
    \approx
    3.108.
    \label{eq:p26-trace-seq-3}
\end{equation}
The retained filter at time two is again uniform under the intentionally crude
constant basis:
\begin{equation}
    \widehat p_2(z_2)\approx \frac12.
    \label{eq:p26-trace-seq-4}
\end{equation}

The sequential bookkeeping can be summarized as a chain of quantities:
\begin{equation}
\begin{array}{c|c|c|c|c|c}
t & \text{carried input} & \widetilde q_t(0,0)
& y_{\rm fit,t}(0,0) & \widehat Z_t & \widehatell_t\\
\hline
1 & p_0(x_0=0)=0.3989 & 0.6960 & 0.8343 & 2.7840 & 1.024\\
2 & \widehat p_1(z_1=0)=0.5 & 2.007 & 1.417 & 8.0320 & 3.108
\end{array}
    \label{eq:p26-trace-seq-5}
\end{equation}
This table is small, but it displays the actual filtering loop: the normalized
retained object from time one is not a side output; it is the first factor in
the time-two target.
...
Using the same frozen branch, the numerical comparison is displayed in
\eqref{eq:p26-trace-fd-table-1}--\eqref{eq:p26-trace-fd-table-2} after the
two-point basis trace, where \(D(h)\) is compared directly to the analytical
gradient \(G\).
...
For a synthetic fixed-branch check of the two-step scalar, suppose the
analytical derivative at \(\beta_0=0.7\) is
\begin{equation}
    G=\partial_\beta\widehatell_2(\beta_0;B)=-0.384721.
    \label{eq:p26-trace-fd-table-1}
\end{equation}
...
h & D(h) & G & |D(h)-G|\\
\hline
10^{-2} & -0.384102 & -0.384721 & 6.19\times10^{-4}\\
10^{-3} & -0.384715 & -0.384721 & 6.00\times10^{-6}\\
10^{-4} & -0.384721 & -0.384721 & 7.20\times10^{-8}\\
10^{-5} & -0.384720 & -0.384721 & 9.50\times10^{-7}
\end{array}
    \label{eq:p26-trace-fd-table-2}
\end{equation}
The expected pattern is visible: the error decreases as \(h\) moves from
\(10^{-2}\) to \(10^{-4}\), then roundoff and fitting sensitivity begin to
dominate at \(10^{-5}\).  The table would support the claim that the coded
derivative matches the declared fixed scalar.  It would not support the claim
that the fixed scalar is an accurate approximation to the exact evidence.

\section{What Must Be Held Fixed For The Derivative}

The finite-difference diagnostic needs a precise meaning for "the same
branch."  The fixed-field record is
\begin{equation}
    \mathcal H(B)
    =
    \{\mathcal H_{\rm fixed},\mathcal H_{\rm diff},\mathcal H_{\rm diag}\}.
...
\begin{equation}
    B(\beta_a)=B(\beta_b)
    \quad\Longleftrightarrow\quad
    \mathcal H_{\rm fixed}(\beta_a)=\mathcal H_{\rm fixed}(\beta_b).
...
\begin{equation}
    \mathcal H_{\rm fixed}(\beta_0-h)
    =
    \mathcal H_{\rm fixed}(\beta_0)
    =
    \mathcal H_{\rm fixed}(\beta_0+h),
...
\begin{equation}
    \mathcal H_{\rm diff}(\beta_0-h),
    \quad
    \mathcal H_{\rm diff}(\beta_0),
    \quad
    \mathcal H_{\rm diff}(\beta_0+h)
    \quad
    \text{may differ.}
...
\end{equation}
It must also contain the fixed-branch record used for \(\beta_0\),
\(\beta_0+h\), and \(\beta_0-h\).  The mathematical pass condition is that
those fixed-branch records are identical and the derivative error has a stable
decreasing window before numerical roundoff dominates.

\section{Large-Scale Validation Models And Test Protocol}

The derivation above explains what the squared-TT filter computes and what the
fixed-branch derivative differentiates.  A large-scale state-space model also
needs empirical validation: the approximation must be accurate enough, the
memory growth must be controlled, the runtime must be credible, and the
diagnostics must reveal failure before the posterior summaries are trusted.
This section specifies such a validation protocol; it does not report
BayesFilter benchmark outcomes.
The natural backbone for this validation is the numerical model suite used by
Zhao and Cui \cite{zhao2024ttsequential}.  Those examples form a diverse
benchmark suite: an exact-reference linear Gaussian model, a long-horizon
nonlinear stochastic-volatility model, a moderate-dimensional spatial epidemic
model, and a nonlinear dynamical system where preconditioning is essential.
The large-scale BayesFilter claim would come only after the additional
dimension, horizon, rank, and memory stress ladders below have also been run.

The validation ladder should be read from bottom to top:
\begin{equation}
\begin{array}{ccl}
\mathcal V_0&=&\text{algebraic unit identities},\\
\mathcal V_1&=&\text{small exact state-space models},\\
\mathcal V_2&=&\text{Zhao--Cui model-suite reproduction},\\
\mathcal V_3&=&\text{dimension, horizon, rank, and basis stress ladders},\\
\mathcal V_4&=&\text{robustness and failure-mode tests},\\
\mathcal V_5&=&\text{fixed-branch derivative finite-difference tests}.
\end{array}
\label{eq:p27-v1}
\end{equation}
The order matters.  A large epidemic run cannot rescue a failed mass-matrix
identity; a good finite-difference derivative cannot prove posterior accuracy;
and an accurate small Kalman run does not prove scalability.  The validation
claim at level \(\mathcal V_j\) is conditional on the lower levels being
healthy:
\begin{equation}
    \text{credible at level }\mathcal V_j
    \quad\Longrightarrow\quad
    \bigcap_{i<j}\{\mathcal V_i\text{ has no veto failure}\}.
    \label{eq:p27-v2}
\end{equation}

\subsection{What The Tests Must Measure}

For a fixed time horizon \(T\), state dimension \(m\), parameter dimension
\(d\), basis size \(p\), maximum rank \(R\), and number of alternating sweeps
\(S_{\rm ALS}\), the implementation should record the mathematical quantities
that control the computation:
\begin{equation}
\begin{array}{ll}
\text{target evaluations} &
    N_{\rm eval}
    =
    \sum_{t=1}^T\sum_{k=1}^{D_t} n_{t,k}^{\rm fit},\\[2mm]
\text{core storage} &
    B_{\rm core}
    =
    \sum_{t=1}^T\sum_{k=1}^{D_t}
    r_{t,k-1}\,p_{t,k}\,r_{t,k},\\[2mm]
\text{derivative-core storage} &
    B_{\dot{\rm core}}
    =
    \sum_{t=1}^T\sum_{k=1}^{D_t}
    r_{t,k-1}\,p_{t,k}\,r_{t,k},\\[2mm]
\text{largest normal equation} &
    n_{\rm solve,max}
    =
    \max_{t,k} r_{t,k-1}p_{t,k}r_{t,k},\\[2mm]
\text{mass-contraction count} &
    N_{\rm mass}
    =
    \sum_{t=1}^T D_t,\\[2mm]
\text{KR inversion count} &
    N_{\rm KR}
    =
    \sum_{t=1}^T N_{\rm samp,t}D_t.
\end{array}
\label{eq:p27-v3}
\end{equation}
Here \(D_t\) is the dimension of the density fitted at time \(t\).  For the
unpreconditioned adjacent-state target \(q_t(x_t,\theta,x_{t-1})\),
\[
    D_t=2m+d,
\]
while after a retained-filter specialization without static parameter
integration, a smaller \(D_t\) may be used.  The quantities in
\eqref{eq:p27-v3} are not merely software counters.  They correspond to the
places where the mathematical approximation can fail: too many target
evaluations indicate an unaffordable design, large cores indicate rank growth,
large normal equations indicate conditioning risk, and many KR inversions
amplify monotonicity and quadrature errors.

The peak memory should be reported in both array units and bytes.  If one
floating-point scalar occupies \(b_{\rm fp}\) bytes, then a first-order memory
model is
\begin{equation}
    M_{\rm peak}
    \approx
    ...
    \label{eq:p27-v4}
\end{equation}
where the terms denote live forward cores, live derivative cores, sweep
environments, current fitting matrices, and retained-filter representations.
For a fixed-branch derivative run the derivative storage must be counted
separately:
\begin{equation}
    M_{\rm peak}^{\rm value+grad}
    -
    M_{\rm peak}^{\rm value}
    \approx
    b_{\rm fp}
    \max_t
    \left[
    B_{\dot{\rm live}}(t)+B_{\dot{\rm env}}(t)+B_{\dot{\rm retained}}(t)
    \right].
    \label{eq:p27-v5}
\end{equation}
This difference is the memory price of the analytical derivative.

Runtime should be split into mathematically meaningful components:
\begin{equation}
    T_{\rm wall}
    =
    T_{\rm target}
    ...
    \label{eq:p27-v6}
\end{equation}
The same total time can be interpreted very differently depending on which term
dominates.  If \(T_{\rm target}\) dominates, the model likelihood or ODE solve
is the bottleneck.  If \(T_{\rm fit}\) dominates, the rank/basis/sweep protocol
is the bottleneck.  If \(T_{\rm KR}\) dominates, smoothing or path sampling is
the bottleneck.  If \(T_{\rm retained}\) dominates, the carried-filter storage
format is probably too expensive.

\subsection{Accuracy Metrics With Exact Or Reference Targets}

The first accuracy metric should be used only when an exact or high-resolution
reference density is available.  If \(p_{\rm ref}\) and \(\widehat p\) are
normalized densities on the same coordinate system, use the Hellinger distance
\begin{equation}
    D_H(p_{\rm ref},\widehat p)
    =
    \left[
    \frac12
    \int
...
\begin{equation}
    ...
    \int
    \left|
    \widehat\pi(u)-\pi_{\rm ref}(u)
    \right|\dd u}
    {\int |\pi_{\rm ref}(u)|\dd u}.
    \label{eq:p27-v8}
\end{equation}
For posterior means, variances, and trajectory summaries, use normalized
moment discrepancies:
\begin{equation}
    E_{\rm mean}
    =
    \frac{
    \|\widehat\mu-\mu_{\rm ref}\|_2}
    {1+\|\mu_{\rm ref}\|_2},
    \qquad
    E_{\rm var}
    =
    \frac{
    \|\diag(\widehat\Sigma)-\diag(\Sigma_{\rm ref})\|_2}
    {1+\|\diag(\Sigma_{\rm ref})\|_2}.
    \label{eq:p27-v9}
\end{equation}
For hidden trajectories in synthetic data, report the root mean squared error
of the posterior median \(\widehat x_{t,0.5}\):
\begin{equation}
    {\rm RMSE}_{\rm med}
    =
    \left[
    \frac1{T+1}
    \sum_{t=0}^T
    \|x_t^{\rm true}-\widehat x_{t,0.5}\|_2^2
    \right]^{1/2},
    \label{eq:p27-v10}
\end{equation}
and empirical credible-band coverage:
\begin{equation}
    {\rm Cov}_{90}
    =
    \frac1{T+1}
    ...
    \ind\{
    x_t^{\rm true}\in
    [\widehat x_{t,0.05},\widehat x_{t,0.95}]
    \}.
    \label{eq:p27-v11}
\end{equation}
For vector states, \eqref{eq:p27-v11} should be reported coordinatewise and
through a simultaneous ellipsoid or box event; coordinatewise coverage alone
can hide joint undercoverage.

When importance-weighted path samples are produced from the approximate
transport, report the normalized effective sample size
\begin{equation}
    {\rm ESS}
    =
    ...
    \label{eq:p27-v12}
\end{equation}
where \(w_i\) are the correction weights.  Zhao and Cui report ESS quantiles
over repeated experiments \cite{zhao2024ttsequential}.  The same convention is
useful here:
\begin{equation}
    Q_{\rm ESS}(q,t)
    =
    ...
    \left(
    {\rm ESS}^{(1)}_t,\ldots,{\rm ESS}^{(B)}_t
    \right),
    \qquad
    q\in\{0.25,0.50,0.75\}.
    \label{eq:p27-v13}
\end{equation}
ESS is a diagnostic for the proposal and correction weights, not an exact
posterior-distance theorem.  A high ESS with a biased approximate target is
still possible, and a low ESS can occur even when the fitted density has useful
marginals.

\subsection{The Exact-Reference Linear Gaussian Benchmark}

The first large-scale validation model should be the linear Gaussian benchmark
used by Zhao and Cui \cite{zhao2024ttsequential}.  The state and observation
equations are
\begin{equation}
    X_t-\mu{\bf 1}
    =
    b(X_{t-1}-\mu{\bf 1})+a\epsilon_t^{(x)},
    \qquad
    \epsilon_t^{(x)}\sim N(0,I_m),
    \label{eq:p27-lg1}
\end{equation}
\begin{equation}
    Y_t
    ...
    C X_t+d\epsilon_t^{(y)},
    \qquad
    \epsilon_t^{(y)}\sim N(0,I_n),
    \label{eq:p27-lg2}
\end{equation}
with independent state and observation noises.  The initial state prior is
\begin{equation}
    X_0\mid\mu\sim N(\mu{\bf 1},I_m),
    \label{eq:p27-lg3}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:p27-lg4}
\end{equation}
In the paper's numerical instance \(m=n=3\), \(T=50\), \(\mu=0\), and
\(a^2+b^2=1\), so the estimated parameter is effectively
\begin{equation}
    \theta_{\rm est}=(a,d)\in[0.4,1]^2.
    \label{eq:p27-lg5}
\end{equation}

For a fixed \(\theta\), the Kalman recursion gives the exact conditional
Gaussian filtering law
\begin{equation}
    X_t\mid \theta,y_{1:t}
    \sim
    N(m_t(\theta),P_t(\theta)).
    \label{eq:p27-lg6}
\end{equation}
The one-step predictive density is also Gaussian:
\begin{equation}
    Y_t\mid \theta,y_{1:t-1}
    \sim
    N\!\left(
    C m_{t|t-1}(\theta),
    C P_{t|t-1}(\theta)C^\top+d^2 I_n
    \right).
    \label{eq:p27-lg7}
\end{equation}
Therefore the exact parameter posterior is available up to normalization:
\begin{equation}
    p(\theta\mid y_{1:t})
    \propto
    p(\theta)
    \prod_{j=1}^t
    p(y_j\mid \theta,y_{1:j-1}).
    \label{eq:p27-lg8}
\end{equation}
This is the most important accuracy benchmark because it tests the TT
approximation against a non-Monte-Carlo reference.  The required comparison is
\begin{equation}
\begin{array}{ll}
\text{density accuracy} &
    D_H(p_{\rm Kalman}(\theta\mid y_{1:t}),
        \widehat p_{\rm TT}(\theta\mid y_{1:t})),\\[1mm]
\text{relative density error} &
    E_{L^1,t}
    =
    \|\widehat\pi_t-\pi_{\rm Kalman,t}\|_1/
    \|\pi_{\rm Kalman,t}\|_1,\\[1mm]
\text{evidence error} &
    E_{Z,t}
    =
    |\log\widehat Z_{1:t}-\log Z_{1:t}^{\rm Kalman}|,\\[1mm]
\text{sampling quality} &
    Q_{\rm ESS}(q,t),\quad q\in\{0.25,0.50,0.75\}.
\end{array}
\label{eq:p27-lg9}
\end{equation}
The rank and basis ladder should include the Zhao--Cui choices:
\begin{equation}
    \ell\in\{17,25,33,41,49\},
    \qquad
    R\in\{10,15,20,25,30\},
    \label{eq:p27-lg10}
\end{equation}
with the piecewise Lagrange basis on four subintervals and repeated ALS sweeps
as in their experiments \cite{zhao2024ttsequential}.  For BayesFilter, add a
gradient column for the fixed-branch scalar:
Here \(\beta\) denotes the scalar coordinate selected for fixed-branch
differentiation and
\begin{equation}
    \widehatell_T(\beta;B)
    =
...
\begin{equation}
    ...
    =
    \frac{
    \widehatell_T(\beta+h;B)-\widehatell_T(\beta-h;B)}
    {2h}.
    \label{eq:p27-lg11}
\end{equation}
The linear Gaussian benchmark passes the derivative check only if
\begin{equation}
    B(\beta+h)=B(\beta)=B(\beta-h)
    \quad\text{and}\quad
    |D_h-G_{\rm FB}|
    \text{ decreases over a stable window of }h.
    \label{eq:p27-lg12}
\end{equation}
The branch record \(B\) is the discrete transcript of every choice that can
change the value path without being differentiable:
\begin{equation}
\begin{aligned}
    B=\{&
    \text{coordinate order, domains, basis families, ranks},\\
    &\text{selected fitting points, pivot choices, rank-truncation choices},\\
    &\text{shifts, floors, KR inversion branches, solver fallbacks}
    \}.
\end{aligned}
    \label{eq:p27-lg13}
\end{equation}
Branch equality means exact equality of this transcript at
\(\beta-h\), \(\beta\), and \(\beta+h\).  The default finite-difference ladder is
\begin{equation}
    \mathcal H_{\rm FD}
    =
    \{10^{-2},10^{-3},10^{-4},10^{-5}\}.
    \label{eq:p27-lg14}
\end{equation}
Let \(e(h)=|D_h-G_{\rm FB}|\).  A branch-stable decreasing window exists if
there are at least two consecutive ladder values \(h_i>h_{i+1}\) such that
\begin{equation}
    B(\beta-h_j)=B(\beta)=B(\beta+h_j),
    \qquad
    e(h_{i+1})<e(h_i),
    \qquad
    j\in\{i,i+1\}.
    \label{eq:p27-lg15}
\end{equation}
If no such window exists, the derivative validation fails and no
gradient-accuracy claim is made.  If roundoff later causes \(e(h)\) to stop
decreasing, the reported diagnostic is the minimum \(e(h)\) over the
branch-stable window before the plateau.

\subsection{The Long-Horizon Stochastic-Volatility Benchmark}

The stochastic-volatility model tests whether the method remains stable over a
long sequence with nonlinear observations.  The model is
\begin{equation}
    X_t=\gamma X_{t-1}+\sigma\epsilon_t^{(x)},
    \qquad
    \epsilon_t^{(x)}\sim N(0,1),
    \label{eq:p27-sv1}
\end{equation}
\begin{equation}
    Y_t
    =
    \beta\exp(X_t/2)\epsilon_t^{(y)},
    \qquad
    \epsilon_t^{(y)}\sim N(0,1),
    \label{eq:p27-sv2}
\end{equation}
with
\begin{equation}
    X_0\mid\gamma,\sigma
    \sim
    N\!\left(0,\frac{\sigma^2}{1-\gamma^2}\right).
    \label{eq:p27-sv3}
\end{equation}
For synthetic data Zhao and Cui fix \(\sigma=1\), estimate
\(\theta=(\gamma,\beta)\), use \(T=1000\), and generate data at
\((\gamma,\beta)=(0.6,0.4)\) \cite{zhao2024ttsequential}.  The joint posterior
dimension is
\begin{equation}
    \dim(\theta,x_{0:T})
    =
    ...
    =
    1003
    \quad\text{when }T=1000.
    \label{eq:p27-sv4}
\end{equation}
If one instead counts \(x_{1:T}\) with \(x_0\) absorbed into the initial
density, the state-parameter dimension is \(1002\).  The validation report
must state which convention is used.

The transformed coordinates used for unbounded-domain preconditioning should be
recorded explicitly and separately for the synthetic and real-data cases.  In
the synthetic fixed-\(\sigma\) benchmark, \(\sigma\equiv1\) and the transformed
parameter coordinates are
\begin{equation}
    \Theta_{\rm syn}'
    =
    \left(\Phi^{-1}(\gamma),\log\beta\right),
    \qquad
    X_{t,{\rm syn}}'=X_t.
    \label{eq:p27-sv5a}
\end{equation}
In the real-data free-\(\sigma\) benchmark following Zhao and Cui
\cite{zhao2024ttsequential}, the transformed coordinates are
\begin{equation}
    \Theta_{\rm real}'
    =
    \left(\Phi^{-1}(\gamma),\log\sigma,\frac{\log\beta}{\sigma}\right),
    \qquad
    X_{t,{\rm real}}'=\frac{X_t}{\sigma}.
    \label{eq:p27-sv5b}
\end{equation}
Here \(\Phi\) is the standard normal distribution function.  Each benchmark row
must state whether it uses \eqref{eq:p27-sv5a} or \eqref{eq:p27-sv5b}.

Because an analytical marginal posterior is not available for this nonlinear
long-horizon model, the main diagnostics are path-sample ESS, trajectory
recovery, posterior concentration around the synthetic truth, and comparison
with an independent SMC or SMC2 reference where feasible:
\begin{equation}
\begin{array}{ll}
\text{joint ESS} &
    Q_{\rm ESS}(q,t;R),\\[1mm]
\text{state-path RMSE} &
    {\rm RMSE}_{\rm med}(R),\\[1mm]
\text{credible-band coverage} &
    {\rm Cov}_{90}(R),\\[1mm]
\text{parameter concentration} &
    |\widehat\gamma_{0.5,t}-\gamma_{\rm true}|,
    \quad
    |\widehat\beta_{0.5,t}-\beta_{\rm true}|,\\[1mm]
\text{reference discrepancy} &
    D_{\rm ref}(
    \widehat p_{\rm TT}(\gamma,\beta\mid y_{1:t}),
    \widehat p_{\rm SMC}(\gamma,\beta\mid y_{1:t})).
\end{array}
\label{eq:p27-sv6}
\end{equation}
The rank ladder should include
\begin{equation}
    R\in\{5,10,20\},
    \qquad
    \ell=33,
    ...
    \label{eq:p27-sv7}
\end{equation}
matching the Zhao--Cui experimental scale \cite{zhao2024ttsequential}.  For
large-scale implementation work, record the memory and runtime curves
\begin{equation}
    R\mapsto
    \left(
    M_{\rm peak}(R),\,
    ...
    \label{eq:p27-sv8}
\end{equation}
A larger rank should not be treated as better if it produces ill-conditioned
solves, unstable KR inversions, or a memory blow-up that would make the method
unusable.

For a real-data volatility run, such as daily S\&P 500 returns, the parameter
priors should be recorded in the transformed and physical coordinates.  A
typical form, following the Zhao--Cui example, is
\begin{equation}
    \frac{\gamma+1}{2}\sim{\rm Beta}(\omega_1,\omega_2),
    \qquad
    \sigma^2\sim{\rm IG}(\zeta/2,S_\sigma/2),
    \qquad
    \log\beta\mid\sigma\sim N(\beta_0,\sigma^2/q_0),
    \label{eq:p27-sv9}
\end{equation}
with
\begin{equation}
    ...
    \label{eq:p27-sv10}
\end{equation}
Since no true hidden volatility path is observed in real data, the real-data
run is not an accuracy proof.  It tests stability, posterior reasonableness,
rank sensitivity, and whether inferred volatility responds to observed return
bursts.

\subsection{The Spatial SIR Benchmark For High-Dimensional State Filtering}

The spatial susceptible-infectious-removed model tests dimension scalability.
For \(J\) spatial compartments, let
\begin{equation}
    (S_j(t),I_j(t),R_j(t)),
    \qquad
    j=1,\ldots,J,
    \label{eq:p27-sir1}
\end{equation}
denote susceptible, infectious, and removed populations.  The coupled dynamics
are
\begin{equation}
    \frac{\dd S_j}{\dd t}
    =
...
\begin{equation}
    ...
    +
    \frac12
    \sum_{i\in\mathcal I_j}(R_i-R_j),
    \label{eq:p27-sir4}
\end{equation}
where \(\mathcal I_j\) is the neighbor set of compartment \(j\).  Because
\(S_j+I_j+R_j\) determines the third compartment once two are known, the state
can be represented as
\begin{equation}
    x(t)
    =
    (S_1(t),I_1(t),\ldots,S_J(t),I_J(t))
    \in\R^{2J}.
    \label{eq:p27-sir5}
\end{equation}
Let \(G(x)\) denote the right-hand side of the \(S\) and \(I\) equations.  The
discrete state transition is
\begin{equation}
    X_k
    =
    X_{k-1}
    ...
    \epsilon_k^{(x)},
    \qquad
    \epsilon_k^{(x)}\sim N(0,\Sigma_x),
    \label{eq:p27-sir6}
\end{equation}
where the integral is evaluated numerically, for example by fourth-order
Runge--Kutta.  The observation model sees only infectious components:
\begin{equation}
    Y_{k,j}=X_{k,2j}+\epsilon_{k,j}^{(y)},
    \qquad
    \epsilon_k^{(y)}\sim N(0,\Sigma_y),
    \qquad
    j=1,\ldots,J.
    \label{eq:p27-sir7}
\end{equation}
Zhao and Cui use \(J=9\), corresponding to an 18-dimensional state, partial
observations, \(T=20\), \(\kappa_j=0.1\), \(\nu_j=18\), and rank ladder
\begin{equation}
    R\in\{10,20,40\}.
    \label{eq:p27-sir8}
\end{equation}
Their reproduction settings are
\begin{equation}
    \Delta=0.02,
    \qquad
    \text{RK4 internal step}=0.005,
    \qquad
    \Sigma_x=I_{2J},
    \qquad
    \Sigma_y=100I_J,
    \label{eq:p27-sir8a}
\end{equation}
with synthetic initial conditions
\begin{equation}
    S_j(0)=485+j,
    \qquad
    I_j(0)=15-j,
...
and prior
\begin{equation}
    X_0\sim N(\mu_0,I_{18}),
    \qquad
    \mu_0=(S_1(0),I_1(0),\ldots,S_9(0),I_9(0)).
    \label{eq:p27-sir8c}
\end{equation}

This benchmark should report accuracy separately for observed and unobserved
state components.  Define
\begin{equation}
    \mathcal O=\{2,4,\ldots,2J\},
    \qquad
    \mathcal U=\{1,3,\ldots,2J-1\}.
    \label{eq:p27-sir9}
\end{equation}
Then report
\begin{equation}
    ...
    \sum_{t=0}^T
    \sum_{j\in\mathcal O}
    (x_{t,j}^{\rm true}-\widehat x_{t,j,0.5})^2
    \right]^{1/2},
    \label{eq:p27-sir10}
\end{equation}
and
\begin{equation}
    ...
    \label{eq:p27-sir11}
\end{equation}
The unobserved susceptible components are important: if the filter only tracks
the observed infectious series, it has not solved the high-dimensional latent
state problem.

The SIR model is also where memory and rank growth should be most visible.  A
dimension ladder should vary
\begin{equation}
    J\in\{3,5,9,J_{\rm large}\},
    \qquad
    D=2J,
    \qquad
    R\in\{10,20,40,R_{\rm max}\},
    \label{eq:p27-sir12}
\end{equation}
and report
\begin{equation}
    ...
    \label{eq:p27-sir13}
\end{equation}
A credible large-scale result should show not only a good final trajectory, but
also explain whether the cost is driven by state dimension \(2J\), rank \(R\),
ODE solves, retained-filter storage, or KR sampling.

\subsection{Rank-Calibrated Spatial SIR Route For Fixed-Branch Filtering}
\label{sec:p52-rank-calibrated-spatial-sir-route}

The spatial SIR benchmark exposes a distinction that is easy to miss.  A fixed
design is desirable for HMC because it defines one deterministic approximate
likelihood, but a dense all-pairs retained-grid transition is not a viable
high-dimensional fixed design.  If \(d=2J\) and each state coordinate uses
\(n\) retained points, the dense retained state grid has
\begin{equation}
    N_{\rm dense}=n^d
    \label{eq:p52-dense-grid-count}
\end{equation}
points, and a direct previous/current transition table has
\begin{equation}
    N_{\rm pairs}=N_{\rm dense}^2=n^{2d}
    \label{eq:p52-dense-pair-count}
\end{equation}
entries.  For the Zhao--Cui spatial SIR setting \(J=9\), \(d=18\), and
\(n=3\), this is
\begin{equation}
    N_{\rm dense}=3^{18}=387420489,
    \qquad
    N_{\rm pairs}=3^{36}=150094635296999121.
    \label{eq:p52-sir-d18-dense-count}
\end{equation}
This count is a route blocker, not a reason to abandon fixed branches.  The
replacement route must keep the likelihood branch fixed while applying the
transition through a streamed, local, or tensor-factorized contraction whose
rank and workspace are declared before the likelihood is evaluated.

For a tensor train with dimension \(d\), per-coordinate basis or grid size
\(n\), float64 storage, and fixed rank cap \(r\), the state-core storage
forecast is
\begin{equation}
    M_{\rm state}\approx 8\,d\,n\,r^2.
    \label{eq:p52-state-memory}
\end{equation}
This quantity is not enough for a production decision because the transition
application creates intermediate operator ranks, autodiff tapes, contraction
buffers, and orthogonalization workspace.  Let \(R_{\rm eff}\) denote the
effective transition-rank multiplier and let \(\omega\ge1\) be the workspace
multiplier.  The single-step transition workspace forecast is
\begin{equation}
    M_{\rm step}\approx
    8\,d\,n\,(R_{\rm eff}r)^2\,\omega.
    \label{eq:p52-step-memory}
\end{equation}
Given a single-step workspace cap \(M_{\rm step}^{\rm cap}\), the hard rank
ceiling is
\begin{equation}
    r_{\max}
    =
    \left\lfloor
    \sqrt{
    \frac{M_{\rm step}^{\rm cap}}
         {8\,d\,n\,\omega\,R_{\rm eff}^2}}
    \right\rfloor.
    \label{eq:p52-rank-ceiling}
\end{equation}
The source of \(R_{\rm eff}\) must be recorded before the run: a lower-rung
measurement, an operator-core estimate, or a conservative declared bound.  If
\(R_{\rm eff}\) is unknown, the row is a preflight blocker rather than an
invitation to launch a large rank ladder.

The fixed-rank target is the approximate likelihood defined by a complete
...
\begin{equation}
    ...
    \Pi_{\rm trunc}
    \right),
    \label{eq:p52-fixed-rank-branch}
\end{equation}
where \(r\) is the rank cap, \(b_{1:d}\) are the coordinate bases, \(c_{1:d}\)
are the grid centers, \(s_{1:d}\) are the coordinate scales,
\(\Pi_{\rm contract}\) is the contraction path, and \(\Pi_{\rm trunc}\) is the
truncation policy.  These choices are part of the scalar being differentiated.
They may be calibrated offline, but they cannot change during an HMC
likelihood call or during a same-branch derivative check.

An unscented Kalman filter may be used as a scout for centers, scales, and
covariance structure.  For sigma points \(\chi_i\) and weights
\((W_i^{(m)},W_i^{(c)})\), the prediction equations are
\begin{align}
    m_{t|t-1}
    &=
    \sum_i W_i^{(m)} f_\theta(\chi_i),
    \label{eq:p52-ukf-pred-mean}\\
    P_{t|t-1}
    &=
    Q_t+
    \sum_i W_i^{(c)}
    \{f_\theta(\chi_i)-m_{t|t-1}\}
    \{f_\theta(\chi_i)-m_{t|t-1}\}^{\top}.
    \label{eq:p52-ukf-pred-cov}
\end{align}
Applying the observation map \(h_\theta\) to sigma points from
\((m_{t|t-1},P_{t|t-1})\) gives
\begin{align}
    \widehat y_t
    &=
    \sum_i W_i^{(m)}h_\theta(\chi_i^-),
    \label{eq:p52-ukf-obs-mean}\\
    S_t
    &=
    R_t+
    \sum_i W_i^{(c)}
    \{h_\theta(\chi_i^-)-\widehat y_t\}
    \{h_\theta(\chi_i^-)-\widehat y_t\}^{\top},
    \label{eq:p52-ukf-obs-cov}\\
    C_t
    &=
    \sum_i W_i^{(c)}
    \{\chi_i^- -m_{t|t-1}\}
    \{h_\theta(\chi_i^-)-\widehat y_t\}^{\top}.
    \label{eq:p52-ukf-cross-cov}
\end{align}
The update is
\begin{equation}
    K_t=C_tS_t^{-1},
    \qquad
    m_{t|t}=m_{t|t-1}+K_t(y_t-\widehat y_t),
    \qquad
    P_{t|t}=P_{t|t-1}-K_tS_tK_t^\top.
    \label{eq:p52-ukf-update}
\end{equation}
These equations are useful for scale discovery, effective-dimension spectra,
and local-correlation diagnostics.  They are not a correctness oracle for the
Zhao--Cui filter, not an exact likelihood, and not evidence of HMC readiness.

The rank-selection protocol is therefore:
\begin{equation}
\begin{array}{ll}
1. & \text{Run the UKF scout and record }(m_{t|t},P_{t|t})
     \text{ only as scout evidence.}\\[1mm]
2. & \text{Set }(c_{1:d},s_{1:d})\text{ from scout moments or a declared
     lower-rung rule.}\\[1mm]
3. & \text{Estimate or declare }R_{\rm eff}\text{ and compute }r_{\max}
     \text{ by \eqref{eq:p52-rank-ceiling}.}\\[1mm]
4. & \text{Filter the candidate set }
     \mathcal R_0=\{2,4,8,16,32\}
     \text{ to }
     \mathcal R=\{r\in\mathcal R_0:r\le r_{\max}\}.\\[1mm]
5. & \text{If }\mathcal R=\varnothing,\text{ stop with a rank-budget
     blocker.}\\[1mm]
6. & \text{Run candidates in increasing }r\text{ with the same branch
     definition except rank.}\\[1mm]
7. & \text{Select the first rank passing the declared reference or
     same-route self-convergence criteria.}\\[1mm]
8. & \text{Freeze }\mathcal B\text{ and evaluate the HMC-facing likelihood
     only on that frozen branch.}
\end{array}
\label{eq:p52-rank-protocol}
\end{equation}
The ladder cannot pass self-convergence at its largest feasible rank unless a
separate reviewed comparator exists.  If two consecutive ranks produce
nonfinite values, nonfinite gradients, memory-preflight failures, or worsening
diagnostics without a model-invariant explanation, the outcome is a recorded
rank, coordinate, factorization, or reference-strategy blocker.  It is not an
unbounded rank increase.

The dimension policy for spatial SIR is also part of the claim boundary:
\begin{equation}
\begin{array}{c|c|c}
d & J & \text{first allowed role}\\ \hline
18 & 9  & \text{filtering and calibration candidate after route repair}\\
50 & 25 & \text{high-dimensional stress and rank self-convergence candidate}\\
100 & 50 & \text{UKF scout and memory preflight by default.}
\end{array}
\label{eq:p52-dimension-policy}
\end{equation}
Thus \(d=50\) is the first reasonable full filtering stress target under a
32 GB practical memory contract.  Dimension \(d=100\) may become a bounded
filtering stress row only after the lower gates pass and the memory predictor
clears; otherwise it remains scout and preflight evidence.

\subsection{P53 Route-Class Repair For Spatial SIR Transition Application}
\label{sec:p53-spatial-sir-route-class-repair}

The failed P52 execution adds one more design rule.  A route contract and a
lower-rung streaming evaluator are useful, but they are not the same evidence as
a high-dimensional scaling route.  We therefore split the transition
application evidence into two route classes:
\begin{equation}
    \mathsf C_{\rm low}
    =
    \text{lower-rung dense-equivalent streaming route},
    \qquad
    \mathsf C_{\rm scale}
    =
    \text{local-neighborhood or TT--MPO scaling route}.
    \label{eq:p53-route-classes}
\end{equation}
The lower-rung route \(\mathsf C_{\rm low}\) computes the same predictive
quantity as the dense retained-grid update while avoiding a full in-memory
transition matrix.  For current points \(x_i\), previous retained points
\(z_j\), previous log density \(\ell_{t-1}(z_j)\), quadrature weights \(w_j\),
and Jacobian term \(a_j\), the predictive log density is
\begin{equation}
    \ell^-_t(x_i)
    =
    \log\sum_{j=1}^{N_{\rm prev}}
    \exp\left\{
      \log w_j+a_j+\ell_{t-1}(z_j)
      +\log p_\theta(x_i\mid z_j)
    \right\}.
    \label{eq:p53-streaming-predictive}
\end{equation}
The streaming evaluator partitions current and previous indices into fixed
blocks \(B_c,B_p\), evaluates \(\log p_\theta(x_i\mid z_j)\) only inside one
block pair at a time, and accumulates \eqref{eq:p53-streaming-predictive} by a
stable log-sum-exp reduction.  Its replay identity is
\begin{equation}
    \mathcal I_{\rm low}
    =
    (\mathsf C_{\rm low}, B_c, B_p, \text{point ordering},
     \text{sum order}, \text{dtype}, \text{branch id}),
    \label{eq:p53-low-route-identity}
\end{equation}
which is part of the frozen branch.  The peak storage scales like
\begin{equation}
    M_{\rm low}
    =
    O\!\left(8\,B_cB_p+8(B_c+B_p)d\right),
    \label{eq:p53-low-route-memory}
\end{equation}
plus autodiff workspace.  This route is appropriate for interface hardening and
tiny-grid dense tie-out.  It is not a scalability proof: its arithmetic still
has dense-equivalent pair semantics.

The scaling route \(\mathsf C_{\rm scale}\) must remove dense-equivalent
semantics from the production transition application.  Two admissible designs
are:
\begin{equation}
    \mathsf C_{\rm loc}:
    \quad
    ...
    \approx
    \sum_{m=1}^{M}
    \phi_m\!\left(x_{\mathcal N_m},z_{\mathcal N_m};\theta\right),
    \label{eq:p53-local-transition-factor}
\end{equation}
where \(\mathcal N_m\) are deterministic spatial neighborhoods, and
\begin{equation}
    \mathsf C_{\rm mpo}:
    \quad
    K_\theta(x,z)
    ...
    A^{(1)}_{\alpha_0,\alpha_1}(x_1,z_1;\theta)
    \cdots
    A^{(d)}_{\alpha_{d-1},\alpha_d}(x_d,z_d;\theta),
    \label{eq:p53-mpo-transition-factor}
\end{equation}
with \(\alpha_0=\alpha_d=1\).  In either case the route must emit a conservative
route-width bound \(R_{\rm eff}\) and memory metadata before the rank ladder is
run.  Its replay identity is
\begin{equation}
    \mathcal I_{\rm scale}
    =
    (\mathsf C_{\rm scale}, \mathcal N_{1:M}\text{ or MPO cores},
     R_{\rm eff}, \Pi_{\rm contract}, \Pi_{\rm trunc},
     \text{dtype}, \text{branch id}).
    \label{eq:p53-scale-route-identity}
\end{equation}
The scaling route must be tested on lower rungs against
\eqref{eq:p53-streaming-predictive} or the dense reference before it can unlock
rank selection.  The gate is
\begin{equation}
\begin{array}{ll}
\text{if only }\mathsf C_{\rm low}\text{ has passed,}
    & \text{block rank/scaling phases;}\\[1mm]
\text{if }\mathsf C_{\rm scale}\text{ has passed design, implementation,}
    & \text{lower-rung tie-out, replay, and metadata checks,}\\
    & \text{then rank selection may consume }R_{\rm eff}.
\end{array}
\label{eq:p53-scaling-route-gate}
\end{equation}
Thus \(R_{\rm eff}\) in \eqref{eq:p52-step-memory} must come from
\(\mathsf C_{\rm scale}\), not from the lower-rung dense-equivalent streaming
route.  Any implementation that has only \(\mathsf C_{\rm low}\) evidence may
claim lower-rung tie-out and interface correctness, but it must emit a
scaling-route blocker rather than a spatial SIR \(d=18\), \(d=50\), or
\(d=100\) filtering claim.

\subsection{The Predator-Prey Benchmark For Nonlinear Preconditioning}

The predator-prey model tests the preconditioning story.  Let \(P(t)\) be prey
population and \(Q(t)\) predator population.  The continuous dynamics are
\begin{equation}
    \frac{\dd P}{\dd t}
    =
...
\begin{equation}
    ...
    \label{eq:p27-pp3}
\end{equation}
Writing \(x=(P,Q)\) and \(G(x;\theta)\) for the right-hand side, the discrete
state equation is
\begin{equation}
    X_k
    =
...
\begin{equation}
    ...
    \label{eq:p27-pp5}
\end{equation}
Zhao and Cui use the synthetic truth
\begin{equation}
    \theta_{\rm true}
    =
    ...
    \label{eq:p27-pp5a}
\end{equation}
with \(\Delta=2\), RK4 internal step \(0.1\), and
\begin{equation}
    \Sigma_x=4I_2,
    \qquad
    \Sigma_y=4I_2.
    \label{eq:p27-pp5b}
\end{equation}
The parameter prior is uniform on the box
\begin{equation}
    [a,b]
    =
    [0.1,1.1]\times[110,130]\times[20,30]
    \times[0.1,1.1]\times[0,1]\times[0,1],
    \label{eq:p27-pp5c}
\end{equation}
and the initial-state prior is
\begin{equation}
    X_0\sim N((50,5),I_2).
    \label{eq:p27-pp5d}
\end{equation}
The validation should compare linear and nonlinear preconditioning at the same
nominal rank:
\begin{equation}
    \mathcal P_{\rm lin}
    =
    \text{linear Gaussian bridge},
    \qquad
    \mathcal P_{\rm nonlin}
    =
    \text{tempered nonlinear bridge},
    \label{eq:p27-pp6}
\end{equation}
with the same basis size, rank, and sweep count.  The main preconditioning
effect is measured by
\begin{equation}
    \Delta_{\rm ESS}(t)
    =
    Q_{\rm ESS}^{\rm nonlin}(0.50,t)
    -
    Q_{\rm ESS}^{\rm lin}(0.50,t),
    \label{eq:p27-pp7}
\end{equation}
and by the cost-normalized improvement
\begin{equation}
    \Delta_{\rm cost}(t)
    =
    \frac{Q_{\rm ESS}^{\rm nonlin}(0.50,t)}
    {T_{\rm wall}^{\rm nonlin}(t)}
    -
    \frac{Q_{\rm ESS}^{\rm lin}(0.50,t)}
    {T_{\rm wall}^{\rm lin}(t)}.
    \label{eq:p27-pp8}
\end{equation}
The second quantity prevents a misleading conclusion: nonlinear
preconditioning may improve ESS but still be unattractive if it makes the
computation too expensive.

\subsection{BayesFilter Stress Ladders Beyond The Paper}

The Zhao--Cui models should be the core suite, but a BayesFilter validation
suite reproduction alone is not enough for a BayesFilter large-scale claim.  A
BayesFilter validation program also needs controlled stress ladders that
isolate one difficulty at a time.
The generic nonlinear state-space model is
\begin{equation}
    X_t=F_\theta(X_{t-1})+\Sigma_x^{1/2}\epsilon_t^{(x)},
    \qquad
    Y_t=H_\theta(X_t)+\Sigma_y^{1/2}\epsilon_t^{(y)}.
    \label{eq:p27-bf1}
\end{equation}
Use it to define four increasing ladders:
\begin{equation}
\begin{array}{ll}
\text{dimension ladder} &
    m\in\{2,4,8,16,32,64\},\\[1mm]
\text{horizon ladder} &
    T\in\{10,50,100,500,1000\},\\[1mm]
\text{rank ladder} &
    R\in\{4,8,16,32,64\},\\[1mm]
\text{basis ladder} &
    p\in\{9,17,33,65\}.
\end{array}
\label{eq:p27-bf2}
\end{equation}
Only one ladder should be varied at a time unless the interaction is the
question being tested.  The reported response surface is
\begin{equation}
    \mathcal R(m,T,R,p)
    =
    \left[
    ...
    \label{eq:p27-bf3}
\end{equation}
where \(E_{\rm acc}\) is the best available accuracy metric,
\(\kappa_{\rm max}\) is the largest observed least-squares condition number,
and \(\rho_{\rm mass}\) is a mass-contraction residual such as
\begin{equation}
    \rho_{\rm mass}
    =
    \max_t
    \frac{
    |\widehat Z_t^{\rm left}-\widehat Z_t^{\rm right}|}
    {1+|\widehat Z_t^{\rm left}|+|\widehat Z_t^{\rm right}|}.
    \label{eq:p27-bf4}
\end{equation}
Here \(\widehat Z_t^{\rm left}\) and \(\widehat Z_t^{\rm right}\) are the same
normalizer computed by two contraction orders when both are available.  They
should agree up to numerical error; disagreement suggests a contraction,
mass-matrix, or conditioning defect.

\subsection{Robustness Tests And Veto Conditions}

The validation report should mark a run as failed before interpreting posterior
plots if any mathematical veto fires.  Let
\begin{equation}
    \mathcal F_t
    =
    \{
...
\begin{equation}
    ...
    =
    \ind\left\{
    \exists t,\exists a\in\mathcal F_t:
    a\text{ contains NaN or Inf}
    \right\}.
    \label{eq:p27-r2}
\end{equation}
...
\begin{equation}
    ...
    \widehat Z_t\le0
    \text{ or }
    \left|\int \widehat p_t(z)\dd z-1\right|>\varepsilon_{\rm norm}
    \right\}.
    \label{eq:p27-r3}
\end{equation}
...
\begin{equation}
    ...
    =
    \ind\left\{
    \max_{t,k}\kappa(N_{t,k})
    >
    \kappa_{\rm max}^{\rm allowed}
    \right\}.
    \label{eq:p27-r4}
\end{equation}
...
\begin{equation}
    ...
    R_t=R_{\rm max}
    \text{ and }
    {\rm residual}_t>\varepsilon_{\rm fit}
    \right\}.
    \label{eq:p27-r5}
\end{equation}
...
\begin{equation}
    ...
    \text{ or }
    B(\beta-h)\ne B(\beta)
    \text{ or no branch-stable decreasing window in }\mathcal H_{\rm FD}
    \right\}.
    \label{eq:p27-r7}
\end{equation}
...
\begin{equation}
    ...
    +V_{\rm rank}
    +V_{\rm KR}
    +V_{\rm grad}^{\rm required}
    =
    0,
    \label{eq:p27-r8}
\end{equation}
where \(V_{\rm grad}^{\rm required}=V_{\rm grad}\) only for runs whose purpose
includes validating the derivative.
The tolerances in these vetoes are declared before a run.  A minimal default
table is
\begin{equation}
\begin{array}{c|c|c}
...
\hline
\varepsilon_{\rm norm}
&10^2\epsilon_{\rm mach}+10\,\varepsilon_{\rm quad}
&\text{normalization tolerance}\\
\kappa_{\rm max}^{\rm allowed}
&10^{10}\text{ in double precision}
&\text{least-squares conditioning ceiling}\\
\varepsilon_{\rm fit}
&\max(10^{-8},10\,{\rm tol}_{\rm ALS})
&\text{rank-saturation residual ceiling}\\
\varepsilon_{\rm KR}
&10^2\epsilon_{\rm mach}+10\,\varepsilon_{\rm quad}
&\text{monotonicity/inversion tolerance}
\end{array}
\label{eq:p27-r9}
\end{equation}
Here \(\epsilon_{\rm mach}\) is machine precision, \(\varepsilon_{\rm quad}\)
is the declared quadrature or interpolation error proxy, and
\({\rm tol}_{\rm ALS}\) is the ALS stopping tolerance.  These are
implementation tolerances, not scientific outcomes; changing them after seeing
posterior plots invalidates the benchmark interpretation.

\subsection{A Compact Benchmark Table Template}

For each model and each configuration, the validation table should contain the
same core columns, while making clear that the accuracy quantities are
model-specific rather than directly comparable across rows:
\begin{equation}
\begin{array}{c|c|c|c|c}
\text{model}
&D
...
\label{eq:p27-t1}
\end{equation}
The corresponding accuracy-diagnostic table is
\begin{equation}
\begin{array}{c|c|c}
\text{model}
&\text{diagnostic class}
&\mathcal E_{\rm model}\\
\hline
\text{LG}
&\text{exact reference}
&D_H,E_{L^1},E_Z\\
\text{SV}
&\text{synthetic truth/reference}
&{\rm RMSE}_{\rm med},{\rm Cov}_{90}\\
\text{SIR}
&\text{synthetic truth}
&{\rm RMSE}_{\mathcal O},{\rm RMSE}_{\mathcal U}\\
\text{PP}
&\text{synthetic truth/preconditioning}
&{\rm RMSE}_{\rm med},{\rm Cov}_{90}
\end{array}
\label{eq:p27-t1b}
\end{equation}
In a completed empirical study the measured values and uncertainty summaries
are reported beside these symbols.  A separate derivative table should be
attached whenever the fixed-branch gradient is part of the claim:
\begin{equation}
\begin{array}{c|c|c|c}
h
&D_h
...
\\
\hline
10^{-2}&\text{measured}&\text{measured}&\text{measured}\\
10^{-3}&\text{measured}&\text{measured}&\text{measured}\\
10^{-4}&\text{measured}&\text{measured}&\text{measured}\\
10^{-5}&\text{measured}&\text{measured}&\text{measured}
\end{array}
\label{eq:p27-t2}
\end{equation}
The same empirical table should also record
\(\widehatell_T(\beta+h;B)\) and \(\widehatell_T(\beta-h;B)\) for each \(h\).
The derivative table is meaningful only when the branch record is identical in
the three columns \(\beta-h,\beta,\beta+h\).

\subsection{What Each Benchmark Can And Cannot Establish}

The linear Gaussian model can establish exact-reference agreement for the
implemented approximation at the tested dimensions and horizons, conditional
on the declared seeds, tolerances, and uncertainty summaries:
\begin{equation}
    \text{LG pass}
    \Rightarrow
    \text{exact-reference accuracy is healthy under the declared run contract.}
    \label{eq:p27-c1}
\end{equation}
It cannot establish nonlinear posterior accuracy.  The stochastic-volatility
model can establish long-horizon nonlinear stability and credible agreement
with references or synthetic truth, again under the declared run contract:
\begin{equation}
    \text{SV pass}
    \Rightarrow
    \text{long-horizon nonlinear filtering is plausible under the run contract.}
    \label{eq:p27-c2}
\end{equation}
It cannot by itself establish high-dimensional scalability.  The spatial SIR
model can establish whether the method remains useful as the latent state
dimension grows, conditional on replicated runs or an explicit uncertainty
justification:
\begin{equation}
    \text{SIR pass}
    \Rightarrow
    \text{partial-observation filtering is credible at the tested }J,T,R,p.
    \label{eq:p27-c3}
\end{equation}
It cannot prove that all spatial graphs or all epidemic models have moderate
TT rank.  The predator-prey model can establish that nonlinear
preconditioning has a measurable benefit on a nonlinear dynamical system:
\begin{equation}
    \text{PP pass}
    \Rightarrow
    \Delta_{\rm ESS}>0
    \text{ and }
    \Delta_{\rm cost}>0
    \text{ support the preconditioning choice.}
    \label{eq:p27-c4}
\end{equation}
It cannot prove that nonlinear preconditioning is always worth its cost.
One-seed demonstrations are diagnostic unless the empirical result explicitly
justifies why stronger interpretation is appropriate.

The complete validation statement therefore has the form
\begin{equation}
\begin{aligned}
&\mathcal V_0,\ldots,\mathcal V_5\text{ pass on declared models, seeds,
hardware, and tolerances}
\\
&\qquad\Longrightarrow
\text{the implementation is empirically credible for the tested large-scale
SSM regimes.}
\end{aligned}
\label{eq:p27-c5}
\end{equation}
The right-hand side is deliberately empirical and conditional.  It is the
right conclusion for a large-scale approximation algorithm: the mathematics
defines the scalar and the derivative, while the validation suite measures
whether the approximation is accurate, stable, and affordable enough to use.
...
\section{Four Checklists For Reading The Derivative}

This section collects four compact checklists for the fixed-branch part.  They
state, after the derivation, what has been established and what a later
...
\appendix
\section{Relation To Zhao And Cui}
...
the preconditioning composition formulas are represented in the notation of this note
