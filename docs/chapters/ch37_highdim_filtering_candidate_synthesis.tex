...
\[
  ...
  \Var(L)=\frac{d}{2R^2}.
\]
The heuristic variance factor above is therefore of exponential order in
\(d/R^2\).  This example is deliberately simple; its role is to show why a
large macro-finance observation panel cannot be rescued by a particle count
that scales only linearly with \(d\).
...
\end{equation}
If the filter supplies \(\widehat\ell_T\), the sampler targets the declared
approximate posterior \(\widehat\pi(\theta\mid y_{1:T})\), not the exact
posterior.  Pseudo-marginal or particle-MCMC citations are relevant only when a
separately defined extended-state correction scheme is part of the algorithm;
they do not by themselves convert an approximate-filter HMC trajectory into an
ordinary exact-target HMC method.  The analytical exact-gradient contract is
developed in Chapter~\ref{ch:bf-highdim-hmc-research}; an approximate-filter
gradient supports only the declared approximate scalar.  It is not an exact
posterior gradient or exact state-space likelihood gradient unless such an
...
list of fashionable methods.  It is a sequence of vetoes.  This chapter consumes
those exported defects and gates; it does not reopen the construction of the
upstream approximation families or strengthen their source support by synthesis
alone.
