...
% Local compatibility aliases for imported p47 notation
\newcommand{\widehatell}{\widehat{\ell}}
\newcommand{\dotwidehatell}{\dot{\widehat{\ell}}}
...
a fixed sparse-grid quadrature filter that carries a single Gaussian surrogate,
evaluates the required nonlinear moments by a fixed deterministic cloud in
standardized Gaussian coordinates, and differentiates the same declared
approximate scalar on a frozen computational branch.

This chapter imports the exact-target and same-scalar discipline from
Chapter~\ref{ch:bf-highdim-nonlinear-foundations}, develops the deterministic
Gaussian/sparse-grid lane as one explicit approximate object, and exports its
projection, branch, and target-boundary defects to
Chapter~\ref{ch:bf-highdim-candidate-synthesis}.  The next chapter,
Chapter~\ref{ch:bf-highdim-particle-transport-tensor}, turns to richer
non-Gaussian carried objects for the regimes where one Gaussian is too narrow.
The task here is to make one deterministic Gaussian-surrogate lane precise
enough to read, implement, audit, and compare.  That task has several layers.
One must identify the exact filtering object being approximated, state what the
Gaussian surrogate keeps and discards, show how the sparse-grid cloud is
actually built, define the deterministic scalar computed on a fixed branch, and
then explain why the implementation details matter to the identity of that
scalar.

The running quadratic-observation cell from
Chapter~\ref{ch:bf-highdim-nonlinear-foundations} reappears here in two roles.
First, it provides a compact nonlinear filtering example when the chapter needs
a shared object with the rest of the block.  Second, the later one-step numeric
oracle uses the same lane contract as an implementation check rather than as a
replacement for the chapter-wide structural discussion.
...
single Gaussian surrogate \(\calN(m_t,P_t)\).  The nonlinear moments needed by the
...
\begin{equation}
  \widehatell_T^{\rm FSGQ}(\theta)
  =
  \sum_{t=1}^T \widehatell_t^{\rm FSGQ}(\theta),
...
\begin{aligned}
  ...
  &\quad \widehatell_T^{\rm FSGQ}(\theta).
\end{aligned}
  \label{eq:p32-exact-vs-approximate-summary}
\end{equation}
This distinction remains the governing contract of the note.  Later sections
return to it only when a new consequence must be made explicit: what the cloud is
approximating, what the scalar actually depends on, and what the derivative is
...
one-dimensional rules, tensor products, active index sets, merged clouds, and the
UKF bridge.  Third, that fixed cloud is inserted into the filtering value
recursion and then into the same-scalar derivative recursion.  Fourth, the
resulting lane is verified, positioned against neighboring deterministic Gaussian
alternatives, and bounded by explicit non-claims.
...
state space through the current Gaussian factor.  That sentence is worth keeping
in view because it is the bridge between the cloud-construction chapter and the
later value, implementation, and derivative chapters.
...
The chapter uses the following principal objects throughout the value and
gradient paths.  An implementation worker should treat this table as the core
object inventory for the chapter.
\begin{center}
\small
...
\(\widehatell_t^{\rm FSGQ}\) & one-step deterministic approximate log-likelihood increment & scalar \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=0.28in]
  ...
  \(P=CC^\top\), whenever the declared branch is valid.
...
\end{align}
where \(X_{t-1}\sim\calN(m_{t-1},P_{t-1})\) under the carried Gaussian
...
Let \(X=m+C\xi\) with \(\xi\sim\calN(0,I_2)\).  If each standardized coordinate
...
\subsubsection{The one-dimensional Gaussian quadrature rule}
...
for its number of points.  Concretely, one picks nodes
\(\zeta_{\ell,a}\) and weights \(\omega_{\ell,a}\), evaluates the integrand at
those nodes, and forms the weighted sum
\[
  \sum_{a=1}^{s_\ell}\omega_{\ell,a}\psi(\zeta_{\ell,a}).
\]
The reason this family is important here is simple: once a Gaussian moment has
been standardized to the coordinate \(\xi\), standard-normal GHQ gives a canonical
way to approximate that moment by a small deterministic set of points.  In this
note, the first few members are the familiar odd-point rules centered at zero:
one point for level 1, three points for level 2, five points for level 3, and so
on.
...
three-point rule.  The later formal policy statement
\eqref{eq:p41-ghq-family} records the same choice more precisely.
...
\(\xi=0\) with total weight \(1\).  In the notation of
\eqref{eq:p31-univariate-rule}, this means \(s_1=1\),
\(\zeta_{1,1}=0\), and \(\omega_{1,1}=1\), so
\[
  I_1[\psi]=\psi(0).
\]
...
\]
So \(I_1:(0,1)\) is not mysterious notation.  It is the natural level-1 base
rule: evaluate at the center of the standardized Gaussian and assign it the full
mass.
...
the two-dimensional and three-dimensional examples.  In other words, the notation
\(I_2\) is not a new assumption introduced later: it is simply shorthand for the
symmetric three-point standard-normal rule obtained from
\eqref{eq:p36-three-point-mass}--\eqref{eq:p36-three-point-fourth}.  Together
with the level-1 rule \(I_1:(0,1)\), it gives the smallest pair of
one-dimensional rules needed for the worked tensor examples below.
...
played by \(\psi\).  In several coordinates we simply switch to \(F(\xi)\)
to remind the reader that the function may depend on many variables.  A concrete
...
\subsubsection{What is the multi-index \texorpdfstring{\(\mathbf i=(i_1,\ldots,i_b)\)}{i=(i1,...,ib)}?}
...
\end{quote}
So the tensor rule is just the multi-coordinate version of the scalar weighted
sum from \eqref{eq:p31-univariate-rule}.  First choose a one-dimensional rule in
each coordinate.  Then list every possible combination of those coordinatewise
nodes.  Finally, attach to each combined point the product of the corresponding
one-dimensional weights.  The tensor notation therefore does not introduce a new
kind of quadrature object; it only packages the coordinatewise Cartesian-product
construction compactly.

\subsubsection{A fully worked two-dimensional tensor example}
...
\end{align}
Equation \eqref{eq:p31-tensor-node} says how to build one tensor point: choose one
standardized node from each coordinate rule and place those chosen values next to
each other.  Equation \eqref{eq:p31-tensor-weight} then says how to weight that
point: multiply the weights attached to the chosen one-dimensional nodes.

In the concrete \((2,2)\) case, each coordinate uses the level-2 rule
\(I_2=\{(-\sqrt3,1/6),(0,2/3),(\sqrt3,1/6)\}\).  So
\(\alpha_1,\alpha_2\in\{1,2,3\}\), and every pair
\((\alpha_1,\alpha_2)\) chooses one node from coordinate 1 and one node from
coordinate 2.  For example,
\[
  (\alpha_1,\alpha_2)=(1,1)
  \mapsto
  (-\sqrt3,-\sqrt3)
...
\[
  ...
  \frac16\cdot\frac16=\frac1{36}.
\]
So equation \eqref{eq:p35-nine-point-grid} is not a different construction from
\eqref{eq:p31-tensor-node}--\eqref{eq:p31-tensor-weight}.  It is simply those two
abstract formulas written out explicitly for the case \(b=2\) with level 2 in
both coordinates.
...
\end{equation}
Here ``level 2'' means: in each one-dimensional coordinate, use the level-2 rule
\(I_2\), namely the three-point standard-normal rule with nodes
\(-\sqrt3,0,\sqrt3\) and weights \(1/6,2/3,1/6\).  So the product rule
\((2,2,2)\) is only a bookkeeping label saying
\begin{quote}
use \(I_2\) in coordinate 1, use \(I_2\) in coordinate 2, and use \(I_2\) in
coordinate 3.
\end{quote}
Because each coordinate contributes three one-dimensional nodes, the full tensor
product has \(3\times 3\times 3=27\) raw node combinations before any sparse-grid
selection or duplicate merging.
...
To make the reduction feel less abstract, fix a simple explicit test function,
for example
\begin{equation}
  F(\xi_1,\xi_2,\xi_3)=\xi_1^2+\xi_2^2+\xi_3^2.
  \label{eq:p39-3d-test-function}
\end{equation}
This is a good teaching example because it depends only on single-coordinate
quadratic terms.  It therefore matches the kind of low-order interaction
structure that a low sparse-grid level is designed to preserve.

Under the full 27-point tensor rule \((2,2,2)\), the weighted average of
\(F\) is exactly
\begin{equation}
  \E[F(\xi)] = \E[\xi_1^2]+\E[\xi_2^2]+\E[\xi_3^2] = 1+1+1 = 3.
  \label{eq:p39-3d-test-exact}
\end{equation}
Now check the sparse-grid construction on the same function.  On the merged
seven-point cloud described below, the center point contributes
\(F(0,0,0)=0\), while each axis point such as \((\sqrt3,0,0)\) or
\((0,-\sqrt3,0)\) gives
\begin{equation}
  F = (\sqrt3)^2 = 3.
  \label{eq:p39-axis-value}
\end{equation}
If the seven-point merged cloud has center weight 0 and six axis weights
\(1/6\), then the sparse-grid sum is
\begin{equation}
  \sum_r w_rF(\xi^{(r)})
  = 6\cdot \frac16\cdot 3
  = 3,
  \label{eq:p39-3d-test-sparse-sum}
\end{equation}
which exactly matches \eqref{eq:p39-3d-test-exact}.  So in this concrete case,
the sparse-grid rule keeps far fewer points than the 27-point tensor product but
still gets the selected low-order quadratic structure exactly right.
...
all four tensor components, so after duplicate merging the cloud has seven
distinct nodes:
\begin{equation}
  (0,0,0),
  \qquad
...
Because the merged center weight is exactly zero in this worked level-2 case,
the stored fixed cloud defined later by the zero-weight pruning convention keeps
only the six axis points.  So there are three distinct 3D objects to keep
separate:
\begin{enumerate}[leftmargin=0.28in]
  \item the 27-point full tensor-product comparator \((2,2,2)\),
  \item the 10 raw sparse-grid contributions from
  \((1,1,1),(1,1,2),(1,2,1),(2,1,1)\), and
  \item the final stored 6-point cloud after duplicate merging and zero-weight
  pruning.
\end{enumerate}
So in this concrete 3D case the sparse-grid construction replaces the direct
27-point tensor-product picture by a signed sparse-grid combination with 10 raw
contributions, 7 merged distinct nodes, and 6 stored nodes after pruning.  Why
does this work?
...
\subsubsection{Sparse-Grid Rule Reconstructed In Source Order}
...
\subsubsection{Start with the scalar and 2D cases before the general notation}
...
\subsubsection{The Jia--Xin--Cheng Index Sets}
...
In the 3D preview from the previous section, the same symbols mean the following.
Take:
\begin{equation}
  b=3,
  \qquad
  ...
  \label{eq:p38-3d-index-symbols}
\end{equation}
Then:
\begin{itemize}[leftmargin=0.28in]
  \item \(b=3\) means there are three standardized coordinates,
  \item \(q\) says how far the total index is above the baseline value \(b\),
  \item \(N_q^3\) is the set of all three-coordinate multi-indices with total
  sum \(3+q\),
  \item \(\mathbf i=(i_1,i_2,i_3)\) records which one-dimensional rule level is
  used in each of the three coordinates.
\end{itemize}
So, for example,
\begin{align}
  N_0^3 &= \{(1,1,1)\},
  ...
  \label{eq:p38-N13}
\end{align}
These are exactly the 3D tensor rules used in the preview: one central
one-point tensor rule and three one-axis three-point tensor rules.  This is the
most concrete way to read the notation \(N_q^b\) in the present note.
...
\end{equation}
\paragraph{Important warning about the word ``level''.}
This is the place where many readers first get lost.  The sparse-grid level
\(L=2\) is \emph{not} the same object as the tensor rule \((2,2)\) or the
three-dimensional tensor rule \((2,2,2)\).  The tensor label \((2,2)\) says
``use the level-2 one-dimensional rule in both coordinates.''  By contrast, the
sparse-grid level \(L=2\) says ``keep tensor rules whose total index lies in the
active band for level 2 and combine them by the Smolyak rule.''  That is why the
active two-dimensional sparse-grid set contains \((1,1)\), \((1,2)\), and
\((2,1)\), but not \((2,2)\).
...
\begin{equation}
  ...
  \label{eq:p38-A32}
\end{equation}
because those are exactly the multi-indices whose total level lies between
\(2\) and \(2+3-1=4\) while still respecting that every coordinate level is at
least 1.  So the abstract band notation now directly recovers the explicit 3D
preview from the previous section.

\subsubsection{The Smolyak Coefficients}
...
\begin{tikzpicture}[scale=0.95, every node/.style={font=\small}]
  ...
  \end{scope}
  \draw[-{Latex[length=2mm]}, thick] (1.55,0) -- (2.55,0);
  \draw[-{Latex[length=2mm]}, thick] (5.65,0) -- (6.65,0);
  \draw[-{Latex[length=2mm]}, thick] (9.75,0) -- (10.75,0);
\end{tikzpicture}
\caption{How the two-dimensional sparse-grid cloud is built.  The active tensor
rules \((1,1)\), \((1,2)\), and \((2,1)\) are expanded separately, then
combined with Smolyak coefficients and merged.  This makes visible the fact that
the final SGQF cloud is assembled from several smaller tensor rules rather than
chosen directly as one single point set.}
\label{fig:p42-sgqf-cloud-buildup}
\end{figure}

The same story can now be read in the 3D case.  Using
\eqref{eq:p38-A32}, the active rules are
\((1,1,1)\), \((1,1,2)\), \((1,2,1)\), and \((2,1,1)\).  Their coefficients are
\begin{equation}
  c_{(1,1,1)}=-2,
  \qquad
  c_{(1,1,2)}=1,
  ...
  \qquad
  c_{(2,1,1)}=1.
  \label{eq:p38-3d-smolyak-coeffs-repeat}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:p38-3d-smolyak-rule}
\end{equation}
This is the formal equation behind the earlier statement that the 27-point tensor
product is being replaced by a signed combination of one one-point tensor rule
and three one-axis three-point tensor rules.

\subsubsection{Univariate Moment Matching}
...
\end{equation}
The moments on the left are explicit, with the usual convention
\(M_0=1\) and the empty double-factorial product taken to equal one:
\begin{equation}
  M_j=
  \begin{cases}
  0, & j\ \hbox{odd},\\
...
\(\{1/6,2/3,1/6\}\).  In the source SGQ family, however, the level-2 three-point
rule is tunable through the location parameter shown in Jia 2012 Eq. (34)--(35);
this note fixes the Gauss--Hermite specialization \(\hat p_1=\sqrt3\) so that
the worked toy rules, the 3D preview, and the level-2 UKF bridge all use one
explicit three-point reference rule.  The same moment-matching logic extends to
the higher level symmetric rules discussed by
\citet[Section 3.2]{jia2012sparsegrid}.

\paragraph{Which univariate family is fixed in this note?}
The source theorem does not force one unique formula for every level.  What it
requires is a sequence of one-dimensional rules
\(
  \mathcal I=\{I_\ell:\ell\in\mathbb N\}
\)
such that the level-\(\ell\) rule is exact for univariate polynomials of degree
at least \(2\ell-1\) under the standard-normal weight.  Many families can satisfy
that property, and Jia, Xin, and Cheng explicitly allow other point-selection
methods as long as the required exactness is met.

This note therefore makes one additional choice that the source itself does not
force: it fixes the univariate family to the odd-point standard-normal
Gauss--Hermite rules,
\begin{equation}
  ...
  \qquad
  \text{and in general } I_\ell = \text{the } (2\ell-1)\text{-point standard-normal GHQ rule}.
  \label{eq:p41-ghq-family}
\end{equation}
Here ``standard-normal GHQ'' means the Gaussian quadrature rule for the weight
\(\phi(\xi)\); equivalently, it is the classical Gauss--Hermite rule rescaled into
standard-normal coordinates.  Its \(n\)-point member integrates standard-normal
weighted polynomials through degree \(2n-1\) exactly, so choosing
\(n=2\ell-1\) gives a level-\(\ell\) rule that is actually exact through degree
\(4\ell-3\), which is stronger than the source theorem's minimum requirement
\(2\ell-1\).

This is not an accidental choice.  It is a deliberate specialization made for
four reasons.  First, it satisfies the Jia--Xin--Cheng admissibility condition
with margin.  Second, it gives the note one concrete deterministic family rather
than a tunable family left implicit, which is important for the fixed-cloud and
same-scalar contracts.  Third, its first low levels are symmetric odd-point
rules centered at zero, which makes the worked 1D, 2D, and 3D examples easy to
teach and audit.  Fourth, the familiar 1-point / 3-point / 5-point ladder is a
canonical Gaussian-filtering reference family, so using it makes the sparse-grid
construction easier to compare against dense GHQ and against the source's UKF
bridge.

Moment matching is still the right concrete way to see how these low levels are
constructed.  The derivations above for \(I_1\) and \(I_2\) are not separate from
this GHQ-family choice; they are the first two explicit members of the family in
standard-normal coordinates.  One could derive \(I_3\) in the same style by
matching more moments, but the present note does not need the full five-point
formula because its worked sparse-grid examples use only levels 1 and 2.
...
considered.  The 3D preview does the same thing once more: each coordinate still
uses the same one-dimensional rule, but the combination logic changes because
there are now more possible tensor directions.

\subsubsection{From Formula To Point Cloud}
...
The same mechanism works in the 3D preview, only with one more coordinate.  The
active tensor rules are \((1,1,1)\), \((1,1,2)\), \((1,2,1)\), and \((2,1,1)\).
They are not the full level-2 tensor rule \((2,2,2)\); they are the lower-total-index
sparse-grid components whose signed combination replaces that 27-point comparator
at level \(L=2\).  Their explicit tensor points are:
\begin{itemize}[leftmargin=0.28in]
  \item \((1,1,1)\): only \((0,0,0)\), with tensor weight \(1\) and sparse-grid
  coefficient \(-2\),
  \item \((1,1,2)\): \((0,0,-\sqrt3)\), \((0,0,0)\), \((0,0,\sqrt3)\),
  with tensor weights \(1/6\), \(2/3\), \(1/6\),
  \item \((1,2,1)\): \((0,-\sqrt3,0)\), \((0,0,0)\), \((0,\sqrt3,0)\),
  with tensor weights \(1/6\), \(2/3\), \(1/6\),
  \item \((2,1,1)\): \((-\sqrt3,0,0)\), \((0,0,0)\), \((\sqrt3,0,0)\),
  with tensor weights \(1/6\), \(2/3\), \(1/6\).
\end{itemize}
So the origin again appears in every tensor component.  Its accumulated 3D weight
is
\begin{equation}
  -2 + \frac23 + \frac23 + \frac23 = 0.
  \label{eq:p38-3d-origin-merge}
\end{equation}
Thus the center point cancels completely at this level.  The six axis points
survive, each with weight \(1/6\), so the final merged 3D cloud is
\begin{equation}
  (\pm\sqrt3,0,0),
  \qquad
  (0,\pm\sqrt3,0),
  \qquad
  (0,0,\pm\sqrt3).
  \label{eq:p38-3d-final-merged-cloud}
\end{equation}
This is the explicit 3D analogue of the 2D five-point merged cloud: after the
signed combination and duplicate merge are done, the cloud has seven distinct
nodes before zero-weight pruning and six stored nodes after the canceled center
is removed.
...
\subsubsection{Exactness, Point Count, And The UKF Relation}
...
\paragraph{Why this can look like a UKF picture.}
A beginner who has seen a UKF or Unscented Transform diagram should notice that
low-level sparse-grid clouds can look familiar: one often sees a center point and
symmetric off-center points.  That geometric similarity is real and useful for
intuition.  Both methods are deterministic weighted point clouds used to
approximate Gaussian expectations.  But the construction rule is different.
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
  \begin{scope}[shift={(0,0)}]
    ...
    \filldraw[blue] (0,-1.2) circle (2.2pt);
    \node[font=\scriptsize, align=center] at (0,-2.35) {built from active\\tensor rules};
  \end{scope}
  \begin{scope}[shift={(6.0,0)}]
    ...
    \filldraw[red!80!black] (0,-1.2) circle (2.2pt);
    \node[font=\scriptsize, align=center] at (0,-2.35) {usually introduced as\\one direct point set};
  \end{scope}
\end{tikzpicture}
\caption{A safe visual analogy.  The low-level SGQF cloud and a canonical
UKF-style sigma-point sketch can look similar geometrically: both show a center
point plus symmetric axis points.  The similarity is useful for intuition, but it
does not mean the construction is the same.}
\label{fig:p42-sgqf-vs-ukf-2d}
\end{figure}

\paragraph{What is similar and what is different.}
At a high level, SGQF clouds and UKF-style sigma-point sets are similar in three
ways: both are deterministic weighted point clouds, both are centered around the
Gaussian mean, and both are used to approximate Gaussian expectations.  But the
difference in construction is important.  A UKF is usually introduced as one
directly specified sigma-point set with its own location and weighting formulas.
By contrast, SGQF is built indirectly: choose a one-dimensional rule family,
choose sparse-grid level \(L\), determine which tensor rules are active, attach
Smolyak coefficients, and merge repeated point contributions.

\paragraph{When the final cloud can look very UKF-like.}
A further subtlety is worth stating explicitly.  If two methods end with the same
symmetric axis geometry and both are required to match the same second moments,
then their final weights can match.  Consider a symmetric axis cloud in dimension
\(d\): one center point at the origin with weight \(w_0\), and two axis points
\(\pm a e_i\) in each coordinate direction, each carrying the same weight
\(w\).  There are \(2d\) off-center points, so total weight 1 requires
\begin{equation}
  w_0 + 2dw = 1.
  \label{eq:p42-axis-cloud-total-weight}
\end{equation}
Matching variance 1 in each coordinate requires
\begin{equation}
  2wa^2 = 1.
  \label{eq:p42-axis-cloud-second-moment}
\end{equation}
Hence
\begin{equation}
  w = \frac{1}{2a^2},
  \qquad
  w_0 = 1-\frac{d}{a^2}.
  \label{eq:p42-axis-cloud-weights-solved}
\end{equation}
So second-moment accuracy alone does not uniquely determine the whole cloud.  It
only says that once the radius \(a\) is chosen, the weights are determined by
that radius.

In two dimensions, choosing \(a=\sqrt3\) gives
\begin{equation}
  w = \frac16,
  \qquad
  w_0 = \frac13,
  \label{eq:p42-axis-cloud-2d-weights}
\end{equation}
which matches the merged SGQF level-2 five-point cloud.  In three dimensions,
choosing \(a=\sqrt3\) gives
\begin{equation}
  w = \frac16,
  \qquad
  w_0 = 0,
  \label{eq:p42-axis-cloud-3d-weights}
\end{equation}
which matches the low-level SGQF toy cloud after the center cancels.  This
explains why SGQF can look very UKF-like at low level.  But it does \emph{not}
mean the constructions are the same; it only means that the final low-level cloud
can coincide in special symmetric-axis cases.

\subsubsection{A Deterministic Level Ladder Before Failure}
...
\begin{align}
  ...
  \left|
  \widehatell_T^{(L)}(\theta)-\widehatell_T^{(L-1)}(\theta)
  \right|
  &\le \epsilon_\ell
  \quad\hbox{for }L>2
...
\subsubsection{Merged Toy Clouds And Duplicate Merging}
\label{sec:p31-toy-grid}

This section makes the sparse-grid cloud concrete.  Start with the 2D case,
which is the smallest nontrivial merged example, and then record the matching 3D
case explicitly.

\subsubsection{Two-dimensional toy cloud}
...
\subsubsection{Three-dimensional toy cloud}

Now carry the same construction through the concrete 3D case from the previous
section.  Take \(b=3\) and \(L=2\), and reuse the same one-dimensional rules
\(I_1\) and \(I_2\) from \eqref{eq:p31-toy-univariate}.  The active band is
\begin{equation}
  \mathcal A_{3,2}
  =
  \{(1,1,1),(1,1,2),(1,2,1),(2,1,1)\},
  \label{eq:p38-3d-toy-indices}
\end{equation}
with coefficients
\begin{equation}
  c_{(1,1,1)}=-2,
  ...
  \qquad
  c_{(2,1,1)}=1.
  \label{eq:p38-3d-toy-coefficients}
\end{equation}
Before merging, the tensor components contribute:
\begin{itemize}[leftmargin=0.28in]
  \item \((1,1,1)\): the single point \((0,0,0)\), with signed contribution
  \(-2\),
  \item \((1,1,2)\): the three points
  \((0,0,-\sqrt3)\), \((0,0,0)\), \((0,0,\sqrt3)\), with signed weights
  \(1/6\), \(2/3\), \(1/6\),
  \item \((1,2,1)\): the three points
  \((0,-\sqrt3,0)\), \((0,0,0)\), \((0,\sqrt3,0)\), with signed weights
  \(1/6\), \(2/3\), \(1/6\),
  \item \((2,1,1)\): the three points
  \((-\sqrt3,0,0)\), \((0,0,0)\), \((\sqrt3,0,0)\), with signed weights
  \(1/6\), \(2/3\), \(1/6\).
\end{itemize}
The origin \((0,0,0)\) therefore appears in all four tensor components.  Its
accumulated weight is
\begin{equation}
  -2+\frac23+\frac23+\frac23=0.
  \label{eq:p38-3d-origin-weight}
\end{equation}
So the center point cancels completely after duplicate merging.  The six axis
points each appear once and keep weight \(1/6\).  The merged cloud is
\begin{center}
\begin{tabular}{ccc}
\toprule
node \(\xi\) & accumulated weight & source component \\
\midrule
\((\sqrt3,0,0)\) & \(1/6\) & \((2,1,1)\) \\
\(( -\sqrt3,0,0)\) & \(1/6\) & \((2,1,1)\) \\
\((0,\sqrt3,0)\) & \(1/6\) & \((1,2,1)\) \\
...
\begin{equation}
  6\cdot \frac16 = 1.
  \label{eq:p38-3d-weight-total}
\end{equation}
This explicit 3D cloud shows the sparse-grid economy in its final merged form.
The full 27-point tensor-product picture is only a comparator.  The actual
sparse-grid construction at \((b,L)=(3,2)\) first forms the signed combination
of \((1,1,1)\), \((1,1,2)\), \((1,2,1)\), and \((2,1,1)\), producing 10 raw
contributions, then 7 merged distinct nodes, and finally 6 stored nodes after
zero-weight pruning removes the canceled center.  That stored 6-point cloud is
the concrete 3D object the later high-dimensional notation is abstracting.
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
  ...
  \begin{scope}[shift={(6.5,0)}]
    ...
    \draw[gray!20] (-2,0) -- (2,0);
    \draw[gray!20] (0,-2) -- (0,2);
    \node[align=center] at (0,2.35) {Full tensor comparator\\corner coverage included};
    ...
    \filldraw[red!80!black] (0,-1.3) circle (2pt);
    \node[align=center, font=\scriptsize] at (0,-2.45) {schematic view: corners appear,\\so three-way interaction is sampled};
  \end{scope}
\end{tikzpicture}
\caption{A schematic 3D comparison.  A low sparse-grid level keeps center and
axis-type structure, while the full tensor comparator also includes corner
coverage.  For functions driven by genuine three-way products, those corners can
matter substantially.}
\label{fig:p42-sgqf-vs-full-tensor-3d}
\end{figure}

To keep the same test function \eqref{eq:p39-3d-test-function} in view, observe
again that every surviving axis point has value \(F=3\) and the center point has
already canceled.  So the merged-cloud sum is just
\begin{equation}
  \frac16\cdot 3 + \frac16\cdot 3 + \frac16\cdot 3 + \frac16\cdot 3 +
  \frac16\cdot 3 + \frac16\cdot 3 = 3,
  \label{eq:p39-3d-toy-cloud-sum}
\end{equation}
which matches the exact Gaussian expectation for this low-order quadratic test
function.  This is the concrete computational sense in which the sparse-grid
reduction ``works'' in the present example: it reduces the point count
substantially while still preserving the targeted low-order structure.
...
\begin{enumerate}[leftmargin=0.28in]
  ...
  \item form \(v_t\), \(\widehatell_t^{\rm FSGQ}\), \(K_t\), \(m_t\), and \(P_t\),
...
\end{equation}
The declared covariance-factor map therefore gives
\begin{equation}
  C_1^-=C(P_1^-)=\sqrt{1.25}=\frac{\sqrt5}{2}.
...
\(m_1\), \(P_1\), \(\widehatell_1^{\rm FSGQ}\), and
\(\dot\widehatell_1^{\rm FSGQ}\).  This is the compact end-to-end numerical oracle
...
The fixed scalar is
\begin{equation}
  \widehatell_T^{\rm FSGQ}(\theta;\mathcal B)
  =
  \sum_{t=1}^T \widehatell_t^{\rm FSGQ}(\theta;\mathcal B),
  \label{eq:p31-fixed-scalar}
\end{equation}
where every term is computed by
\eqref{eq:p31-pred-placed-points}--\eqref{eq:p31-filter-cov}.  For the value
path, only the value-defining structural subset of \(\mathcal B\) enters this
scalar: cloud choice, one-dimensional rules, merge/prune/order policy,
observation preprocessing, initial-condition policy, factor family, and veto
thresholds.  The derivative-support routines and differentiated initial objects
listed inside \(\mathcal B\) are not extra arguments to the value scalar; they are
the metadata needed to differentiate that same declared scalar without changing
its branch identity.  The same full \(\mathcal B\) must be used for values,
...
\begin{equation}
  ...
  (\mathcal C,\ \tau_{\rm merge},\ \tau_{\rm zero},\ \text{ordering},\
   ...
   \text{symmetrize-then-veto rule},\ C(P) is the Cholesky factor of P,\ \epsilon_S,\epsilon_P,
...
\subsubsection{Analytical Gradient Of The Fixed Scalar}
...
\begin{aligned}
  ...
  (\bar z_t,S_t,C_{xz,t})
  \longmapsto
  (v_t,K_t,\widehatell_t^{\rm FSGQ},m_t,P_t).
\end{aligned}
...
\begin{equation}
\begin{aligned}
  \text{previous factor stage}
  &\Longrightarrow
  \dot C_{t-1},\\
  \text{prediction stage}
  &\Longrightarrow
  \dot\chi_{t-1}^{(r)},\dot a_t^{(r)},\dot m_t^-,\dot P_t^-,\\
  \text{predictive factor stage}
  &\Longrightarrow
  \dot C_t^-,\\
  \text{observation stage}
  &\Longrightarrow
  \dot\chi_t^{(r)},\dot z_t^{(r)},\dot{\bar z}_t,\dot v_t,\dot S_t,\dot C_{xz,t},\\
  \text{score stage}
  &\Longrightarrow
  \dot\widehatell_t^{\rm FSGQ},\\
  \text{propagation stage}
  &\Longrightarrow
  \dot K_t,\dot m_t,\dot P_t.
\end{aligned}
...
\]
The gradient path differentiates that same line.  It is helpful to separate the
one-step derivative into the three pieces that will later reappear in the score
formula:
\begin{equation}
  \partial_i \widehatell_t^{\rm FSGQ}
  =
  -\frac12\,
...
\(\dot\widehatell_t\).  It is needed because \(C_{xz,t}\) forms the gain \(K_t\),
...
\begin{align}
  ...
  &(\dot v_t,\dot S_t)\mapsto \dot\widehatell_t^{\rm FSGQ},
...
path computes a covariance factor by \(P_t=C_tC_t^\top\) or
\(P_t^-=C_t^-(C_t^-)^\top\) on the declared branch, the gradient path computes the matching
...
lower-triangular Cholesky branch \(P=CC^\top\) with the usual Cholesky factor.
...
\(\dot\widehatell_t^{\rm FSGQ}\) in this box are the one-component versions of the
indexed quantities \(\dot m_t^{(i)},\dot P_t^{(i)},\dot C_t^{-(i)},\dot S_t^{(i)},
\dot C_{xz,t}^{(i)}\), and \(\partial_i\widehatell_t^{\rm FSGQ}\) used in the full
...
\(f_\theta,h_\theta,Q_\theta,R_\theta\), the declared lower-triangular covariance-factor
map \(P=CC^\top\) on the chosen branch, the symmetrize-then-veto rule, the eigenvalue thresholds
...
\(\widehatell_T^{\rm FSGQ}\gets 0\), and the running derivative
\(G_i\gets 0\), where \(G_i=\partial_i\widehatell_T^{\rm FSGQ}\) is the
...
equations \eqref{eq:p31-obs-placed-points}--\eqref{eq:p31-cross-cov}, together
with the observation sensitivity equations \eqref{eq:p31-dot-obs-place}--\eqref{eq:p31-dot-Cxz}.
\item Symmetrize the innovation covariance \(S_t\) and, if it fails the declared
positive-definite branch test in \eqref{eq:p32-failure-eigen}, stop and report
an innovation-branch failure.
\item Only on that accepted innovation branch, form the one-step increment
\(\widehatell_t^{\rm FSGQ}\) from \eqref{eq:p31-fsgq-loglik}, solve \(S_tu_t=v_t\),
and then continue with score accumulation and the Gaussian projection update.
...
\begin{equation}
  ...
  =
  \frac{
  \widehatell_T^{\rm FSGQ}(\theta+he_i;\mathcal B)
  -
  \widehatell_T^{\rm FSGQ}(\theta-he_i;\mathcal B)}
  {2h}.
...
saved structural branch record \(\mathfrak B\), that is, the implementation-level
branch-identity record induced by the saved branch context \(\mathcal B\): the
same cloud,
...
Let \(X\sim\calN(0,1)\), \(h_\theta(x)=\theta x\), \(R=1\), and \(y=2\).  Any
...
\begin{equation}
  ...
  \qquad
  X\sim\calN(0,I_2),\qquad R=1,\qquad y=3.
...
\subsubsection{Diagnostics, Accuracy, Memory, And Performance Tests}
...
\subsubsection{Mathematical Test Models}
...
\begin{equation}
  X\sim\calN(0,P),\qquad Y=X^2+\varepsilon,\qquad
  \varepsilon\sim\calN(0,R).
...
Let \(\alpha_j\in\R^d\), \(\beta_j,\gamma_j\in\R\), and
\(B_j\in\R^{d\times d}\).  Use the state-space model
\begin{align}
  X_t&=A_\theta X_{t-1}+\eta_t,
  &
  \eta_t&\sim\calN(0,Q_\theta),
...
\end{align}
with \(j=1,\ldots,m\), \(x_{[m+1]}=x_{[1]}\), and observation-noise vector
\(\varepsilon_t=(\varepsilon_{t,1},\ldots,\varepsilon_{t,m})^\top\sim\calN(0,R_\theta)\)
independent of the process noise and independent across time in the intended
state-space-model sense.  This model has controlled low-order interactions.  It
is designed to test whether moderate sparse-grid levels exploit interaction
sparsity and whether point count, memory, and runtime grow as expected.  Diagnostics should report \(b,L,M_{b,L},T\), model
...
Use the same state-side validation template as Model D, but pair it with a
deliberately all-coordinate observation.  Let
\begin{align}
  X_t&=A_\theta X_{t-1}+\eta_t,
  &
  \eta_t&\sim\calN(0,Q_\theta),
  \label{eq:p32-test-hard-state}\\
  Y_t
  &=
  \delta\prod_{j=1}^b(1+\alpha X_{t,j})+\varepsilon_t,
  &
  \varepsilon_t&\sim\calN(0,r),
  \label{eq:p32-test-hard}
\end{align}
with $\eta_t$ independent across time and $\varepsilon_t$ independent of the
latent state conditional on the model.  This model is not expected to be easy
for a moderate sparse grid.  It probes the failure mode where high-order
coordinate interactions matter.  A good validation report should not hide this
case; it should show when the level ladder or covariance diagnostics reject the
fixed grid.
...
\subsubsection{Large-Scale Report Template}
...
\begin{equation}
  ...
  =
  \begin{cases}
  |\widehatell_T-\ell_T^{\rm Kalman}|, & \hbox{linear-Gaussian test},\\
  |\widehatell_T-\ell_T^{\rm dense}|, & \hbox{small dense quadrature test},\\
...
that operator has the explicit form
\begin{equation}
  \bar z_t^{(M)}
  \approx
  \sum_{r=1}^{M_M} w_{r,m}^{(M)}
...
\end{equation}
The one-weight specialization \(w_{r,m}^{(M)}=w_{r,c}^{(M)}\) covers symmetric
point rules such as CKF-like or fixed sparse-grid clouds.  Standard scaled UKF
comparisons generally require distinct mean and covariance weights, so the
operator must be read with that distinction when UKF is the comparator.  Thus
the decisive question is not whether these methods are all “Gaussian,” but what
kind of moment engine they use and what that engine implies for the scalar and
its derivative.
...
This chapter has addressed a specific scientific problem: high-dimensional
...
What the chapter has constructed is therefore precise.  It has reconstructed the
...
The chapter has also been explicit about what it does \emph{not} claim.  It does
...
criteria the chapter used to distinguish FixedSGQF from its nearest deterministic
...
That is not a defect in the present chapter; it is the explicit boundary that
keeps the proposal honest.

A fair review-panel judgment should therefore be the following.  FixedSGQF is a
coherent, mathematically explicit, self-contained, and mathematically specified
for implementation and audit high-dimensional filtering lane.  It is narrower
than a full non-Gaussian density approximation, but that narrowness buys a
transparent deterministic surrogate and a branch-conditioned analytical gradient
that can be audited, implemented, and checked directly.  The approval claim
remains bounded: this chapter is not asserting that FixedSGQF is the best
nonlinear filter overall, but that it is the selected deterministic
gaussian-surrogate lane for the explicit approximate likelihood-and-gradient
objective developed here.  On that basis, this chapter provides a coherent and
auditable basis for further implementation and validation review of FixedSGQF as
one serious lane in a broader high-dimensional filtering program.

\section{Defects Passed To Synthesis}
\label{sec:bf-hd-gq-defects}

This chapter passes four defects to the synthesis chapter.

\begin{enumerate}[label=(\arabic*)]
  \item The carried object is one Gaussian moment pair.  Higher-order or
  sparse-grid exactness in the Gaussian-weighted moment engine does not by
  itself preserve multimodal or strongly non-Gaussian posterior shape.
  \item Deterministic point-rule comparisons depend on the declared moment
  engine, including any distinction between mean and covariance weights; a
  geometric resemblance to UKF/CKF pictures is not itself an identity of
  filtering rules.
  \item The same-scalar derivative contract freezes branch-defining structure,
  not parameter-dependent numerical values.  If the cloud, factor family,
  thresholds, or initial-condition policy change, the scalar has changed.
  \item Sparse-grid viability is local to declared active dimension, level, and
  smoothness.  Without those bounds, improved Gaussian moment accuracy can still
  buy an unaffordable global rule.
\end{enumerate}
...
\midrule
\endhead
Gaussian moment pair & filtering law and normalizer contribution & one carried
Gaussian and deterministic moment closure & projection bias or lost posterior
shape & innovation, PSD, comparator residual, active-dimension check & block
state dimension and point count & projection limits named &
Sections~\ref{sec:p31-gaussian-projection} and \ref{sec:p31-comparison} \\
Sparse-grid moment engine & Gaussian-weighted nonlinear moments & declared point
rule, merge policy, active level & point explosion or wrong rule-family
comparison & point count, merge semantics, weight-family audit &
\(M_{b,L}\) / active dimension & local rule contract explicit &
Sections~\ref{sec:p31-sgq-reconstruction} and \ref{sec:p31-comparison} \\
Same-scalar surrogate & approximate likelihood scalar
\(\widehatell_T^{\rm FSGQ}\) & cloud, factor branch, thresholds, initial-condition
policy & changed scalar under branch drift & finite-difference parity, branch
identity, veto pattern & gradient-through-filter cost & branch contract passes &
Sections~\ref{sec:p31-same-scalar} and \ref{sec:p31-gradient} \\
\bottomrule
\end{longtable}
\endgroup

Exported forward: Chapter~\ref{ch:bf-highdim-particle-transport-tensor} takes up
the richer non-Gaussian carried objects needed when one Gaussian is too narrow;
Chapter~\ref{ch:bf-highdim-hmc-research} may use only the declared approximate
scalar and same-scalar derivative consequences stated here; and
Chapter~\ref{ch:bf-highdim-candidate-synthesis} may consume only the exported
projection, rule-family, and branch-contract defects named above.
...
The full chapter-level routine has inputs
\begin{equation}
\begin{aligned}
  \mathcal I_{\rm in}
  =
  (&y_{1:T},\theta,L,\{I_\ell\}_{\ell=1}^{L},
  m_0(\theta),P_0(\theta),\\
...
\begin{equation}
  \left(
  \widehatell_T^{\rm FSGQ}(\theta),
  \nabla_\theta\widehatell_T^{\rm FSGQ}(\theta),
  \{m_t,P_t\}_{t=0}^T,
  \mathcal D_{\rm diag}
  \right)
...
\begin{aligned}
  ...
  &\longmapsto (v_t,\widehatell_t^{\rm FSGQ},K_t)
  \longmapsto (m_t,P_t).
\end{aligned}
...
\(\epsilon_\ell\) & \(10^{-3}\max(1,|\widehatell_T|)\) & pilot level-ladder change threshold \\
...
\item Initialize \(\widehatell_T^{\rm FSGQ}\gets0\),
...
\item Compute \(\bar z_t,\Delta_t^{(r)},S_t,C_{xz,t},v_t\) by
\eqref{eq:p31-obs-mean}--\eqref{eq:p31-innovation}.  Symmetrize \(S_t\).
If \(S_t\) fails \eqref{eq:p32-failure-eigen}, return an innovation
covariance failure.
\item Only on that accepted innovation branch, form
\(\widehatell_t^{\rm FSGQ}\) by \eqref{eq:p31-fsgq-loglik}.
\item Compute \(\dot{\bar z}_t^{(i)},\dot v_t^{(i)},\dot S_t^{(i)}\), and
\(\dot C_{xz,t}^{(i)}\) by \eqref{eq:p31-dot-zbar}--\eqref{eq:p31-dot-Cxz}.
\item Solve \(S_tu_t=v_t\).  Add \(\widehatell_t^{\rm FSGQ}\) to
\(\widehatell_T^{\rm FSGQ}\) only after the innovation branch has passed, and add
...
\item Return \((\widehatell_T^{\rm FSGQ},G_{1:p},\{m_t,P_t\},\mathcal D_{\rm diag})\).
\end{enumerate}
\end{minipage}}
...
For implementation review, the value path can be located in the chapter as
follows.
\begin{center}
\small
...
\toprule
construction & support & use in this chapter \\
\midrule
...
one-dimensional and tensor-product Gaussian quadrature, with the present note fixing a GHQ-specialized odd-point rule family & \citet[Section 2.2.1 and Eq. 34--35]{jia2012sparsegrid} & Section~\ref{sec:p31-ghq} \\
...
For implementation review, the chapter can be navigated by the following inventory.\begin{center}
\small
...
exact object versus approximate object & \eqref{eq:p32-exact-vs-approximate-summary}, this chapter \\
