...
\end{equation}
with fixed expansion factor \(\gamma\).  The out-of-domain diagnostic at fitting
or proposal points is
\begin{equation}
  \delta_{\rm out}=
...
useful baseline is constant-channel initialization: initialize every core to zero
except the constant channel, and let one bidirectional sweep mean updates in the
order
\begin{equation}
