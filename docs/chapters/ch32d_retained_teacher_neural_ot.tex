...
\subsection{Retained-teacher approximation family, not one architecture}
\label{sec:bf-learned-ot-retained-family}

The scalable-OT survey suggests broadening the reader's orientation here before
committing to one neural warm-start architecture.  The retained-teacher lane is
best understood as a family of approximations that all try to preserve a declared
entropic teacher object while reducing the cost of reaching or applying it.
Warm-start predictors are the narrowest member of that family, but they are not
the only one.

Representative retained-teacher approximation routes include:
\begin{enumerate}[label=(\roman*)]
  \item learned warm starts for dual or scaling variables,
  \item Nystrom kernel factorizations that approximate the Gibbs kernel while
    keeping a teacher-style scaling structure,
  \item positive-feature Sinkhorn variants that replace the dense kernel by a
    positive low-rank feature factorization,
  \item direct low-rank transport factorizations when the resulting transported
    object is still audited against the teacher output,
  \item stochastic or minibatch routes used only as teacher-side approximations
    rather than silently promoted to full replacement resampling rules.
\end{enumerate}
The common doctrine is that these routes should still be judged first by how well
they preserve the declared teacher coupling or teacher barycentric output.  They
are not automatically semantics-preserving merely because they are faster than a
dense Sinkhorn pass.  Their status depends on whether the approximation changes
only the execution path, the kernel/coupling family, or the final transported
object itself.
