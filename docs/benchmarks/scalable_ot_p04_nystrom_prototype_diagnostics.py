"""Phase 4 diagnostics for the fixed-rank Nystrom transport prototype."""
...
VIABILITY_MAX_ERROR_THRESHOLD = 5.0e-2
VIABILITY_RMS_ERROR_THRESHOLD = 2.0e-2
PROMOTION_FIXTURES = ("tiny_manual", "small_parity", "high_dim_low_rank")
NONCLAIMS = (
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--nystrom-max-iterations", type=int, default=120)
    ...
    parser.add_argument("--ranks", default="full")
...
def _rank_list(spec: str, num_particles: int) -> list[int]:
    if spec == "full":
...
def _run_nystrom(
    ...
    *,
    ...
    record = {
        ...
        "dense_reference_max_abs_particle_error": _float(max_error),
        "dense_reference_rms_particle_error": _float(rms_error),
...
def _candidate_schema_record(result: dict[str, Any]) -> dict[str, Any]:
    ...
    record = CandidateResultRecord(
        candidate_id="phase4_fixed_rank_nystrom_transport",
        ...
        transport_object=TransportObjectRecord(
            ...
            materialized=False,
            factor_shapes={
                "V": [max_particles, max_rank],
                "A_inv": [max_rank, max_rank],
        ...
        diagnostics={
            ...
            "viability_pass": result["viability_pass"],
            "max_row_residual": result["summary"]["max_row_residual"],
            "max_column_residual": result["summary"]["max_column_residual"],
            "max_dense_reference_particle_error": result["summary"]["max_dense_reference_particle_error"],
            "max_dense_reference_rms_error": result["summary"]["max_dense_reference_rms_error"],
        ...
        diagnostic_roles={
            ...
            "viability_pass": "promotion_criterion",
            "max_dense_reference_particle_error": "promotion_criterion",
            "max_dense_reference_rms_error": "promotion_criterion",
...
def _build_result(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for fixture_name, (particles_np, log_weights_np) in _fixtures().items():
        ...
        ranks = _rank_list(args.ranks, particles_np.shape[1])
        nystrom_rows = [
            _run_nystrom(
                particles_np,
                log_weights_np,
    ...
    for fixture_name, fixture in fixture_results.items():
        ...
        for row in fixture["nystrom"]:
            if not row["validity_pass"]:
                hard_vetoes.append(f"{fixture_name}:rank_{row['rank']}:validity_failed")
    validity_pass = not hard_vetoes
    viability_by_fixture = {}
    for fixture_name in PROMOTION_FIXTURES:
        rows = fixture_results[fixture_name]["nystrom"]
        viability_by_fixture[fixture_name] = any(
            row["validity_pass"]
            and row["dense_reference_max_abs_particle_error"] <= VIABILITY_MAX_ERROR_THRESHOLD
            and row["dense_reference_rms_particle_error"] <= VIABILITY_RMS_ERROR_THRESHOLD
            for row in rows
        )
    viability_pass = all(viability_by_fixture.values())
    if not validity_pass:
        phase4_status = "PHASE_4_NYSTROM_PROTOTYPE_FAILED_HARD_VETO"
    elif viability_pass:
        phase4_status = "PHASE_4_NYSTROM_PROTOTYPE_PASSED"
    else:
        phase4_status = "PHASE_4_NYSTROM_PROTOTYPE_COMPLETED_CANDIDATE_NOT_PROMOTED"
    summary = {
    ...
    manifest = {
        ...
        "device": args.device,
        "dtype": "tf.float64",
        "command": "scalable_ot_p04_nystrom_prototype_diagnostics.py",
        "plan_path": "docs/plans/bayesfilter-dpf-ledh-pfpf-ot-scalable-ot-p04-nystrom-prototype-subplan-2026-06-17.md",
    }
    result: dict[str, Any] = {
        ...
        "rank_scope": (
            ...
            if args.ranks == "full"
            else "declared_fixed_rank_probe"
        ),
        "thresholds": {
...
def _write_markdown(result: dict[str, Any], path: Path) -> None:
    lines = [
        ...
        f"- Phase 4 status: `{result['phase4_status']}`",
        f"- Validity pass: `{result['validity_pass']}`",
        f"- Viability pass: `{result['viability_pass']}`",
        f"- Rank scope: `{result['rank_scope']}`",
    ...
    for fixture_name, fixture in result["fixtures"].items():
        for row in fixture["nystrom"]:
            lines.append(
                "| {fixture} | {rank} | `{valid}` | `{row_res:.6e}` | `{col_res:.6e}` | `{max_err:.6e}` | `{rms_err:.6e}` |".format(
