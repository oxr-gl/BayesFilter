"""Probe zero-init corrected retained-Sinkhorn budgets on Austria SIR d18 teacher data."""
...
INPUT_JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_teacher_data_austria_sir_d18_2026-07-03.json"
JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_zero_init_budget_probe_austria_sir_d18_2026-07-03.json"
REPORT_PATH = REPORT_DIR / "retained-teacher-sinkhorn-zero-init-budget-probe-austria-sir-d18-2026-07-03.md"
RESULT_PATH = "docs/plans/bayesfilter-neural-ot-austria-sir-d18-retained-teacher-zero-init-probe-result-2026-07-03.md"
PLAN_PATH = "docs/plans/bayesfilter-neural-ot-austria-sir-d18-retained-teacher-smoke-test-plan-2026-07-03.md"
SCALAR_ID = "retained_teacher_sinkhorn_zero_init_budget_probe_austria_sir_d18_tf"
...
NO_RUNG_DECISION = "NO_USABLE_AUSTRIA_SIR_D18_RANKING_RUNG_UNDER_CURRENT_ARTIFACT"
DISCRIMINATING_DECISION = "AUSTRIA_SIR_D18_ZERO_INIT_DISCRIMINATING_BUDGETS_IDENTIFIED"
...
def _run() -> dict[str, Any]:
    ...
    if not heldout_examples:
        raise RuntimeError("Austria SIR d18 teacher-data artifact must include heldout examples")
    ...
    payload = {
        "decision": decision,
        "question": "Does the current Austria SIR d18 retained-teacher artifact contain any discriminating zero-init corrective budget before saturation, or must the artifact be amended before further ranking claims?",
        ...
        "run_manifest": environment_manifest(
            command="CUDA_VISIBLE_DEVICES=-1 python -m experiments.dpf_implementation.tf_tfp.runners.run_retained_teacher_sinkhorn_zero_init_budget_probe_austria_sir_d18_tf",
...
def _interpretation_text(payload: dict[str, Any]) -> str:
    if payload["decision"] == DISCRIMINATING_DECISION:
        return (
            "The current Austria SIR d18 artifact furnishes a governed discriminating rung. "
            "Future donor-aligned replay evaluation should be bound to the recommended primary budgets "
            "rather than reusing any saturated higher-budget comparison as if it were informative."
        )
    return (
        "The current Austria SIR d18 artifact has no usable discriminating ranking rung under the "
        "zero-init probe ladder. Further retained-teacher claims therefore require artifact or ladder "
        "calibration before replay evaluation can be interpreted."
    )
