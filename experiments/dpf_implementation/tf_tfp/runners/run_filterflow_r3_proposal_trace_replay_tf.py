"""R3 proposal-stream trace/replay against executable filterflow.
...
PLAN_PATH = "docs/plans/bayesfilter-dpf-filterflow-r3-proposal-trace-replay-plan-2026-06-02.md"
RESULT_PATH = "docs/plans/bayesfilter-dpf-filterflow-r3-proposal-trace-replay-result-2026-06-02.md"
JSON_PATH = OUTPUT_DIR / "dpf_filterflow_r3_proposal_trace_replay_2026-06-02.json"
REPORT_PATH = REPORT_DIR / "dpf-filterflow-r3-proposal-trace-replay-2026-06-02.md"
...
def _run() -> dict[str, Any]:
    ...
    if filterflow_trace.get("status") != "executed":
        return _blocked_payload(
            "filterflow_r3_trace_replay_filterflow_blocker",
    ...
    if not filterflow_trace["trace_validation"]["official_trace_match"]:
        return _blocked_payload(
            "filterflow_r3_trace_replay_trace_validation_failed",
    ...
    return {
        ...
        "run_manifest": environment_manifest(
            command=(
                ...
                "run_filterflow_r3_proposal_trace_replay_tf"
            ),
...
def _filterflow_trace_script() -> str:
    return textwrap.dedent(
        ...
        transition_matrix_np = np.array([[1.0, 1.0], [0.0, 1.0]], dtype=np.float32)
        observation_matrix_np = np.array([[1.0, 0.0]], dtype=np.float32)
        transition_covariance_np = np.array([[1.0 / 3.0, 0.5], [0.5, 1.0]], dtype=np.float32)
        observation_covariance_np = np.array([[0.01]], dtype=np.float32)
        ...
        initial_particles_np = np_random_state.normal(0.0, 0.01, [1, N, 2]).astype(np.float32)
        ...
        observations = tf.convert_to_tensor(observations_np, dtype=tf.float32)
        ...
        payload = {{
            "status": "executed",
            "trace_contract": "external_eager_loop_reproduces_official_filterflow_before_replay",
            "trace_validation": {{
...
def _bayesfilter_replay(
    ...
    *,
    ...
    try:
        ...
        transition_matrix = tf.constant(model["transition_matrix"], dtype=tf.float32)
        observation_matrix = tf.constant(model["observation_matrix"], dtype=tf.float32)
        transition_chol = tf.constant(model["transition_covariance_chol"], dtype=tf.float32)
        observation_chol = tf.constant(model["observation_covariance_chol"], dtype=tf.float32)
        observations = tf.constant(model["observations"], dtype=tf.float32)
        particles = tf.constant(model["initial_particles"], dtype=tf.float32)
        num_particles = int(model["num_particles"])
        log_weights = tf.fill(
            [1, num_particles],
            -tf.math.log(tf.cast(num_particles, tf.float32)),
        )
        log_likelihoods = tf.zeros([1], dtype=tf.float32)
        ...
        transition_cov_inv = tf.linalg.cholesky_solve(transition_chol, tf.eye(2, dtype=tf.float32))
        observation_cov_inv = tf.linalg.cholesky_solve(observation_chol, tf.eye(1, dtype=tf.float32))
        ...
        for time_index, row in enumerate(filterflow_trace["trace_rows"]):
            if replay_mode in {
                ...
                particles = tf.constant(row["pre_particles"], dtype=tf.float32)
                log_weights = tf.constant(row["pre_log_weights"], dtype=tf.float32)
            ess = _ess(log_weights)
            flags = ess <= tf.math.log(
                tf.cast(num_particles, tf.float32) * tf.constant(RESAMPLING_NEFF, tf.float32)
            )
            bool_flags = tf.reshape(flags, [-1])
            if replay_mode == "traced_resampling_state":
                particles = tf.constant(row["post_resample_particles"], dtype=tf.float32)
                log_weights = tf.constant(row["post_resample_log_weights"], dtype=tf.float32)
                ...
                if bool(tf.reduce_any(bool_flags).numpy()):
                    transport_matrix = tf.constant(row["transport_matrix"], dtype=tf.float32)
                ...
                transport_matrix = tf.cast(transported.transport_matrix, tf.float32)
            ...
            proposed_particles = tf.constant(row["proposal_particles"], dtype=tf.float32)
            observation = observations[time_index]
...
def _filterflow_normalize(log_weights: tf.Tensor, num_particles: int) -> tf.Tensor:
    ...
    float_n = tf.cast(num_particles, tf.float32)
    stop_gradient_mask = normalized < tf.maximum(
        tf.constant(-13.8, dtype=tf.float32),
        tf.constant(-4.0, dtype=tf.float32) * float_n,
    )
    mask = tf.cast(stop_gradient_mask, tf.float32)
...
def _decision(
    ...
    if comparator_drift:
        return "filterflow_r3_trace_replay_blocked_by_comparator_drift"
    if (
        ...
        return "filterflow_r3_trace_replay_blocked"
    if (
        ...
        return "filterflow_r3_trace_replay_nonfinite_veto"
    if computed_comparison.get("implementation_agreement"):
        return "filterflow_r3_trace_replay_pass"
    if traced_input_computed_comparison.get("implementation_agreement"):
        return "filterflow_r3_trace_replay_accumulated_roundoff_localized"
    if traced_transport_comparison.get("implementation_agreement"):
        return "filterflow_r3_trace_replay_transport_matrix_delta_localized"
    if traced_comparison.get("implementation_agreement"):
        return "filterflow_r3_trace_replay_resampling_state_delta_localized"
    return "filterflow_r3_trace_replay_mismatch"
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    allowed = {
        "filterflow_r3_trace_replay_filterflow_blocker",
        "filterflow_r3_trace_replay_trace_validation_failed",
        "filterflow_r3_trace_replay_blocked_by_comparator_drift",
        "filterflow_r3_trace_replay_blocked",
        "filterflow_r3_trace_replay_nonfinite_veto",
        "filterflow_r3_trace_replay_pass",
        "filterflow_r3_trace_replay_accumulated_roundoff_localized",
        "filterflow_r3_trace_replay_transport_matrix_delta_localized",
        "filterflow_r3_trace_replay_resampling_state_delta_localized",
        "filterflow_r3_trace_replay_mismatch",
    }
    ...
    if trace.get("status") == "executed":
        ...
        if not trace["trace_validation"]["official_trace_match"]:
            if payload["decision"] != "filterflow_r3_trace_replay_trace_validation_failed":
                raise ValueError("trace mismatch without trace-validation decision")
            return
    if payload["decision"] == "filterflow_r3_trace_replay_pass":
        if not payload["comparison"].get("implementation_agreement"):
            raise ValueError("pass decision without implementation agreement")
    if payload["decision"] == "filterflow_r3_trace_replay_resampling_state_delta_localized":
    ...
    if payload["decision"] == "filterflow_r3_trace_replay_accumulated_roundoff_localized":
        if payload["computed_resampling_comparison"].get("implementation_agreement"):
    ...
    if payload["decision"] == "filterflow_r3_trace_replay_transport_matrix_delta_localized":
...
def _markdown(payload: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        _json_block(payload["computed_resampling_comparison"]),
        "",
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if decision == "filterflow_r3_trace_replay_pass":
        return "R3 is cleared for this bounded trace/replay scenario."
    if decision == "filterflow_r3_trace_replay_resampling_state_delta_localized":
        return (
    ...
    if decision == "filterflow_r3_trace_replay_accumulated_roundoff_localized":
        return (
    ...
    if decision == "filterflow_r3_trace_replay_transport_matrix_delta_localized":
        return (
    ...
    if decision == "filterflow_r3_trace_replay_trace_validation_failed":
        return "The non-mutating trace loop did not reproduce official filterflow, so replay evidence is blocked."
    if decision == "filterflow_r3_trace_replay_mismatch":
