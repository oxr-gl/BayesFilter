"""Run v2 fixed-noise fixed-ancestor path tie-outs."""
...
DTYPE = tf.float64
PLAN_PATH = "docs/plans/bayesfilter-dpf-common-model-suite-v2-production-p4-fixed-ancestor-paths-subplan-2026-06-07.md"
RESULT_PATH = "docs/plans/bayesfilter-dpf-common-model-suite-v2-production-p4-fixed-ancestor-paths-result-2026-06-07.md"
...
P3_JSON_PATH = OUTPUT_DIR / "dpf_common_model_suite_v2_noresampling_2026-06-07.json"
...
JSON_PATH = OUTPUT_DIR / "dpf_common_model_suite_v2_fixed_resampling_2026-06-07.json"
REPORT_PATH = REPORT_DIR / "dpf-common-model-suite-v2-fixed-resampling-2026-06-07.md"
...
def _run() -> dict[str, Any]:
    ...
    _preflight_prior_artifacts(p1_manifest, p2_payload, p3_payload)
    filterflow_status = _filterflow_checkout_manifest()
    specs = common_model_specs_v2()
    ready_ids = _ready_ids_for_phase(p1_manifest, "P4_fixed_ancestor", "READY_FOR_P4")
    ...
    for spec in specs:
        if spec.model_id not in ready_ids:
            cells.append(_classified_cell(spec, "CONTRACT_BLOCKED", "row not READY_FOR_P4 in P1 classification"))
            continue
    ...
    return {
        ...
        "question": (
            ...
            "match deterministic fixed-noise fixed-ancestor filter-path values "
        ...
        "primary_criterion_fields": {
            ...
            "all_executed_rows_used_expected_resampling_count": all(
                cell.get("bayesfilter", {}).get("resampling_count") == 1
                and cell.get("filterflow", {}).get("resampling_count") == 1
                for cell in cells
                if cell["status"] != "CONTRACT_BLOCKED"
            ),
        },
        "veto_diagnostics": {
        ...
        "explanatory_only_fields": {
            ...
            "p3_noresampling_summary": p3_payload.get("summary"),
        },
        "cells": cells,
        ...
        "next_allowed_action": "run Claude P4 result review before P5",
        "artifact_paths": {
        ...
        "run_manifest": environment_manifest(
            command=(
                ...
                "experiments.dpf_implementation.tf_tfp.runners.run_common_model_suite_v2_fixed_resampling_tf"
            ),
...
def _path_contract(spec: CommonModelSpecV2) -> dict[str, Any]:
    contract = dict(spec.fixed_ancestor_contract)
    payload = {
        ...
        "resampling_policy": contract["resampling_policy"],
        "resampling_flags": list(contract["resampling_flags"]),
        "fixed_ancestor_indices": list(contract["fixed_ancestor_indices"]),
        "expected_resampling_count": int(contract["expected_resampling_count"]),
...
def _bayesfilter_path(spec: CommonModelSpecV2, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(int(contract["horizon"])):
        ...
        resampling_applied = bool(contract["resampling_flags"][step])
        ...
        previous_particles = post_particles
    ...
    return {
        "status": "executed",
        "backend": "bayesfilter_v2_fixed_ancestor_resampling",
        "model_id": spec.model_id,
...
def _filterflow_path_subprocess(contracts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        return {
            "status": "blocked",
            "blocker": "filterflow v2 fixed-resampling subprocess failed",
            "returncode": completed.returncode,
            "stdout_excerpt": completed.stdout[-4000:],
            "stderr_excerpt": completed.stderr[-4000:],
            "cells": [],
        }
    start = completed.stdout.rfind("FILTERFLOW_COMMON_MODEL_SUITE_V2_FIXED_RESAMPLING_JSON_BEGIN")
    end = completed.stdout.rfind("FILTERFLOW_COMMON_MODEL_SUITE_V2_FIXED_RESAMPLING_JSON_END")
    if start < 0 or end < 0 or end <= start:
        return {
            "status": "blocked",
            "blocker": "filterflow v2 fixed-resampling JSON sentinels missing",
            "stdout_excerpt": completed.stdout[-4000:],
            "stderr_excerpt": completed.stderr[-4000:],
            "cells": [],
        }
    raw = completed.stdout[start + len("FILTERFLOW_COMMON_MODEL_SUITE_V2_FIXED_RESAMPLING_JSON_BEGIN") : end].strip()
...
def _filterflow_script(contracts: list[dict[str, Any]]) -> str:
    ...
    return textwrap.dedent(
        ...
        def step_ledger(step, pre_resampling_particles, pre_resampling_log_weights,
                        resampling_applied, ancestor_indices, post_resampling_particles,
                        post_resampling_log_weights, previous_particles,
                        transition_innovations, predicted_particles,
            ...
            return {{
                "step": int(step),
                "pre_resampling_particles": to_json(pre_resampling_particles),
                "pre_resampling_log_weights": to_json(pre_resampling_log_weights),
                "pre_resampling_weights": to_json(tf.exp(pre_resampling_log_weights)),
                "resampling_applied": bool(resampling_applied),
                "ancestor_indices": [int(x) for x in tf.cast(ancestor_indices, tf.int32).numpy().tolist()],
                "post_resampling_particles": to_json(post_resampling_particles),
                "post_resampling_log_weights": to_json(post_resampling_log_weights),
                "post_resampling_weights": to_json(tf.exp(post_resampling_log_weights)),
        ...
        def run_contract(contract):
            ...
            for step in range(int(contract["horizon"])):
                ...
                ledger.append(step_ledger(
                    step, pre_particles, pre_log_weights, resampling_applied,
                    ancestors, post_particles, post_log_weights, previous,
                    innovations[step], predicted, observations[step],
            ...
            return {{
                "status": "executed",
                "backend": "filterflow_env_local_v2_fixed_ancestor_resampling",
                "model_id": contract["model_id"],
                "scalar": scalar(total),
                "finite": finite,
                "resampling_count": int(resampling_count),
        ...
        print("FILTERFLOW_COMMON_MODEL_SUITE_V2_FIXED_RESAMPLING_JSON_BEGIN")
        print(json.dumps({{"status": "executed", "cells": cells}}, sort_keys=True))
        print("FILTERFLOW_COMMON_MODEL_SUITE_V2_FIXED_RESAMPLING_JSON_END")
        """
    )
...
def _cell(spec: CommonModelSpecV2, contract: dict[str, Any], bayesfilter: dict[str, Any], filterflow: dict[str, Any]) -> dict[str, Any]:
    if filterflow.get("status") != "executed":
        return {
            ...
            "cell_type": "v2_fixed_ancestor_resampling_path",
            "status": "INTERFACE_BLOCKED",
            "decision": f"{spec.model_id}_filterflow_fixed_resampling_adapter_blocked",
            "primary_criterion": "both adapters must execute the same deterministic fixed-noise fixed-ancestor path",
            "metrics": {},
            "mismatch_class": "filterflow_v2_fixed_resampling_adapter_blocked",
    ...
    matched = (
        ...
        and bayesfilter["resampling_count"] == contract["expected_resampling_count"]
        and filterflow["resampling_count"] == contract["expected_resampling_count"]
    )
    return {
        "model": spec.model_id,
        "family": spec.family,
        ...
        "decision": f"{spec.model_id}_fixed_resampling_path_matched" if matched else f"{spec.model_id}_fixed_resampling_path_mismatch",
        "primary_criterion": "scalar and primary path ledger fields match within tolerance",
        "metrics": comparison,
        "mismatch_class": None if matched else "v2_fixed_resampling_path_delta",
        "contract": contract,
        "bayesfilter": bayesfilter,
        "filterflow": filterflow,
        "spec_checksum": spec.checksum(),
        "non_claim": "fixed-noise fixed-ancestor path agreement is not stochastic resampling correctness",
    }
...
def _compare_path_payloads(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    ...
    for left_step, right_step in zip(left["ledger"], right["ledger"], strict=True):
        ...
        for field in fields:
            if field == "resampling_applied":
                field_max = 0.0 if bool(left_step[field]) == bool(right_step[field]) else float("inf")
            else:
                dtype = tf.int32 if field == "ancestor_indices" else DTYPE
                ...
                delta = tf.abs(tf.cast(left_value, DTYPE) - tf.cast(right_value, DTYPE))
...
def _step_ledger(
    *,
    ...
    return {
        ...
        "pre_resampling_weights": tensor_to_json(tf.exp(pre_resampling_log_weights)),
        "resampling_applied": bool(resampling_applied),
        "ancestor_indices": [int(value) for value in tf.cast(ancestor_indices, tf.int32).numpy().tolist()],
        ...
        "post_resampling_weights": tensor_to_json(tf.exp(post_resampling_log_weights)),
...
def _filterflow_cell(payload: dict[str, Any], model_id: str) -> dict[str, Any]:
    for cell in payload.get("cells", []):
        if cell.get("model_id") == model_id:
            return cell
    return {"status": "missing", "blocker": f"missing FilterFlow v2 fixed-resampling cell for {model_id}"}
...
def _preflight_prior_artifacts(
    ...
    p3_ids = [cell.get("model") for cell in p3_payload.get("cells", [])]
    ...
    if tuple(p3_ids) != EXPECTED_V2_MODEL_IDS:
        raise ValueError(f"P3 no-resampling cell id gate failed: {p3_ids}")
    ...
    if not P2_RESULT_PATH.read_text(encoding="utf-8").splitlines()[4].endswith("PASS_P2_DENSITY_READY_FOR_P3"):
        raise ValueError("P2 result ledger is not marked PASS_P2_DENSITY_READY_FOR_P3")
    if p3_payload.get("decision") not in {"PENDING_CLAUDE_REVIEW", "PASS_P3_NORESAMPLING_READY_FOR_P4"}:
        raise ValueError(f"P3 no-resampling phase is not passable: {p3_payload.get('decision')}")
    if not P3_RESULT_PATH.read_text(encoding="utf-8").splitlines()[4].endswith("PASS_P3_NORESAMPLING_READY_FOR_P4"):
        raise ValueError("P3 result ledger is not marked PASS_P3_NORESAMPLING_READY_FOR_P4")
...
def _decision(cells: list[dict[str, Any]]) -> str:
    ...
    if any(cell["status"] == "INTERFACE_BLOCKED" for cell in cells):
        return "P4_INTERFACE_BLOCKED"
    return "P4_CLASSIFIED_MISMATCH_PENDING_REVIEW"
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["decision"] not in {"PENDING_CLAUDE_REVIEW", "PASS_P4_FIXED_RESAMPLING_READY_FOR_P5"}:
        raise ValueError(f"P4 payload decision not passable: {payload['decision']}")
    ...
    if payload["veto_diagnostics"].get("missing_filterflow_subprocess_environment"):
        raise ValueError("P4 missing FilterFlow subprocess environment")
    if payload["veto_diagnostics"].get("nonfinite_path_value"):
        raise ValueError("P4 nonfinite path value")
    ready_ids = set(payload["primary_criterion_fields"]["ready_model_ids"])
    for cell in payload["cells"]:
        if cell["model"] in ready_ids and cell["status"] != "MATCHED":
            raise ValueError(f"P4 ready row did not match: {cell['model']} {cell['status']}")
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- expected resampling count: `{payload['primary_criterion_fields']['expected_resampling_count']}`",
            f"- all executed rows used expected resampling count: `{payload['primary_criterion_fields']['all_executed_rows_used_expected_resampling_count']}`",
            "",
            "## Veto Diagnostics",
            "",
        ]
    ...
    lines.extend(
        [
            ...
            f"| validation commands | `python -m json.tool {payload['artifact_paths']['json']}`; `CUDA_VISIBLE_DEVICES=-1 MPLCONFIGDIR=/tmp/bayesfilter-dpf-mpl python -m experiments.dpf_implementation.tf_tfp.runners.run_common_model_suite_v2_fixed_resampling_tf --validate-only`; `git diff --check` on P4 files |",
            ...
            "## Repair History",
            "",
            "- No P4 repair has been required before Claude result review.",
            "",
            "## Decision Table",
            "",
            "| Decision | Primary criterion | Veto status | Main uncertainty | Next action | Not concluded |",
            "|---|---|---|---|---|---|",
            "| PENDING_CLAUDE_REVIEW | all P1-ready rows matched on deterministic fixed-ancestor branch replay fields locally | no local veto fired | Claude may identify branch-contract or adapter-governance gaps | run Claude P4 result review | no stochastic resampling distribution, gradient, filter correctness, or student claim |",
            "",
            "## Post-Run Red Team",
            "",
