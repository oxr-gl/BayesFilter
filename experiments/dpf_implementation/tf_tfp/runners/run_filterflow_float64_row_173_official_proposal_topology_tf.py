"""Probe row-173 official proposal-likelihood topology at the first divergent time."""
...
PLAN_PATH = (
    "docs/plans/"
    "bayesfilter-dpf-filterflow-float64-row-173-official-proposal-topology-plan-2026-06-05.md"
)
RESULT_PATH = (
    "docs/plans/"
    "bayesfilter-dpf-filterflow-float64-row-173-official-proposal-topology-result-2026-06-05.md"
)
JSON_PATH = (
    OUTPUT_DIR / "dpf_filterflow_float64_row_173_official_proposal_topology_2026-06-05.json"
)
REPORT_PATH = (
    REPORT_DIR / "dpf-filterflow-float64-row-173-official-proposal-topology-2026-06-05.md"
)
...
def _run() -> dict[str, Any]:
    ...
    if filterflow.get("status") != "executed":
        return _blocked_payload(
            "filterflow_float64_row_173_official_proposal_topology_filterflow_blocker",
    ...
    if bayesfilter.get("status") != "executed":
        return _blocked_payload(
            "filterflow_float64_row_173_official_proposal_topology_bayesfilter_blocker",
    ...
    return {
        ...
        "question": "row_173_time_43_official_proposal_topology_difference_audit",
        ...
        "canonical_input_manifest": _canonical_input_manifest(
            filterflow,
            initial_fingerprint,
            final_fingerprint,
        ),
        ...
        "run_manifest": {
            **environment_manifest(
                command=(
                    ...
                    "run_filterflow_float64_row_173_official_proposal_topology_tf"
                ),
...
def _blocked_payload(
    ...
    return {
        ...
        "comparator_drift": continuation._fingerprints_drifted(initial_fingerprint, final_fingerprint),
        "model_contract": _model_contract(),
        "canonical_input_manifest": _blocked_input_manifest(
            initial_fingerprint,
            final_fingerprint,
        ),
        "filterflow_probe": _compact_side(filterflow),
    ...
    return textwrap.dedent(
        ...
        def local_gradient_payload(tape, target, nodes):
            ...
            gradients = {{
                ...
                "target_to_internal_likelihood_mean": safe_gradient(
                    tape,
                    target,
                    nodes["internal_likelihood_mean"],
                ),
                ...
                "proposal_ll_to_internal_likelihood_mean": safe_gradient(
                    tape,
                    nodes["proposal_ll"],
                    nodes["internal_likelihood_mean"],
                ),
        ...
        def official_loglikelihood_with_internal_mean(proposal_model, proposed_state, prior_state, inputs, observation):
            capture = {{}}
            original_get = proposal_model._get_proposal_dist

            def wrapped_get(state_arg, observation_arg):
                distribution = original_get(state_arg, observation_arg)
                capture["mean"] = distribution_loc(distribution)
                return distribution

            proposal_model._get_proposal_dist = wrapped_get
            try:
                value = proposal_model.loglikelihood(
                    proposed_state,
                    prior_state,
                    inputs,
                    observation,
                ...
                proposal_model._get_proposal_dist = original_get
            if "mean" not in capture:
                raise RuntimeError("official FilterFlow internal proposal mean was not captured")
            return value, capture["mean"]
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            for time_index in range(T):
                ...
                proposal_ll, internal_likelihood_mean = official_loglikelihood_with_internal_mean(
                    smc._proposal_model,
                    proposed_state,
                    resampled_state,
                    inputs,
                ...
                if time_index in PROBE_TIMES:
                    ...
                    probe_records.append({{
                        ...
                        "nodes": {{
                            ...
                            "internal_likelihood_mean": internal_likelihood_mean,
...
def _bayesfilter_source_bundle(
    ...
    *,
    ...
    for time_index in range(vjp.T):
        ...
        proposed_particles = proposal_dist.sample(seed=seed2)
        sample_noise = proposed_particles - proposal_mean
        likelihood_particles_detached_sampling_mean = (
            proposed_particles - proposal_mean + tf.stop_gradient(proposal_mean)
        )
        proposal_dist_log_prob = proposal_dist.log_prob(proposed_particles)
        ...
        fresh_dist_log_prob = fresh_proposal_dist.log_prob(proposed_particles)
        detached_internal_mean = _optimal_proposal_mean(
            tf.stop_gradient(post_particles),
        ...
        detached_internal_dist = tfd.MultivariateNormalTriL(
            detached_internal_mean,
            sigma_chol,
        )
        detached_internal_dist_log_prob = detached_internal_dist.log_prob(
            proposed_particles
        )
        detached_sampling_mean_dist_log_prob = fresh_proposal_dist.log_prob(
            likelihood_particles_detached_sampling_mean
        )
        ...
        proposal_ll = _select_proposal_likelihood(
            ...
            proposal_dist_log_prob,
            fresh_dist_log_prob,
            detached_internal_dist_log_prob,
            detached_sampling_mean_dist_log_prob,
            helper_dist_log_prob,
        )
        internal_likelihood_mean = _select_internal_likelihood_mean(
            wiring_variant,
            time_index,
            proposal_mean,
            fresh_proposal_mean,
...
def _compare_sources(filterflow: dict[str, Any], bayesfilter: dict[str, Any]) -> dict[str, Any]:
    ...
    for variant in variant_comparisons.values():
        variant["primary_vjp_deltas"] = _primary_vjp_deltas(variant)
        variant["primary_vjp_matches_filterflow"] = all(
            delta <= GRADIENT_TOLERANCE
            for delta in variant["primary_vjp_deltas"].values()
        )
    ...
    primary_matching_variants = [
        variant
        for variant in variant_comparisons.values()
        if variant["primary_vjp_matches_filterflow"]
    ]
    best_primary_match = (
        min(primary_matching_variants, key=lambda row: row["max_abs_total_gradient_delta"])
        if primary_matching_variants
        else None
    )
    best_variant = min(
        variant_comparisons.values(),
        key=lambda row: row["max_abs_total_gradient_delta"],
    )
    material_global_reduction = (
        best_variant["max_abs_total_gradient_delta"]
        < direct["max_abs_total_gradient_delta"] - GRADIENT_TOLERANCE
    )
    global_gap_remaining = (
        best_variant["max_abs_total_gradient_delta"] > GRADIENT_TOLERANCE
    )
    return {
        "status": "compared",
        ...
        "primary_matching_variants": [
            variant["wiring_variant"] for variant in primary_matching_variants
        ],
        "any_primary_vjp_match": bool(primary_matching_variants),
        "best_primary_match_variant": (
            best_primary_match["wiring_variant"] if best_primary_match else None
        ),
        "best_variant_by_gradient_delta": best_variant["wiring_variant"],
        "direct_max_abs_total_gradient_delta": direct["max_abs_total_gradient_delta"],
        "best_variant_max_abs_total_gradient_delta": best_variant["max_abs_total_gradient_delta"],
        "best_primary_match_max_abs_total_gradient_delta": (
            best_primary_match["max_abs_total_gradient_delta"]
            if best_primary_match
            else None
        ),
        "material_global_reduction": material_global_reduction,
        "global_gap_remaining": global_gap_remaining,
        ...
        "interpretation": _interpret_source(
            ...
            direct_proposal_mean_vjp_delta=direct_proposal_mean_vjp_delta,
            variant_comparisons=variant_comparisons,
            primary_matching_variants=primary_matching_variants,
            best_primary_match=best_primary_match,
            global_gap_remaining=global_gap_remaining,
            material_global_reduction=material_global_reduction,
            helper_boundary=helper_boundary,
        ),
...
def _interpret_source(
    *,
    ...
    primary_matching_variants: list[dict[str, Any]],
    best_primary_match: dict[str, Any] | None,
    global_gap_remaining: bool,
    material_global_reduction: bool,
    helper_boundary: dict[str, Any],
) -> str:
    if not primary_matching_variants:
        return (
            "fresh_helper_boundary_material_or_unresolved"
            if not helper_boundary["not_material"]
            else "unresolved_official_topology_gap"
        )
    matching_names = {variant["wiring_variant"] for variant in primary_matching_variants}
    if "official_internal_mean_at_time_43" in matching_names:
        if global_gap_remaining and not material_global_reduction:
            return "internal_node_reconciles_official_vjp_global_gap_remains"
        return "internal_node_reconciles_official_vjp"
    if (
        "detached_sampling_mean_internal_at_time_43" in matching_names
        or "likelihood_particles_detached_sampling_mean_at_time_43" in matching_names
    ):
        return "state_object_topology_required"
    if best_primary_match and best_primary_match["wiring_variant"] in {
        "fresh_recomputed_distribution_at_time_43",
        "fresh_recomputed_distribution_all_times",
        "helper_function_recomputed_distribution_all_times",
    }:
        if global_gap_remaining and not material_global_reduction:
            return "functional_fresh_expression_reconciles_local_vjp_global_gap_remains"
        return "functional_fresh_expression_reconciles_local_vjp"
    if material_global_reduction:
        return "local_reconciled_global_gap_reduced"
    return "local_reconciled_global_gap_remains"


def _primary_vjp_deltas(comparison: dict[str, Any]) -> dict[str, float]:
    row = comparison["rows"][0]
    return {
        "proposal_ll_to_proposed_particles": _local_gradient_delta(
            row,
            "proposal_ll_to_proposed_particles",
        ),
        "proposal_ll_to_sampling_proposal_mean": _local_gradient_delta(
            row,
            "proposal_ll_to_proposal_mean",
        ),
        "proposal_ll_to_internal_likelihood_mean": _local_gradient_delta(
            row,
            "proposal_ll_to_internal_likelihood_mean",
        ),
    }
...
def _classify(comparison: dict[str, Any], veto_status: dict[str, Any]) -> dict[str, str]:
    ...
    mapping = {
        "internal_node_reconciles_official_vjp": (
            "h1_internal_node_reconciles_official_vjp",
            "BayesFilter reproduces the executable FilterFlow official proposal-likelihood VJP pattern using a fresh/internal likelihood mean",
        ),
        "internal_node_reconciles_official_vjp_global_gap_remains": (
            "h4_local_reconciled_global_gap_remains",
            "BayesFilter reproduces the local official proposal-likelihood VJP pattern, but the full row-gradient gap remains",
        ),
        "state_object_topology_required": (
        ...
        "functional_fresh_expression_reconciles_local_vjp": (
            "h3_functional_fresh_expression_reconciles_local_vjp",
            "BayesFilter inline/helper fresh proposal-likelihood expression reproduces the official VJP pattern",
        ),
        "functional_fresh_expression_reconciles_local_vjp_global_gap_remains": (
            "h4_local_reconciled_global_gap_remains",
            "BayesFilter fresh proposal-likelihood expression reproduces the local official VJP pattern, but the full row-gradient gap remains",
        ),
        "local_reconciled_global_gap_reduced": (
            "h3_functional_fresh_expression_reconciles_local_vjp",
            "a value-valid proposal-likelihood topology variant reduces the full row-gradient gap",
        ),
        "local_reconciled_global_gap_remains": (
            "h4_local_reconciled_global_gap_remains",
            "local official proposal-likelihood VJPs are reconciled, but the full row-gradient gap remains above tolerance",
        ),
        "fresh_helper_boundary_material_or_unresolved": (
            "h5_unresolved_official_topology_gap",
            "helper-function and inline fresh proposal-likelihood variants differ in internal-node VJPs; no tested topology fully reproduces the official pattern",
        ),
        "unresolved_official_topology_gap": (
            "h5_unresolved_official_topology_gap",
            "value-valid variants did not reproduce the executable FilterFlow official proposal-likelihood VJP pattern",
        ),
    }
...
def _veto_status(
    ...
    return {
        ...
        "helper_boundary_not_material_explanatory": comparison.get("helper_boundary", {}).get(
            "not_material",
            False,
        ),
...
def _decision(classification: dict[str, str], veto_status: dict[str, Any]) -> str:
    if classification["classification"] == "h1_forward_or_scalar_veto":
        return "filterflow_float64_row_173_official_proposal_topology_h1_forward_or_scalar_veto"
    if not veto_status.get("all_vetoes_clear", False):
        return "filterflow_float64_row_173_official_proposal_topology_blocked_or_vetoed"
    mapping = {
        "h1_internal_node_reconciles_official_vjp": (
            "filterflow_float64_row_173_official_proposal_topology_h1_internal_node_reconciles_official_vjp"
        ),
        "h2_state_object_topology_required": (
            "filterflow_float64_row_173_official_proposal_topology_h2_state_object_topology_required"
        ),
        "h3_functional_fresh_expression_reconciles_local_vjp": (
            "filterflow_float64_row_173_official_proposal_topology_h3_functional_fresh_expression_reconciles_local_vjp"
        ),
        "h4_local_reconciled_global_gap_remains": (
            "filterflow_float64_row_173_official_proposal_topology_h4_local_reconciled_global_gap_remains"
        ),
        "h5_unresolved_official_topology_gap": (
            "filterflow_float64_row_173_official_proposal_topology_h5_unresolved_official_topology_gap"
        ),
    }
    return mapping.get(
        classification["classification"],
        "filterflow_float64_row_173_official_proposal_topology_blocked_or_vetoed",
    )
...
def _select_proposal_likelihood(
    ...
    detached_internal_dist_log_prob: tf.Tensor,
    detached_sampling_mean_dist_log_prob: tf.Tensor,
    helper_dist_log_prob: tf.Tensor,
) -> tf.Tensor:
    if wiring_variant == "direct_sampled_distribution":
        return direct_dist_log_prob
    if wiring_variant == "official_internal_mean_at_time_43":
        return fresh_dist_log_prob if time_index == PROBE_TIME else direct_dist_log_prob
    if wiring_variant == "official_internal_mean_all_times":
        return fresh_dist_log_prob
    if wiring_variant == "detached_sampling_mean_internal_at_time_43":
        return (
            detached_internal_dist_log_prob
            if time_index == PROBE_TIME
            else direct_dist_log_prob
        )
    if wiring_variant == "likelihood_particles_detached_sampling_mean_at_time_43":
        return (
            detached_sampling_mean_dist_log_prob
            if time_index == PROBE_TIME
            else direct_dist_log_prob
        )
    if wiring_variant == "likelihood_particles_detached_sampling_mean_all_times":
        return detached_sampling_mean_dist_log_prob
...
def _select_internal_likelihood_mean(
    wiring_variant: str,
    time_index: int,
    direct_mean: tf.Tensor,
    fresh_mean: tf.Tensor,
    detached_internal_mean: tf.Tensor,
) -> tf.Tensor:
    if wiring_variant == "direct_sampled_distribution":
        return direct_mean
    if wiring_variant == "official_internal_mean_at_time_43":
        return fresh_mean if time_index == PROBE_TIME else direct_mean
    if wiring_variant == "official_internal_mean_all_times":
        return fresh_mean
    if wiring_variant == "detached_sampling_mean_internal_at_time_43":
        return detached_internal_mean if time_index == PROBE_TIME else direct_mean
    if wiring_variant == "likelihood_particles_detached_sampling_mean_at_time_43":
        return fresh_mean if time_index == PROBE_TIME else direct_mean
    if wiring_variant == "likelihood_particles_detached_sampling_mean_all_times":
        return fresh_mean
...
def _local_gradient_payload(
    ...
    gradients = {
        ...
        "target_to_internal_likelihood_mean": _safe_gradient(
            tape,
            target,
            nodes["internal_likelihood_mean"],
        ),
        ...
        "proposal_ll_to_internal_likelihood_mean": _safe_gradient(
            tape,
            nodes["proposal_ll"],
            nodes["internal_likelihood_mean"],
        ),
...
def _model_contract() -> dict[str, Any]:
    return {
        ...
        "dtype": "float64",
        "comparison_scope": (
            "official proposal-likelihood topology variants against the executable "
...
def _canonical_input_manifest(
    filterflow: dict[str, Any],
    initial_fingerprint: dict[str, Any],
    final_fingerprint: dict[str, Any],
) -> dict[str, Any]:
    model = filterflow.get("model", {})
    transition_covariance_chol = model.get("transition_covariance_chol")
    observation_covariance_chol = model.get("observation_covariance_chol")
    return {
        "canonical_sources": {
            "primary_prior_json": (
                "experiments/dpf_implementation/reports/outputs/"
                "dpf_filterflow_float64_row_173_proposal_likelihood_wiring_2026-06-05.json"
            ),
            "prior_reviewed_result": (
                "docs/plans/"
        ...
        "batch_size": vjp.BATCH_SIZE,
        "horizon": vjp.T,
        "initial_particles_digest": stable_digest(model.get("initial_particles")),
        "observation_path_digest": stable_digest(model.get("observations")),
        "transition_covariance_chol_digest": stable_digest(transition_covariance_chol),
        "observation_covariance_chol_digest": stable_digest(observation_covariance_chol),
        "transition_covariance_chol": transition_covariance_chol,
        "observation_covariance_chol": observation_covariance_chol,
        "ess_threshold": vjp.RESAMPLING_NEFF,
        "transport_settings": {
            "epsilon": vjp.EPSILON,
            "scaling": vjp.SCALING,
            "convergence_threshold": vjp.CONVERGENCE_THRESHOLD,
            "max_iterations": vjp.MAX_ITERATIONS,
            "gradient_mode": "filterflow_clipped",
            "application_mode": "filterflow_all_rows",
        },
        "filterflow_fingerprint_initial": initial_fingerprint,
        "filterflow_fingerprint_final": final_fingerprint,
...
def _blocked_input_manifest(
    ...
    return {
        ...
        "batch_size": vjp.BATCH_SIZE,
        "horizon": vjp.T,
        "initial_particles_digest": "blocked",
        "observation_path_digest": "blocked",
        "transition_covariance_chol_digest": "blocked",
        "observation_covariance_chol_digest": "blocked",
        "ess_threshold": vjp.RESAMPLING_NEFF,
        "transport_settings": {
            "epsilon": vjp.EPSILON,
            "scaling": vjp.SCALING,
...
def _evidence_contract() -> dict[str, Any]:
    return {
        "question": (
            "Does FilterFlow-style freshly rebuilt official proposal-likelihood topology "
        ...
        "primary_criterion": (
            "with vetoes clear, classify whether BayesFilter reproduces "
            "FilterFlow's official proposal-likelihood VJP pattern for "
            "proposal_ll_to_proposed_particles, "
            "proposal_ll_to_sampling_proposal_mean, and "
            "proposal_ll_to_internal_likelihood_mean; record whether the "
            "full row-gradient gap remains"
        ),
        "veto_diagnostics": [
            "comparator drift",
        ...
        "explanatory_diagnostics": [
            "direct versus internal/fresh local proposal-likelihood VJP deltas",
            "official-internal, fresh-at-time-43, and fresh-all-times total row-gradient deltas",
            "helper-function versus inline fresh topology deltas",
            "target adjoints to sampling proposal mean, internal likelihood mean, and proposed particles",
...
def _next_action(classification: str) -> str:
    ...
    if classification == "h1_internal_node_reconciles_official_vjp":
        return "record the internal-node topology as locally reconciled and test the next downstream/global gradient source"
    if classification == "h2_state_object_topology_required":
        return "mirror the executable state-object proposal-likelihood topology in the next value-preserving BayesFilter probe"
    if classification == "h3_functional_fresh_expression_reconciles_local_vjp":
        return "carry the functional fresh-expression topology into the next row/time confirmation probe"
    if classification == "h4_local_reconciled_global_gap_remains":
        return "trace the next downstream/global source because official proposal-likelihood topology is local-only"
    if classification == "h4_helper_boundary_not_material":
        return "record helper-boundary evidence as explanatory and continue with the primary topology outcome"
    if classification == "h5_unresolved_official_topology_gap":
        return "trace the remaining official proposal-likelihood graph topology with smaller watched tensors"
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    canonical = payload["canonical_input_manifest"]
    for key in (
        "mesh_index",
        "target_time_index",
        "probe_times",
        ...
        "batch_size",
        "horizon",
        "initial_particles_digest",
        "observation_path_digest",
        "transition_covariance_chol_digest",
        "observation_covariance_chol_digest",
        "ess_threshold",
        "transport_settings",
        "filterflow_fingerprint_initial",
        "filterflow_fingerprint_final",
        ...
        if key not in canonical:
            raise ValueError(f"canonical input manifest missing {key}")
    ...
    allowed = {
        ...
        "filterflow_float64_row_173_official_proposal_topology_bayesfilter_blocker",
        "filterflow_float64_row_173_official_proposal_topology_blocked_or_vetoed",
        "filterflow_float64_row_173_official_proposal_topology_h1_forward_or_scalar_veto",
        "filterflow_float64_row_173_official_proposal_topology_h1_internal_node_reconciles_official_vjp",
        "filterflow_float64_row_173_official_proposal_topology_h2_state_object_topology_required",
        "filterflow_float64_row_173_official_proposal_topology_h3_functional_fresh_expression_reconciles_local_vjp",
        "filterflow_float64_row_173_official_proposal_topology_h4_local_reconciled_global_gap_remains",
