"""Generate an expanded retained-teacher Sinkhorn teacher-data artifact on deterministic LGSSM clouds."""
...
JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_teacher_data_expanded_2026-06-18.json"
REPORT_PATH = REPORT_DIR / "retained-teacher-sinkhorn-teacher-data-expanded-2026-06-18.md"
RESULT_PATH = "docs/plans/bayesfilter-neural-ot-retained-teacher-teacher-data-expanded-result-2026-06-18.md"
PLAN_PATH = "docs/plans/bayesfilter-neural-ot-retained-teacher-expanded-low-budget-eval-plan-2026-06-18.md"
TRAIN_SEEDS = (444, 445, 446, 447, 448, 449)
HELDOUT_SEEDS = (544, 545, 546, 547)
NUM_PARTICLES = 32
...
EXPECTED_DECISION = "RETAINED_TEACHER_SINKHORN_TEACHER_DATA_EXPANDED_READY"
...
def _run() -> dict[str, Any]:
    ...
    decision = EXPECTED_DECISION if _all_examples_finite(examples) and examples else "RETAINED_TEACHER_SINKHORN_TEACHER_DATA_EXPANDED_FAILED"
    split_counts = _split_counts(examples)
    payload = {
        "decision": decision,
        "question": "Can BayesFilter generate a modestly expanded retained-teacher Sinkhorn latent dataset on deterministic LGSSM weighted clouds?",
        ...
        "run_manifest": environment_manifest(
            command="CUDA_VISIBLE_DEVICES=-1 python -m experiments.dpf_implementation.tf_tfp.runners.run_retained_teacher_sinkhorn_teacher_data_expanded_tf",
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["counts"]["heldout_examples"] <= 2:
        raise RuntimeError("expanded artifact did not increase heldout coverage meaningfully")
    if "reproducibility_digest" not in payload:
        raise RuntimeError("missing reproducibility digest")


def _markdown(payload: dict[str, Any]) -> str:
    return f"""# Retained-Teacher Sinkhorn Expanded Teacher-Data Result
