"""Generate retained-teacher Sinkhorn teacher data on range-bearing clouds."""
...
JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_teacher_data_range_bearing_2026-06-18.json"
REPORT_PATH = REPORT_DIR / "retained-teacher-sinkhorn-teacher-data-range-bearing-2026-06-18.md"
RESULT_PATH = "docs/plans/bayesfilter-neural-ot-retained-teacher-teacher-data-range-bearing-result-2026-06-18.md"
...
HELDOUT_SEEDS = (444, 555)
NUM_PARTICLES = 24
ESS_THRESHOLD_RATIO = 0.5
TEACHER_EPSILON = 0.75
TEACHER_MAX_ITERATIONS = 90
TEACHER_TOLERANCE = 1e-8
EXPECTED_DECISION = "RETAINED_TEACHER_SINKHORN_TEACHER_DATA_RANGE_BEARING_READY"
...
def _run() -> dict[str, Any]:
    ...
    decision = EXPECTED_DECISION if _all_examples_finite(examples) and examples else "RETAINED_TEACHER_SINKHORN_TEACHER_DATA_RANGE_BEARING_FAILED"
    payload = {
        "decision": decision,
        "question": "Can BayesFilter generate a retained-teacher Sinkhorn latent dataset on deterministic range-bearing weighted clouds?",
        ...
        "run_manifest": environment_manifest(
            command="CUDA_VISIBLE_DEVICES=-1 python -m experiments.dpf_implementation.tf_tfp.runners.run_retained_teacher_sinkhorn_teacher_data_range_bearing_tf",
...
def _capture_examples_for_seed(fixture, *, seed: int, split: str) -> list[dict[str, Any]]:
    ...
    for time_index, observation in enumerate(tf.unstack(fixture.observations, axis=0)):
        particles = sample_transition_particles_tf(fixture, particles, seed=seed, time_index=time_index)
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["counts"]["heldout_examples"] <= 0:
        raise RuntimeError("range-bearing artifact captured no heldout examples")
    if "reproducibility_digest" not in payload:
        raise RuntimeError("missing reproducibility digest")


def _markdown(payload: dict[str, Any]) -> str:
    return f"""# Retained-Teacher Sinkhorn Range-Bearing Teacher-Data Result
