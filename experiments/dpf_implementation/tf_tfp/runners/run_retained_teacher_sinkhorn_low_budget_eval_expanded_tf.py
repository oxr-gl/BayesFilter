...
JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_low_budget_eval_expanded_better_contract_2026-06-29.json"
REPORT_PATH = REPORT_DIR / "retained-teacher-sinkhorn-low-budget-eval-expanded-better-contract-2026-06-29.md"
RESULT_PATH = "docs/plans/bayesfilter-neural-ot-metaot-refit-broader-better-contract-result-2026-06-29.md"
...
EXPECTED_DECISION = "RETAINED_TEACHER_SINKHORN_EXPANDED_HELDOUT_LOCAL_USEFULNESS_ON_DISCRIMINATING_BUDGETS"
...
def _run() -> dict[str, Any]:
    ...
    if scalar(final_train_loss) >= scalar(initial_train_loss):
        decision = "RETAINED_TEACHER_SINKHORN_EXPANDED_HELDOUT_OBJECTIVE_ROUTE_FAILED"
        ...
        for budget in PRIMARY_BUDGETS:
            ...
            if metrics["mean_student_teacher_cloud_rmse"] > metrics["mean_zero_teacher_cloud_rmse"]:
                decision = "RETAINED_TEACHER_SINKHORN_EXPANDED_HELDOUT_NON_PROMOTED_ON_DISCRIMINATING_BUDGETS"
                break

    payload = {
        "decision": decision,
        "question": "Does donor-aligned one-half retained Sinkhorn show local usefulness on a broader heldout split when the evidence contract is anchored to discriminating budgets rather than saturated zero-init rungs?",
...
def _validate_payload(payload: dict[str, Any]) -> None:
    if payload["decision"] not in {
        EXPECTED_DECISION,
        "RETAINED_TEACHER_SINKHORN_EXPANDED_HELDOUT_NON_PROMOTED_ON_DISCRIMINATING_BUDGETS",
    }:
