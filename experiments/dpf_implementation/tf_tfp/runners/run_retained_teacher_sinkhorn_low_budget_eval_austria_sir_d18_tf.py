"""Run low-budget heldout evaluation for retained-teacher Sinkhorn warm-start on Austria SIR d18 teacher data."""
...
JSON_PATH = OUTPUT_DIR / "retained_teacher_sinkhorn_low_budget_eval_austria_sir_d18_2026-07-03.json"
REPORT_PATH = REPORT_DIR / "retained-teacher-sinkhorn-low-budget-eval-austria-sir-d18-2026-07-03.md"
RESULT_PATH = "docs/plans/bayesfilter-neural-ot-austria-sir-d18-retained-teacher-low-budget-eval-result-2026-07-03.md"
PLAN_PATH = "docs/plans/bayesfilter-neural-ot-austria-sir-d18-retained-teacher-smoke-test-plan-2026-07-03.md"
SCALAR_ID = "retained_teacher_sinkhorn_low_budget_teacher_cloud_fidelity_austria_sir_d18_tf"
TRAINING_SEED = 20260703
EPOCHS = 250
LEARNING_RATE = 1e-2
PRIMARY_BUDGETS = (2, 3)
EXPLANATORY_BUDGETS = (5,)
BUDGET_TOLERANCE_FLOORS = {
    2: 3e-4,
    3: 1e-4,
    5: 1e-5,
}
EXPECTED_DECISION = "RETAINED_TEACHER_SINKHORN_AUSTRIA_SIR_D18_LOCAL_USEFULNESS_ON_DISCRIMINATING_BUDGETS"
...
def _run() -> dict[str, Any]:
    ...
    if not train_examples or not heldout_examples:
        raise RuntimeError("Austria SIR d18 teacher-data artifact must include both train and heldout examples")
    ...
    if scalar(final_train_loss) >= scalar(initial_train_loss):
        decision = "RETAINED_TEACHER_SINKHORN_AUSTRIA_SIR_D18_OBJECTIVE_ROUTE_FAILED"
        ...
        for budget in PRIMARY_BUDGETS:
            ...
            if metrics["mean_student_teacher_cloud_rmse"] > metrics["mean_zero_teacher_cloud_rmse"]:
                decision = "RETAINED_TEACHER_SINKHORN_AUSTRIA_SIR_D18_NON_PROMOTED_ON_DISCRIMINATING_BUDGETS"
                break
    ...
    payload = {
        "decision": decision,
        "question": "On Austria SIR d18 heldout teacher-data examples, does donor-aligned one-half retained Sinkhorn show local usefulness when the evaluation is restricted to discriminating budgets?",
        ...
        "run_manifest": environment_manifest(
            command="CUDA_VISIBLE_DEVICES=-1 python -m experiments.dpf_implementation.tf_tfp.runners.run_retained_teacher_sinkhorn_low_budget_eval_austria_sir_d18_tf",
