function g = matvec_dstiff_sol_sq(mesh, local_elem, kappa_type, kappa, d_kappa, state)
...
switch kappa_type
    case {'scalar'}
        loc_ka  = local_elem.f_quad_pts*kappa(fill_i);
        loc_dka = local_elem.f_quad_pts*d_kappa(fill_i);
        for di = 1:nd
            dksi = 2*loc_dka.*loc_ka.*grad_s{di};
            for dj = 1:nd
                geo = reshape(mesh.detJ,1,[]).*reshape(mesh.inv_Jt{di,dj},1,[]);
                tmp = (local_elem.grad_f_quad_pts{dj}.*local_elem.quad_weights(:))'*(dksi.*geo);
                fill = fill + tmp;
            end
        end
    case {'vector'}               
        for di = 1:nd
            k  = kappa(:,di);
            dk = d_kappa(:,di);
            loc_ka  = local_elem.f_quad_pts*k(fill_i);
            loc_dka = local_elem.f_quad_pts*dk(fill_i);
            dksi    = 2*loc_dka.*loc_ka.*grad_s{di};
            for dj = 1:nd
                geo = reshape(mesh.detJ,1,[]).*reshape(mesh.inv_Jt{di,dj},1,[]);
                tmp = (local_elem.grad_f_quad_pts{dj}.*local_elem.quad_weights(:))'*(dksi.*geo);
