...
opt = SparseOption();
opt.tol = 2e-2;  % 1e-2 for log-uni
opt.init_total_degree = 2;  % 5
...
%     r = ttrankest(base, evalcnt_poly(irun)/Nbetas_poly(irun)/2);
    nswp = ttrankest_amen(base, kickrank, evalcnt_poly(irun)/Nbetas_poly(irun));
    fprintf('Matched nswp %d, TT rank %d\n', nswp, kickrank*(nswp+1));
    tt_opt = TTOption('tt_method', 'amen', ...
        'als_tol', 0, 'local_tol', 1e-12, 'kick_rank', kickrank, 'max_rank', kickrank*(nswp+1), 'max_als', nswp, 'init_rank', kickrank);
